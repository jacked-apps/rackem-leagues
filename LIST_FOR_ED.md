...
## 🐞 2026-09-05 — SWEEP: `loadingText` set without `isLoading` (double-submit risk)

Found while fixing the duplicate team chats. Our `Button` only shows a spinner
and disables itself when it receives an explicit **`isLoading`** prop —
`loadingText` alone does nothing. So a button with `loadingText="Saving..."` and
no `isLoading` stays live during its request, and a double-tap fires the action
twice.

That is exactly how a captain got two identical team chats (fixed), and it's the
same shape as the PWA "Update Now" button that appeared dead for days (fixed).
**Both were found by a user, not by a test.**

15+ other files have the same pattern. Some are safe because they also pass
`disabled={isSaving}`; the rest are not:

- `src/components/messages/NewMessageModal.tsx` — "Creating..." (creates a
  conversation — double-submit means duplicate DMs)
- `src/components/messages/AnnouncementModal.tsx` — "Sending..." (double-send)
- `src/components/operator/OrganizationStaffCard.tsx` — "Adding..."
- `src/components/lineup/LineupActions.tsx` — "Locking..."
- `src/components/lineup/FargoStartPointsCard.tsx` — "Saving..." / "Confirming..."
- `src/components/lineup/OpponentSubstituteModal.tsx` — "Confirming..."
- `src/components/scoring/ConfirmationDialog.tsx` — "Confirming..."
- `src/components/operator/ContentModerationCard.tsx` — "Saving..."
- …plus others; the sweep command is below.

**Worth fixing properly, not one at a time.** Options, roughly in order of how
much they'd prevent recurrence:

1. Make `Button` accept an async `onClick` and manage its own in-flight state —
   then the whole class of bug disappears and `isLoading` becomes optional.
2. A lint rule: `loadingText` present and not `"none"` ⇒ `isLoading` or
   `disabled` required.
3. Fix the current 15 by hand and rely on review.

(1) or (2) — (3) is what we've been doing implicitly and it has now produced two
user-visible bugs.

Find them with:

```
grep -rln 'loadingText="' src --include="*.tsx" | grep -v __tests__ | while read f; do
  if grep -q 'loadingText="[^n]' "$f" && ! grep -q "isLoading=" "$f"; then echo "$f"; fi
done
```
