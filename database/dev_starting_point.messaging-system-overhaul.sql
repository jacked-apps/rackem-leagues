...
-- ============================================================================
-- BRANCH-SCOPED COPY: messaging-system-overhaul
--
-- This is a working copy of database/dev_starting_point.sql with branch-local
-- fixes applied. Reasons it exists:
--   * The canonical file on main gets broken every time a schema change drops
--     a column the seed inserts into. Multiple parallel branches end up trying
--     to "fix" the same file, producing merge conflicts.
--   * Branch-local copies sidestep that: each feature branch maintains its
--     own working copy without touching main's file.
--
-- When this PR merges to main, DELETE this file. Main's dev_starting_point.sql
-- (which the messaging-overhaul PR does NOT modify) will then have the same
-- fixes applied by whatever branch lands first.
--
-- 2026-05-11 branch-local fix: removed `team_format` column references from
-- both `leagues` INSERT blocks (column was dropped by migration
-- 20260502000000_drop_team_format.sql — modular league system v2 PR).
--
