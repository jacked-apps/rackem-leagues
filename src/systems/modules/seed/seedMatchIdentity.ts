 ...
 *
 * Reads the match's identity fields from `context.matchData` and
 * writes them to the state bag: match ID, team IDs, season ID.
 * Downstream modules (team-bonus needs season+teams; scoreboard,
 * confirm, swap will all want match_id) read these from the bag.
 *
 ...
 * Missing fields produce null bag writes (never throws).
 */
...
/**
 * Writes `match_id`, `home_team_id`, `away_team_id`, `season_id` to
 * the bag. If a field is missing or null in matchData, writes `null`
 * for that key — never throws.
 */
