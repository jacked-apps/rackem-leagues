...
const seats = (devices: number, open: number) => ({ devices, hosts: 1, open });

describe('seatCopy', () => {
  it('pluralises empty seats and names zero', () => {
    expect(emptySeatsLabel(1)).toBe('1 empty seat');
    expect(emptySeatsLabel(2)).toBe('2 empty seats');
    expect(emptySeatsLabel(0)).toBe('no empty seats');
  });

  it('a free room says how to get seats instead of counting them', () => {
    expect(seatLine(seats(3, 1), true)).toBe('3 here · 1 empty seat');
...
describe('SeatCounter', () => {
  it('shared room: shows the count and the tap invites', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /3 here · 1 empty seat/ }));
  ...
  it('shared room with no seats: still a tap (the sheet explains)', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /no empty seats/ }));
    expect(onInvite).toHaveBeenCalledTimes(1);
  });
