...
## Pending: Advanced Settings Documentation / Education

**Priority**: Required before releasing advanced settings to users
**Context**: The advanced settings warning dialog tells users to "understand how the system interprets and uses this data" before making changes. We need to provide the resources for them to actually learn this.

### What's Needed

Detailed documentation accessible from the Match Data Page that explains:

1. **How Threshold Charts Work**
   - What a threshold chart is and why it exists
   - How handicap difference maps to games-to-win/tie
   - Points vs percentage chart differences
   - How the chart resolution chain works (season → league → org → global fallback)
   - What happens when you change or override a chart

2. **How Game Generation Works**
   - Single round robin vs double round robin explained
   - How break/rack rotation is determined
   - How player matchup order is calculated
   - What happens with different lineup sizes (3v3 = 18 games, 5v5 = 25 games)
   - How generated games connect to player self-scoring

3. **How Handicap Systems Work**
   - Points-based handicapping (3v3 / integer diff)
   - Percentage-based handicapping (5v5 / BCA-style)
   - Custom mode — when to use it and implications
   - How handicaps feed into threshold generation

4. **Impact of Changing Settings**
   - What happens if lineup size doesn't match the game generation method
   - What happens if handicap type doesn't match the threshold chart type
   - How mid-season changes affect automation and player scoring
   - Which combinations are supported vs unsupported

### Integration Points

The documentation/education content must be accessible from two specific places on the Match Data Page:

1. **Advanced Settings Warning AlertDialog** — When the user clicks "Advanced Settings" and sees the warning dialog, include a "Learn More" or documentation link so they can study how the system works before proceeding.
2. **Advanced Settings Section** — Within the advanced settings area itself (the SetupOptions / ThresholdsSection controls), provide contextual documentation links or InfoButtons so users can reference how each setting works while making changes.

### Delivery Format (TBD)

Options to discuss:
- **In-app help pages** — dedicated route with documentation sections
- **InfoButton tooltips** — contextual help on each setting (partially done)
- **External docs site** — linked from the app
- **Guided walkthrough** — step-by-step first-time user experience

### Goal

Users should be able to study these materials and confidently adjust advanced settings when their league format requires it, without accidentally breaking automation or player-facing scoring flows.
