...
> **Last Updated**: 2026-09-16 (PLANNED the Game Room — `docs/plans/2026-09-16-001-feat-game-room-plan.md`, 7 units. Review-driven decisions worth knowing: guests use Supabase anonymous sign-in and the app must treat `is_anonymous` as NOT logged in (Login/ProtectedRoute/nav), join page offers Sign in vs Continue-as-guest before minting anything, `get_room_by_token` is anon-readable, heartbeats ignored on both tables + room row bumped at most every 5 min, ended state is data-derived (getRoom null), every SUBSCRIBED refetches, `join_room` locks the room row, table validation also checks reserved names + uuid type + cascade FK, `set_room_settings` changes settings without wiping, flip call goes through `call_room_coin` and is frozen after the throw, stale anonymous users swept by cron. Also earlier today: brainstormed the room (superseding the 09-10 doc) and fixed the 2 db tests red on main since #280.)
