...
| `supabase/migrations/20260910181344_race_engine.sql` | **RACE ENGINE** — `start_race` (name who breaks, write game 1), `record_race_game` (enter a result + vouch), `confirm_race_game` (the other player agrees → official → race finishes OR grows its next game), `vacate_race_game` (wipe a result, un-finish the race) | The append lives SERVER-side inside the confirming transaction, under `FOR UPDATE` on the race row — both phones hit the gate at the same instant, so a client-side append would write two game 6s. `race_standing` walks games IN ORDER and stops at the deciding game: "first to reach their goal" ≠ "has the most wins" once a vacated middle game is re-scored. Entering a new result CLEARS the opponent's confirmation (they agreed to the old one). All functions revoked from PUBLIC **and** `anon` (two separate default grants); `race_advance` revoked from `authenticated` too — it never checks who is calling |
