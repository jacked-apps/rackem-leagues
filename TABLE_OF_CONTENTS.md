...
> **Last Updated**: 2026-06-04 (Per-Game Allocator Room **Unit 7 SHIPPED**: league-settings picker UI + apply-time preview. New `AllocatorPicker.tsx` card mounted on LeagueSettings page — surfaces current pick, lets LO choose from officials + their own variations + "Use prepackaged default" (NULL). On select, `applyTimePreview.ts` runs the variation through the league's prepackaged composition over a synthetic 5-game match; surfaces NaN/Infinity, negative-points, validator rejection inline. Clean preview shows green; warnings show yellow but allow Apply; structural failure blocks Apply. On Apply, upserts `preferences.per_game_allocator_id` for the league. 7 new applyTimePreview tests including unregistered-formula rejection. Build green. Branch `feat/per-game-allocator-workshop`.)
