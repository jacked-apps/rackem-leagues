...
- `useInstallApp.ts` - **PWA install capability** — captures the Android/desktop-Chrome `beforeinstallprompt` (single-use native prompt), detects iOS + already-installed (standalone), exposes `{ isStandalone, canPromptInstall, platform, promptInstall }`. Backs `InstallAppCard`. Tested in `useInstallApp.test.ts`.
