...
> **Last Updated**: 2026-06-05 (Merged `main` into the workshop feature branch. Brings in #163 operator-help glossary, #164 doc archive of ~63 docs to `docs/archive/`, #165 onboarding cold-start cascade, #166 DB suite cleanup, #169 scoring-end stranded-client fix. The Per-Game Allocator Room work — `feat/per-game-allocator-workshop` — sits on top: Units 1-9 complete plus iteration on Ed's browser testing, including click-to-build formula UI, side-agnostic available data, base+formula split UI, and two 17-Point templates.) Prior 2026-06-02: archived ~63 completed/superseded brainstorm, plan & memory-bank docs to `docs/archive/` + `memory-bank/archive/` after a code+PR triage.
