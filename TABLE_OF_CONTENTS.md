...
- `api/hooks/useBrackets.ts` - **Tournament bracket tool / Unit 3.** TanStack Query hooks: `useBracket`/`useBracketShare` reads (staleTime 0 — live during play, realtime pushes invalidations) + `useCreateBracket`/`useSetParticipants`/`useStartBracket`/`useAdvanceWinner`/`useCloseBracket` mutations that invalidate the `queryKeys.brackets.*` keys.
