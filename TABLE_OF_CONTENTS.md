...
- `threshold/BuiltInView.tsx` - read-only "built-in calculation" view: shows the REAL formula (via `ReadOnlyFormula`) so the LO can SEE how a Fargo / games-needed / milestone threshold works; falls back to a blurb for chart-style built-ins. Not editable yet.
- `threshold/ReadOnlyFormula.tsx` - renders a formula as the same token pills the builder uses, non-interactive; symbols not yet exposed (`^`, `floor`, `ceil`, `|…|`) render as STAGNANT (dashed, locked) pills. `DisplayToken`/`FormulaLine` types.
- `threshold/builtinFormula.ts` - faithful read-only formula representations of the built-in ops (Fargo `2^(rating/100)` + floor-of-gap, games-needed ceil/floor, milestone round-product, read-a-pref). Display-only transparency layer; the real math still runs in the op.
- `threshold/__tests__/BuiltInView.test.tsx` - 4 cases: Fargo formula includes a locked `^`; every wired built-in has a representation; chart-style returns null; the view renders the formula + power symbol.
...
| `supabase/migrations/20260607000001_threshold_officials_seed.sql` | **Threshold workshop seed** — EVERY threshold the app uses, converted to the saveable shape as 10 learning templates, named encoding-first (points / percentage / fargo): games-to-win as a **chart** (editable, rows embedded) AND a **formula** (any size) for points + percentage; **Fargo start-points** AND **Fargo games-to-win**; the points lower-edge; **read-a-league-setting** + **milestone (percent-of)**; and a blank Empty Starter. DB test: `src/__tests__/database/threshold-officials-seed.db.test.ts` (all build + resolve; formula matches chart). |
