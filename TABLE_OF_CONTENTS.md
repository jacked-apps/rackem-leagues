...
> **Last Updated**: 2026-09-10 (REPAIRED the `db` test project — 11 failures across 8 files, all green now and green twice running. None were caused by recent work: CI runs only the `unit` project (a real Postgres is needed for `db`), so these tests execute only when someone runs them locally and had been rotting unseen. Causes were three kinds. (1) SCHEMA DRIFT the tests never learned about: `match_games.vacate_requested_by` became a varchar(4) side marker ('home'/'away') while the test still wrote a player uuid (22001), and messages gained `check_edit_time_limit`/`check_delete_time_limit` triggers while the RLS test still edited the OLDEST seeded message (P0001) — that one could never have passed. (2) FIXTURES THE SEED CANNOT CONTAIN: games are written at lineup lock, so every seeded match with games is also completed and appendConfirmation's search for a non-completed one with a game always came back empty; likewise NO seeded team has a placeholder captain, which three onboarding suites need. Those suites now BUILD what they need via new `src/test/onboardingFixtures.ts` and tear it down. (3) TESTS MUTATING SHARED SEED ROWS: members.rls picked whatever member came back first and rewrote its email, which fires `ensure_placeholder_invite_token` and left a CANCELLED invite behind — poisoning that member so the NEXT run collided on `unique_pending_invite` (23505), and the failure surfaced in whichever file touched it next. It owns its member now. The pg_net push assertion counted TOTAL queue+response rows, which pg_net's GC of aged-out rows could cancel out; it now uses a high-water-mark id and inspects the request inside a rolled-back transaction, where the worker cannot drain it. Also applied `supabase/seed_test_users.sql`, which had never been run against this database. SEPARATELY FOUND, NOT FIXED: `unique_pending_invite` is a plain unique index on (member_id, email, status) despite its name — cancelling a pending invite when a cancelled one already exists for that email raises 23505, reachable in production by editing a placeholder's email A→B→A. It likely wants to be partial (WHERE status='pending'). Needs Ed's call, since it is a migration. MADE CI RUN THEM, which is the fix that stops this recurring: new db-tests job in checks.yml boots a trimmed Supabase (Postgres + Kong + PostgREST + Auth; studio/storage/realtime/edge/logs excluded), loads the seed and runs the db project, plus a test:db script. REGENERATED supabase/seed.sql, which was the blocker — the committed one was a stale snapshot carrying 0 leagues and 0 match_games while still holding 6 seasons that referenced them, internally inconsistent because it was dumped with FKs disabled. CI on that seed could only fail. The replacement is dumped from the working local database and verified by restoring into a scratch database built from the current schema: 0 errors, leagues=3, match_games=2176, auth users=8.)
