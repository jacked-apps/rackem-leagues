...
| `docs/brainstorms/2026-06-20-lms-schedule-import-requirements.md` | Requirements for importing a schedule (LMS-format .xlsx) into our app — reverse of the export; operator chooses generate-from-our-chart OR import-a-file (LMS export or our edited sample) | File-sourced matchup generation: teams + weeks pre-exist, unplayed-only, team-name reconciliation + review/accept-cancel, home/away from `@`, sample carries our real team names (near-auto reconcile). Feasibility proven; building now. Branch `feat/lms-schedule-import` |
| `/docs/plans/` | **CE implementation plans** | Output of `/compound-engineering:ce-plan` |
| `docs/archive/plans/2026-04-17-001-feat-official-rulebook-reader-plan.md` | Implementation plan for the Official Rulebook Reader | 6 units, active branch `feature/official-rulebook-reader` |
...
#### LMS Schedule Export + Import (`/utils/lmsExport/`, `/utils/lmsImport/`, `/utils/xlsx/`, `/utils/download.ts`)
...
- `xlsx/readXlsx.ts` - **Minimal generic .xlsx reader** (counterpart to writeXlsx). Unzips with fflate → `{ [sheetName]: string[][] }`; resolves the **shared-strings** table LMS uses AND inline strings we write. Targeted-regex XML parse (no XML-lib dep). Validated against a real LMS export. Tested in `__tests__/readXlsx.test.ts`.
- `lmsImport/parseLmsSchedule.ts` - Inverse of the export: raw `readXlsx` sheets → `{ teams: {number,name}[], weeks: {weekNumber, matches:[{awayNumber,homeNumber}]}[] }` with validation (missing sheet, malformed `away @ home` cell, unknown team #). Tested in `__tests__/parseLmsSchedule.test.ts` (6). LMS schedule IMPORT feature, `feat/lms-schedule-import` (reader+parser done; reconciliation/write/UI next).
