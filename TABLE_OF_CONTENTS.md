...
> **Last Updated**: 2026-06-11 (Mid-season drop: added `supabase/migrations/20260611000001_drop_team_mid_season_rpc.sql` — atomic `drop_team_mid_season` RPC that withdraws a team that has already played (keeping its past results), creates a fresh BYE, and rewires its UNPLAYED matches+lineups onto the BYE; wired into `TeamManagement.tsx`'s drop flow (mid-season branch) + DB test `dropTeamMidSeason.db.test.ts`. Completes the Drop Team flow (pre + mid season). Needs `feat/auto-forfeit-sweep` to actually forfeit the resulting BYE matches. Branch `feat/drop-team-to-bye`.) Prior 2026-06-11: (Drop-team-to-BYE (pre-season): added `convertTeamToBye` to `src/api/mutations/teams.ts` — reshapes an active team into the canonical BYE in place (the inverse of PR #207's fill-the-bye), releasing its roster and leaving its matches untouched so the slot count stays even with NO reschedule; guards on one-bye-per-season + no-played-matches. Wired into `TeamManagement.tsx`'s delete/drop flow + DB test `convertTeamToBye.db.test.ts`. Branch `feat/drop-team-to-bye`, stacked on `feat/fill-the-bye` (#207).) Prior 2026-06-07: (Trigger workshop landed — second standalone module workshop in the Workshops building: `triggers` table + 4 officials, never-throw `loadTrigger`, standalone `validateTrigger`, shared `_shared/ExpressionBuilder` extracted from the allocator's FormulaBuilder, full trigger workshop UI + route. Merged current main in to reconcile the stacked branch. Branch `feat/trigger-room`.) Prior 2026-06-06: (Added `docs/file-split-backlog.md` — ranked refactor backlog of files worth splitting, from a codebase sweep. Branch `docs/file-split-backlog`.) Prior 2026-06-05: (Added Facebook OAuth login alongside Google in `Login.tsx`/`Register.tsx` and shipped a public Privacy Policy at `/privacy` — `about/PrivacyPolicy.tsx` — required for Facebook app review. Branch `feat/fb-login`.) Prior 2026-06-02: (Archived ~63 completed/superseded brainstorm, plan & memory-bank docs to `docs/archive/` + `memory-bank/archive/` after a code+PR triage; active/back-burner/reference docs left in place.)
...
- `convertTeamToBye.db.test.ts` - **Pre-season drop → BYE.** Verifies `convertTeamToBye` (`api/mutations/teams.ts`) reshapes an active team into the canonical bye (status='bye', captain/name/venue cleared), releases its roster, and leaves its matches untouched (so they become bye weeks) — plus the guards (only one bye per season, refuse if matches already played). Self-builds + tears down its own org→league→season→teams fixture. Requires local Supabase. Stacked on PR #207 (fill-the-bye) — this is the inverse direction.
- `dropTeamMidSeason.db.test.ts` - **Mid-season drop.** Verifies the `drop_team_mid_season` RPC: withdraws the team (keeps its row + past results), creates a fresh BYE, rewires only the UNPLAYED matches (home + away) and their lineups onto the BYE, and leaves PLAYED matches untouched — plus the one-BYE-per-season guard. Requires local Supabase + the 20260611000001 migration.
...
| `supabase/migrations/20260611000001_drop_team_mid_season_rpc.sql` | **Mid-season drop.** `drop_team_mid_season(p_team_id, p_season_id)` SECURITY DEFINER RPC — atomically withdraws a team that has already played (`status='withdrawn'`, keeping its row + past results), creates a fresh BYE, and rewires every UNPLAYED match (home + away) and its lineups onto the BYE so future opponents get bye weeks. Played matches untouched. Guards: team active, one BYE per season. Returns the new BYE id. The resulting BYE matches are forfeited by `sweep_auto_forfeits()` (separate `feat/auto-forfeit-sweep` branch). Test: `src/__tests__/database/dropTeamMidSeason.db.test.ts`. |
