...
| `/docs/plans/` | **CE implementation plans** | Output of `/compound-engineering:ce-plan` |
| `docs/plans/2026-09-04-001-feat-tournament-paid-foundation-plan.md` | Implementation plan for the Tournaments PAID-tier FOUNDATION (roadmap #1) — 10 units / 4 phases (A tier gate + paid backbone, B identity registered-link/walk-up, C roster+hopper+self-add, D setup + run-count) | Reviewed by 7 personas + reshaped with Ed; tournaments disposable; walk-ups tournament-scoped (no shared-merge surgery); builds on free-tier (now in main); **Units A1–A2 BUILT** (schema + per-player `payment_methods` card-on-file + the create-page Premium-features checklist & verify-card popup); branch `feat/tournament-paid-foundation` |
...
- `paid/PremiumFeaturesSection.tsx` - **PAID A2** — the "Premium features" checklist on the create page. Each feature is a checkbox + blurb + preview price; turning one ON opens `PremiumFeatureDialog`, OFF is immediate; shows the preview total + "not charged until you start" once a card's on file. Test: `PremiumFeaturesSection.test.tsx`.
- `paid/PremiumFeatureDialog.tsx` - **PAID A2** — the verify/confirm popup for enabling a premium feature: feature blurb + preview price + "you won't be charged until you start". No card on file → embeds `PaymentCardForm` ($0 mock verify); card already on file → a one-tap confirm (reuses the player's card).
...
| `supabase/migrations/20260905163726_bracket_paid_foundation.sql` | **Tournament PAID foundation — Phase A, Unit A1 (paid schema).** Adds to `brackets`: `tier` (free\|paid, CHECK), `premium_features` (text[], the organizer's checklist), `game_type`. A CHECK (`brackets_premium_implies_paid_check`) enforces the tier↔premium_features invariant (any feature checked ⇒ tier=paid). **The card-on-file does NOT live here** — it's a per-player asset in `payment_methods`; the FK `brackets.payment_method_id` (added in the payment_methods migration) points at the card to charge. **No sweep change** — tournaments stay disposable in BOTH tiers. All columns nullable/defaulted so free brackets are unaffected. Test: `brackets.paid.schema.db.test.ts`. |
| `supabase/migrations/20260905172511_payment_methods.sql` | **Per-player card-on-file** (tournament paid foundation, Phase A). New `payment_methods` table: one saved card per row, **owned by `member_id`**, reusable for anything the player pays for (tournaments, dues, ...) — matches Stripe's model (charges reference a card by id). Cols: `stripe_customer_id`, `stripe_payment_method_id` (token), `card_last4`, `card_brand`, `verified_at`, `is_default` (partial-unique: one default per member). Adds `brackets.payment_method_id` FK (ON DELETE SET NULL) = the card charged at Start. Mock today; Jack's Stripe install backs the token fields. The LO org card (`organizations`) is left as-is (future consolidation points it here). Test: `payment_methods.schema.db.test.ts`. |
