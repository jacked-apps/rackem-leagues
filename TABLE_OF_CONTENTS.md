...
> **Last Updated**: 2026-09-16 (**The link colour means PRESSABLE — `TeamNameLink` now covers standings, both teams on a schedule card, both on a match result card, the re-up page and a player's profile.** Left off where the roster is already alongside (team stats, team cards, My Teams) and off the My Match panel, where the name sits inside a nav link. Spreading it surfaced two violations of the rule: a schedule card used the link blue for "this is your team" and a result card used it for "this team won" — both on unpressable text. The first now says **"Your team" in words** (which also survives colourblindness), the second leans on the tint + WINNER badge it already had. The popover's fetch is now CACHED and still lazy; that fix nearly collided with the existing `useTeamRoster` under a shared cache key — see `useTeamRosterWithMembers.ts`.) Previously 2026-09-15: (FIXED the two db tests red on main since #280 made CI run them: `appendConfirmation.db.test.ts` built its `baseResult` fixture before `earlyEight` was added to `ConfirmationResult` (#276), so the insert wrote `undefined` → DB default `false`, and the exact-duplicate guard compared `false === undefined` → not a dup → a second identical vouch row. Test file is excluded from tsconfig, which is why typecheck never flagged the missing field; production callers are all typed and pass it. Also restored the `src/components/coinflip/` entries that fell out of the TOC when #279 merged, and trimmed the sandbox line's reference to the cut supplied-face prop.)
...
- `api/hooks/useTeamRosterWithMembers.ts` - Cached roster **with member names/numbers**, behind the `TeamNameLink` popover (tap a team name anywhere → see who is on it). Lazy: `enabled` is the popover's open state, so a standings table of a dozen teams fetches nothing until one is asked about. **Distinct from `useTeamRoster` in `useTeams.ts`**, which returns membership ids only — they keep SEPARATE cache keys (`teams.rosterWithMembers` vs `teams.roster`), because two shapes under one key would let whichever query ran first decide what the other consumer got back.
