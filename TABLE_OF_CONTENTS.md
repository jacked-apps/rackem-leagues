...
> **Last Updated**: 2026-06-11 (Lineup-swap polish: the swap **request** (`LineupChangeModal`) and **approval** (`LineupChangeRequestModal`) modals now show each player's **full name + handicap** (outgoing vs incoming) so captains can see the handicap swing; added `getPlayerFullNameById` to `src/types/member.ts`; `ScoreMatch.tsx` plumbs handicaps + full names through. Branch `feat/lineup-swap-recalibration`.) Prior 2026-06-11: (Unit 5: added `src/components/scoring/LineupSwapWaitingBanner.tsx` (initiator waiting banner) + tests; fixed the Swap Player popover gate in `UnifiedScoreboard.tsx` to key on a `hasPlayerPlayed` predicate (matches the server guard) instead of derived W/L; `ScoreMatch.tsx` plumbs the predicate from `match_games`, renders the banner, and fires a resolution toast on the initiator via a `swap_position` snapshot-compare. Branch `feat/lineup-swap-recalibration`.) Prior 2026-06-11: (Unit 4: rewired `approveLineupChange` in `src/api/mutations/matchLineups.ts` to refetch → `composeMatchThresholds` → `swap_player_in_lineup` RPC → `updateMatchRunningTotals` (the missing recalibration); `requestLineupChange`/`denyLineupChange` now take `memberId` + write the `swap_requested_by_member_id` / `swap_last_resolution` audit; plumbed `memberId` through `ScoreMatch.tsx` call sites — any scorekeeper (not just captain) may request/resolve. Branch `feat/lineup-swap-recalibration`.) Prior 2026-06-11: (Unit 3: added `supabase/migrations/20260602000001_swap_player_in_lineup_rpc.sql` — atomic SECURITY DEFINER swap RPC (apply + cascade unplayed games + thresholds + audit, data-integrity guards only, no identity check per the gate-UI/relax-RLS principle); test `src/__tests__/database/swapPlayerInLineupRpc.db.test.ts`. Branch `feat/lineup-swap-recalibration`.) Prior 2026-06-11: (Unit 2: added `src/utils/match/composeMatchThresholds.ts` — system-agnostic threshold composer dispatching via `buildSystemFromPreferences`/`handicapMechanism.kind`, never `handicap_type`; deleted the `recalculateMatchThresholds` handicap-type heuristic + its call from `matchLineups.ts`; test `src/__tests__/unit/composeMatchThresholds.test.ts`. Branch `feat/lineup-swap-recalibration`.) Prior 2026-06-11: (Unit 1: added `supabase/migrations/20260602000000_add_swap_audit_columns.sql` — `swap_requested_by_member_id` + `swap_last_resolution` audit columns on `match_lineups`, plus the matching `database.types.ts` Row/Insert/Update fields. Branch `feat/lineup-swap-recalibration`.) Prior 2026-06-11: (Added lineup-swap-recalibration plan: `docs/plans/2026-06-02-001-feat-lineup-swap-recalibration-plan.md` — 5-unit plan finishing the Dec 2025 partial swap; system-agnostic threshold composer + atomic RPC + recalibration rewire + UI. Branch `feat/lineup-swap-recalibration`. Merged current main in to reconcile the stale branch.) Prior 2026-06-07: (Trigger workshop landed — second standalone module workshop in the Workshops building: `triggers` table + 4 officials, never-throw `loadTrigger`, standalone `validateTrigger`, shared `_shared/ExpressionBuilder` extracted from the allocator's FormulaBuilder, full trigger workshop UI + route. Branch `feat/trigger-room`.) Prior 2026-06-06: (Added `docs/file-split-backlog.md` — ranked refactor backlog of files worth splitting, from a codebase sweep. Branch `docs/file-split-backlog`.) Prior 2026-06-05: (Added Facebook OAuth login alongside Google in `Login.tsx`/`Register.tsx` and shipped a public Privacy Policy at `/privacy` — `about/PrivacyPolicy.tsx` — required for Facebook app review. Branch `feat/fb-login`.) Prior 2026-06-02: (Archived ~63 completed/superseded brainstorm, plan & memory-bank docs to `docs/archive/` + `memory-bank/archive/` after a code+PR triage; active/back-burner/reference docs left in place.)
