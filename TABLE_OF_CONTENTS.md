...
> **Last Updated**: 2026-09-10 (**Self-scoring Unit 0 done — and the plan is SOLELY tournaments**. The league is NOT "the race, five times": lineup sequencing, league scoring and matchups that are not predetermined are all far larger than a race and are their own effort (Ed, 2026-09-10) — no league schema belongs in these units. Unit 0's one job was to check the race primitive is not being built in a shape a league could never reuse; **it passes** — the race carries its own participants, goals and game list, points at no parent, and its confirm loop knows nothing about teams or tables. ONE carry-forward, now in Unit 9: the race room is **addressable per race with nothing assuming a single race exists**, because a league night is several at once and this room may be reused there N times or wrapped by a list. Everything else the walk turned up is parked in `2026-09-09-002-league-race-walkthrough.md` as a note to a future league brainstorm. Advance-policy default ANSWERED: **automatic**, with confirm and manual kept as the organizer's dial.) Previously 2026-09-09: (**Self-scoring for individual races — requirements DONE** (successor to the 2026-09-08 primer). The race is a STANDALONE PRIMITIVE, not a bracket feature: two participants, a **goal number per side** (equal = unhandicapped, unequal = what a future handicap feature sets), and games **APPENDED, never pre-created** — the next game is written only when every existing game is confirmed AND neither side has met its goal. That one rule makes vacate un-finish the race for free and forces break assignment to happen when the breaker is actually knowable (which winner-breaks requires anyway). **Fake-teams-of-one is DEAD**: `matches` requires `season_id` AND `season_week_id`, so a tournament match would need a fake season, week, teams and lineups. So the race gets its OWN storage and **parents point AT it** — a bracket match points at its race, a league matchup at its several, a practice race at nothing. A finished race **announces; it never acts**. **`ScoreMatch.tsx` and `match_games` are never edited** — instead a second, much smaller room reuses the confirmation layer, which was verified already fully prop-driven (`ConfirmationDialog`, `VacateModal`, `DissentFlag`, `DisputeBanner`, `pendingConfirmations.ts`) and carries zero knowledge of teams or tables. **Who-may-score is stored as a LIST** resolved at creation, so the future neutral scorekeeper is a name added, not a redesign. **Walk-up claim tickets**: QR self-add issues a server-generated non-enumerable id the browser keeps, so two accountless strangers who both scanned the poster can score each other — the identity check (member OR ticket) is written as ONE security boundary the first time, because retrofitting it means a second security pass on a live scoring path. New third setup tab **"Scoring"** (name provisional, tension recorded): break rule, race length + game type PER BRACKET SIDE, walk-up policy, and an **advance policy** dial — automatic / confirm / manual — because scoring is always the players' act and advancing is always the organizer's. Auto-advance is safe because `reopen_bracket_match` already un-advances and refuses on a played downstream match. Acceptance test = the SAME race runs a league night of N individual races. OUT of scope: notifications, venue/tables, handicap races, break-and-run, the modular scoreboard, Phase D saved setups, saving practice races.) Prior 2026-09-08 (Tournament brackets — **late entry into an unused bye**. Someone turns up after the start; if a bye hasn't been used, they take it and the bye holder plays that round after all. New migration `20260907182857_bracket_late_entry.sql` (`add_late_entry`). **Two conditions**, both required: a bye exists (settled winners round-1 match with an empty seat), AND its recipient has neither PLAYED nor STARTED their next match. The played half is enforced by **reusing `reopen_bracket_match`** — it already refuses on a completed downstream match, clears the winner and un-advances the player, so there is one definition of "un-advance" rather than two. The started half (`in_progress`) is checked here on purpose: reopen is UNDO, where correcting a mis-tap mid-game is legitimate; changing the FIELD is not. That flag is manual today and gets trustworthy once self-scoring lands, so the completed-match check stays the real backstop. **Gated on tier, not a feature key** — a QOL extra for paying at all, deliberately not priced and deliberately not tied to `real_players`; that is intent, not a gating leak. The latecomer takes the last seed (seeding is frozen at start) and the same one-name-per-tournament rule applies. A bye cell offers **Add player** instead of **Reset**, since resetting a bye would just leave one player and nobody coming. 10 new DB tests incl. "changes nothing when it refuses"; 4888 unit tests pass.) Prior 2026-09-07 (Tournament brackets — **byes are now real, visible matches** (groundwork for late entry). The generator used to ELIDE them, so a top seed simply appeared in round 2 unexplained — which outside guides name as the most common bracket mistake ("hiding byes inside a bracket image; label them clearly in the first round"). A bye is now a completed round-1 match: the lone player in one seat, an empty seat opposite, that player already recorded as the winner and advanced. **It routes NO loser** — nobody has lost, and the losers bracket is sized on that assumption (verified against the engine: 13-of-16 yields 5 real W1 matches and just 1 losers R1 match, the other three W1 losers being wired straight into losers R2 against real opponents). Only a WINNERS round-1 half-bye is materialised; a fully-empty pairing stays elided, and the losers side is untouched — putting byes there is what the guides specifically warn against. Match counts moved from `N-1` / `2N-2` to those plus one row per bye; a power-of-two field is completely unchanged. `MatchView.isBye` drives a "Bye" label instead of a bare dash. 4884 unit tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament paid foundation — **organizer search-add** (the last gap in Phase C). Find a registered player **by name or player number** and add them, instead of retyping a name the app already has. New `paid/AddRegisteredPlayer.tsx` **reuses the existing `MemberSearchCombobox`** rather than growing a second search — which needed two small generalisations to that shared component: **the filter chips are now configurable** (My Org / State / Staff are LEAGUE scopes, useless to a tournament organizer who has no org; a lone chip hides the row entirely) and it takes an **`id`** so an external `<label htmlFor>` actually associates rather than being decorative. `searchMembers`/`useMemberSearch` gained **`registeredOnly`**, which also SKIPS a full `team_players` read that ran on every keystroke to enforce a placeholder rule meaningless outside a league. New migration `20260906205201_organizer_add_registered.sql`: an RPC rather than a client insert, because the display name must be derived exactly as `join_bracket_hopper` derives it — a browser-chosen name could differ from the one that player gets by scanning the code, and with one-name-per-tournament that becomes a phantom collision nobody can explain. Also gave the two "Add" buttons distinct accessible names; identical ones are ambiguous to anyone navigating by name rather than sight. 304 bracket tests, 4879 unit tests pass.) Prior 2026-09-06 (Tournament paid foundation — **you can get back to a tournament you joined** (Ed: "I'm IN this tournament but if I don't use the QR code I can't see that I'm in it"). The Tournaments page only ever answered "what did I create?", so a player who scanned in had to keep the tab open or go find the code again — impossible once you've walked away from the wall it was taped to. It now shows two lists, **You're playing in** and **You're running**, with joined rows linking to the PLAYER's view of the tournament and badging "Waiting" when that's the standing. New migration `20260906204115_my_tournaments.sql` (`get_my_tournaments()`, returns each tournament's join token, excludes closed ones and ones you created). A walk-up still can't appear there — no account to look them up by — so their only route back remains the link their own browser remembered. 295 bracket tests pass. Branch `feat/tournament-my-tournaments`, stacked on `feat/tournament-paid-foundation` (PR #275).) Prior 2026-09-06 (Tournament PAID foundation — **the waiting-players warning now reads by weight** (Ed: big/little/big buttons in blue/black/black). New **`warning` Button variant** (uses the existing `--warning` theme tokens) for an action that is allowed but carries a consequence — distinct from `destructive`, which is for deleting something. It is an ACTION variant, so it requires an explicit `loadingText` like default/destructive; both the type union and the runtime check were updated together, since they each carried their own list. The dialog's two real choices now share a row at equal width — **Start without them** (warning) and **Add them** (primary) — with **Go back** smaller and plainer underneath, so the quiet option looks quiet. 283 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **a final confirm before the bracket is drawn and the card is charged** (Ed). Start does two irreversible things on one tap, so the last tap is now deliberate: new `paid/ConfirmStartDialog.tsx` shows the FINAL player count and the exact charge, with a button that names the amount ("Pay $2.00 and start") rather than a bare Confirm. It comes SECOND, after the waiting-list question — that answer changes the count, so confirming first would show a number that isn't true; a test asserts the confirm reads "Start with 3 players?" after 1 waiting player is swept in to 2 official. The receipt body was extracted to new `paid/ChargeReceipt.tsx`, shared by the checkout card's accordion and the confirm, so the money glanced at and the money approved are rendered by one component — two receipts computed separately is how a checkout ends up showing two different numbers. Also fixed a no-op assertion (`.toBeNull` without parentheses) that had been silently testing nothing. 282 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **the waiting-list sweep moved to where it belongs, and starting now warns** (Ed). The "add anyone still waiting" checkbox left the checkout card for the **waiting list itself** — it's a standing rule about that list, not a payment option — where it shows even before anyone has arrived and stays off by default. On Start, if people are still waiting and that choice is off, a three-way confirm asks first: **add them and start / start without them / go back**. Leaving someone out isn't recoverable — the bracket is drawn and they simply aren't in it — so it has to be a decision rather than an omission. The choice is passed into the start routine as an argument rather than read from state, since the dialog decides it at the moment of the tap and a `setState` wouldn't have landed. 278 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **the charge breakdown now reads as a receipt** (Ed: the total looked like a second dollar being charged). Items sit left with amounts in a right-hand column, then a rule and the **Total pushed over beside its own amount** — a full-width total row styled like the rows above it reads as one more line item. Tabular figures so prices line up in a column, and the total is a step larger as well as bolder — weight alone is a weak signal at this size. When the $5 cap bites, **Subtotal** and the discount are both shown so the arithmetic reconciles on the page; uncapped, a subtotal identical to the total is noise and is omitted. 273 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **the checkout card itemises what you're paying for** (Ed). A collapsed accordion reads "2 features · $2.00" and opens to the list plus what's charged at start. The **$5 cap gets its own line when it actually bites**: six features at $1 add to $6 while the button says $5, so without it the items wouldn't reconcile with the total and would read as a bug even though the cheaper number is the honest one. New pure `chargeBreakdown()` in `premiumFeatures.ts` does the math, tested to always agree with `totalPriceCents` and to skip a retired feature key rather than render a blank row. 272 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **setup-page layout corrected to Ed's shape**. The tabs belong to the CARD, not the screen: the setup card carries Info / Players at its own bottom edge, and **Start & pay is a separate card below it** — which makes starting reachable from whichever tab you're on, without spending a tab slot on what is an action rather than a destination. (I had built page-level fixed tabs with Start buried inside one.) **Tab budget, recorded because it shapes every feature from here:** mobile takes 4 tabs comfortably and 5 at most (Apple collapses past 5; Material specs 3–5; a 375px screen gives 75px per tab at five). A phase split — setup tabs vs running tabs — does NOT help, because per Ed **almost everything is pre-bracket setup**: venue, tables, notifications, handicaps and payouts are all configured before the bracket is drawn, and once it is drawn the tournament largely runs itself. So all the pressure lands on one tab set, and the answer is to COMBINE strategically as each feature arrives rather than adding a tab per feature. 262 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **organizer setup page restructured, and the tournament is now editable**. Two tabs pinned to the bottom (Info / Players), mirroring the player's view of the same tournament. Players holds the hopper card and, as a SEPARATE card below it, the Start & pay panel — starting is its own decision, not the end of a list. New `paid/BracketInfoTab.tsx` makes the name, game, format and grand-final rule editable while a tournament is in setup; they were set once on the create page and then unreachable, which is wrong for something that deliberately sits in setup all evening. Save is disabled until something differs, and a background refresh does NOT reset the draft (this page has realtime, which would otherwise delete a half-typed edit). Locked after start by the mutation's `status='setup'` WHERE guard — the match tree is generated FROM the format, so changing it later would leave the bracket describing rules it wasn't built to; hiding the form alone wouldn't be enough. Also consolidated the game list into new `src/brackets/gameTypes.ts`: it had been a constant in `DetailsStep` plus a drifting copy of the labels in `tournamentRules`, which rendered "9-ball" where the create page said "9-Ball". 261 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **player page reworked for a phone** (Ed, testing on one). The rules we already had — game, single/double elimination, the grand-final rule — were rendered in a card at the BOTTOM; they now sit second, directly under the tournament name, as one wrapping line of chips (new `paid/TournamentRulesBar.tsx`). Everything else lost its card chrome: "You're on the waiting list as Ed" was taking a third of a phone screen and pushing the player lists — the thing people keep re-checking — off the bottom. **Both tabs are now always present and pinned to the bottom of the screen** where a thumb is. The Bracket tab reads "Bracket · not ready" before there is one and explains itself when tapped, rather than materialising out of nowhere later — a tab that appears on its own teaches nobody the shape of the page, and "is the bracket up yet?" is a question people answer by looking. 252 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **fixed the guest-entry box refusing to go away** (Ed, on an iPhone). Two linked bugs: the remembered name was read from `localStorage` DURING RENDER, and a storage write tells React nothing — so the box stayed after a successful add. Worse, the "drop a stale note" cleanup ran while the list was still refetching, when the new name legitimately wasn't in it yet, so it **erased the note that had just been saved** — which is why a refresh came back to an empty box. The name is now React state (so adding replaces the box with "You're on the waiting list as Rocket" at once) and the cleanup is gated on the fetch being settled. Both pinned by tests. `AddMyNameCard` reworked to Ed's shape: **"You're not signed in"** → Sign in → a short case for registering (it takes a minute, needs almost nothing, and your name/record/fee status then follow you everywhere) → **"Or play as a guest"**. The register button deliberately carries no `?redirect`: `Register` doesn't navigate on success and its OAuth handoff returns to the app root, so a come-straight-back promise would be false — giving Register the redirect support Login already has is its own follow-up. Also made `bracket.hopper.db.test.ts` establish its own zero-roster precondition instead of assuming it — the `db` project shares one Postgres and the new roster suites made that assumption flaky. 249 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **fixed "Could not copy" on the join link**. `navigator.clipboard` only exists in a SECURE CONTEXT (HTTPS or localhost), so reaching the dev server by its LAN address — which is how you test a QR code on a phone — made every bracket copy button fail. `ShareAppCard` already had the correct implementation with an `isSecureContext` check and a `<textarea>`+`execCommand` fallback; the bracket buttons had re-derived a naive version without it. Extracted the working one to new `src/utils/clipboard.ts` (`copyText`, returns false rather than claiming success) and pointed the join-link button, the free-tier share button, `ShareAppCard` and `ShareLinkSection` at it. A failed copy now points at the QR page, which prints the link as selectable text, instead of putting an unselectable URL in a toast. 4 new tests. (`RegisterPlayerModal` and `rules/CopyLinkButton` still call the API directly — same latent bug, left alone as unrelated surfaces.) Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **a player with no account can add themselves**. New migration `20260906181438_bracket_walkup_self_add.sql`: `add_self_as_walkup(join_token, display_name)`, which **deliberately opens an anonymous write** and reverses the authenticated-only hopper posture. The reasoning: the waiting room is a PROPOSAL the organizer must admit from, so abuse costs them a tidy-up rather than corrupting a tournament — and requiring an account would lose exactly the walk-in this whole feature exists for. Guarded in the DATABASE (an input box is trivially bypassed): setup-only, `real_players` required, a 128-entry cap so nobody can bury the list overnight, a 12-character name limit, and one-name-per-tournament reported as `name_taken`. The length limit sits in the function rather than on the column, since organizer-typed names and registered nicknames share that column and aren't bound by the self-entry rule. New `paid/AddMyNameCard.tsx` gives a signed-out visitor two doors (type a name / sign in), with rejections rendered inline beside the box rather than as a vanishing toast. New `paid/walkupMemory.ts` notes the name per tournament in that browser — **a convenience, never truth**: it is re-checked against the live list every visit and dropped if the organizer removed them, and a name noted for one tournament never surfaces on another. A typed name shows no entry-fee status, since a name proves nothing about who typed it. 19 new tests; 245 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **the player gets a live tournament page**. Scanning the QR used to land on a dead-end "You're in!" that never changed again; joining is now a **toast**, and the destination is a live page showing where you stand, who's in, who's waiting, and the rules (format, **how many losses puts you out**, the game). Two tabs — Players always, **Bracket appearing the moment the organizer starts** — so a player can go back and forth. New migration `20260906172524_bracket_player_view.sql`: `get_bracket_player_view(join_token)`, a THIRD read because neither existing one fits — `get_bracket_hopper` carries the PII that distinguishes same-name players (can't reach a poster link) and `get_bracket_share` reads participants, which don't exist until Start. **Anon-callable** (a walk-up has no account); names-only projection is the boundary. **A player sees only their OWN entry fee** — this page hangs on a wall, and publishing who still owes would make it a debt board; a DB test asserts no other player's paid flag is in the payload. `useBracketRealtime` gains an opt-in hopper watch, wired into BOTH the player page and the organizer's setup screen — which had **no live updates at all**, so players scanning in didn't appear until reload. 21 new tests; 226 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **one name per tournament, first come first served** (Ed's rule). New migration `20260906171456_bracket_hopper_unique_name.sql`: a unique index on `(bracket_id, lower(btrim(display_name)))`, plus a `name_taken` outcome on `join_bracket_hopper`. This **deliberately reverses** the earlier position that walk-up de-duplication is the organizer's judgment — right for the free tier (no hopper; a human reads the bracket), wrong the moment automation identifies a player by name, since self-scoring, table alerts and handicap lookups all break identically on two Tim Ps. **Enforced in the database, not the UI**: "first" is only true if the database decides it — two simultaneous joins would each read a list without the other and both insert. A registered player who loses the race is sent to their **profile** to change their nickname and come straight back (a "Try again" link, no re-scan); we never rename anyone from here, because a nickname belongs to their account and changing it would change it on their league team too. A walk-up name collision gets the plain "already on this list — use a different name". The `already_in` no-op is answered BEFORE the name check, so a re-scan can't report the player's own row as somebody else holding their name. 9 new tests; 205 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **fixed the cold scanner being dropped at sign-in**. `JoinHopperPage` linked to a bare `/login`, so someone scanning the QR with no session signed in, landed on the dashboard, and **never joined** — the headline adoption moment silently did nothing and they'd have to find and scan the code again. It now passes `?redirect` back to the join page; `Login` already supported it and validates with `getSafeRedirectPath` (same-origin relative paths only, open-redirect guarded). First tests for that page (3), incl. one pinning the round trip. 199 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **the join QR can leave the app**. `JoinQrPoster` gains SVG / PNG download buttons (new `paid/qrExport.ts`) so an organizer building their own flyer has the code as an asset in Canva/Docs/Publisher rather than only as our printed page. SVG is offered first — vector, so it survives being blown up to poster size; PNG at 1024px covers tools that reject SVG, rasterized through an off-screen canvas with an explicit white fill so a transparent export can't become an unscannable code on a dark flyer. `svgToString` adds the `xmlns` a standalone .svg needs to open (inline SVG in HTML doesn't) and clones rather than mutating the node still on screen. Right-click was never an option — browsers only offer "copy image" on an `<img>`, and ours is inline `<svg>`. Clipboard copy deliberately skipped: `ClipboardItem` image support is patchy (Firefox), and a design tool wants a file anyway. A test asserts the ref actually reaches the rendered `<svg>`, since a forwarded ref that silently didn't land would export nothing. 7 new tests; 196 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **printable join QR sign**. New `paid/JoinQrPoster.tsx` at `/brackets/:bracketId/qr` (organizer-only): a full PAGE rather than a dialog, so one page prints as one sign to tape around the bar and the same page on a TV is already the right shape. Available from creation, since `join_token` is DB-generated on insert. The QR is forced **black-on-white** instead of theme colours — it must survive printing AND scanning across a dim room, and a themed QR in dark mode is a black square on a dark card. Uses the `qrcode.react` dep and the `print:hidden` + `window.print()` convention that were **already in the repo** (`ShareAppCard`, `LmsResultsSheet`) — no new dependency. New `paid/joinUrl.ts` centralises the join URL now that it appears in three places (copied, encoded in the QR, printed as typed-fallback text) — drift there would send hand-typers somewhere the QR doesn't. Setup page gains a "QR code" button beside "Copy join link". 5 new tests; 189 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **a waiting sign-up tournament now opens on its hopper**. The tournaments list linked EVERY bracket to the live view, so a `real_players` tournament sitting in `setup` — which is the entire point, it waits there while players scan in over an evening — opened an **empty bracket tree with no way back to the hopper**. Anyone who created one, walked away, and came back through the list was stranded. New `paid/bracketDestination.ts` holds the rule once (`setup` + `real_players` → the hopper; everything else → the bracket); `BracketsIndexPage` links through it and `BracketView` redirects a waiting sign-up tournament back rather than rendering an empty tree. `BracketSetupPage` now also refuses a tournament that never bought sign-up, so **the free tier can't reach a hopper even by typing the URL** — free is unchanged end to end: type names → review → Start → live bracket, no hopper, no waiting list. (One existing test asserted a no-premium-features tournament starting from the setup page — now an impossible state, replaced with one pinning that every bought feature is charged.) 8 new tests; 184 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **premium features now gate independently** (Ed caught this). Each feature is priced and sold on its own, but the hopper screen was showing the **entry-fee tracker** to anyone who bought **sign-up links** — "Add as paid"/"Add as unpaid", Paid/Unpaid badges, "Mark as paid", and "as unpaid" on the Start checkbox were all ungated. That gave a $1 feature away AND cluttered the menus of an organizer who collects nothing at the door. (The reverse was already fine: `payment_tracker` alone works via `EntryFeePanel` on the live view, which was correctly gated.) New `hasPremiumFeature(features, key)` in `premiumFeatures.ts` is now the single gate — it also absorbs the nullable `premium_features` so a free tournament can't slip through a call site — and `BracketSetupPage` threads `trackEntryFees` into `HopperView`/`HopperEntryMenu`/`StartTournamentPanel`. Without the tracker: admitting is one plain "Add to tournament" (the stored flag defaults to unpaid, unseen), no badges, no Mark-as-paid, and the sweep-in checkbox reads "Also add the N still waiting". `BracketView` moved onto the same helper. 9 new tests pinning BOTH postures; 177 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **the setup screen is now actually usable**. Adding `paid/AddWalkupForm.tsx`: until now nothing on the hopper screen could put a player in it — only the join link, which adds the caller once — so an organizer would land on three empty groups and never reach the 2 players Start requires. A typed name now creates a **walk-up** in the waiting room. Two bugs its tests caught: a failed add threw into an unhandled promise rejection and showed the organizer nothing (the form now swallows after the view reports it, keeping the typed name for a retry), and the form's helper text repeated the Waiting group's empty copy verbatim — the empty state is now just "Nobody waiting yet." since the always-visible form says where they come from. 167 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **past-players housekeeping**. Every name on the setup screen now behaves the same way: tap a past player for a menu (**Add to this tournament** / **Forget this player**) instead of a bare one-tap add. New migration `20260906152223_bracket_roster_forget.sql`: `forget_bracket_roster_entry(member_id, display_name)` drops ONE person from the calling organizer's remembered list — registered by id, walk-up by case-insensitive name. The list is sticky by design (an eject never removes anyone), so this is the deliberate way out; **single-target on purpose, no bulk clear**. Organizer resolved from `auth.uid()`, never passed in — the safety property while RLS is off; returns false rather than erroring when there was nothing to remove. Forgetting takes a confirming second tap whose copy states it changes the saved list and nothing about any tournament played. Also reserved an always-NULL `handicap` column on `bracket_walkup_roster` (folded into that unmerged migration): nothing on a tournament entrant produces a handicap yet — that arrives with `handicap_races`, whose own description already promises "players we already know keep theirs" — so the column is free text and unpopulated, present only so remembering one later costs no migration. 11 new tests (6 forget DB, 5 menu); 161 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **remembered walk-ups**. A walk-up (an entrant with no account) is no longer disposable: the organizer's **Past players** list now remembers their NAME, so a regular who never wants an app doesn't get re-typed every week. New migration `20260906151140_bracket_walkup_roster.sql`: `bracket_walkup_roster` + an admission trigger mirroring the registered-roster one (remembered when ADMITTED, sticky through eject, case-insensitively unique per organizer so "Slim"/"slim" are one regular), and `get_bracket_roster` rewritten to return registered members AND remembered walk-ups as ONE list — each half still excluding anyone already in this bracket's hopper, walk-ups matched by trimmed case-insensitive name. **No global member or placeholder is created**: a remembered walk-up is a private note in one organizer's list, so the walk-ups-are-tournament-scoped decision holds. A remembered **handicap** was deliberately NOT built — it belongs to the `handicap_races` premium feature, whose own description already promises "players we already know keep theirs". Tapping a remembered walk-up re-adds them by name (`addWalkupToHopper`), never as a registered player. New `paid/removalConsequence.ts` replaces the Remove confirm's single hedge with the three true answers, since an admitted walk-up's name now survives but a never-admitted one still doesn't. 11 new tests (7 walk-up DB, 3 grouping, 2 view, 4 copy); 148 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **Unit C3 complete: the setup screen is now reachable and startable**. A tournament with **"Real players & sign-up"** no longer asks the organizer to type a player list: `CreateBracketFlow` stops after Details (stepper hidden, button reads "Create & add players"), creates the tournament in `setup`, and hands off to the new `paid/BracketSetupPage` (`/brackets/:bracketId/setup`) — which hosts the hopper, copies the **join link** (`join_token`, never the view-only `share_token`), and owns Start. New migration `20260906150028_bracket_hopper_finalize.sql`: `finalize_bracket_hopper(bracket_id, include_waiting)` converts the official list into contiguously-seeded `bracket_participants` (carrying `member_id`, `display_name`, and `paid_status` → `entry_fee_paid`) and returns the count — leaving **`start_bracket` untouched**, so the paid flow starts through exactly the same code path as the free one. Setup-only, refuses fewer than 2 players. New `paid/StartTournamentPanel.tsx` adds the QOL checkbox **"Also add the N still waiting, as unpaid"** — default OFF and always named with its count, because the waiting room is usually people standing there wanting to play, but a QR on a flyer also collects the curious and a start can't be undone. Start is a strict three-step hand-off — finalize → start → **charge last** — so a failed start can never leave a charged-but-not-started tournament. A DB test caught a real bug pre-merge: `entry_fee_paid` is NOT NULL while `paid_status = 'paid'` yields NULL for an unflagged entry; the insert now COALESCEs. 13 new tests (8 finalize DB, 8 panel, 5 setup page, 2 create-flow fork); 132 bracket tests pass. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **Unit C3, the organizer's hopper screen**. `HopperView` is where an organizer decides who is playing before the bracket starts: a sticky count bar (`In N · Waiting N · Past N`) over three STACKED groups — **In the tournament** (admitted; what Start seeds) → **Waiting to be added** (players who scanned the QR, opened the link, or were added) → **Past players** (the sticky roster, one tap to add). Stacked rather than tabbed because admitting bounces between the groups and tabs would lose the organizer's place on every tap. **A player appears in exactly one group**: the first two can't overlap (a hopper row has one `status`), and the new migration `20260906141649_bracket_roster_read.sql` — `get_bracket_roster(bracket_id)`, SECURITY DEFINER, organizer derived from `created_by` — excludes anyone already in this bracket's hopper, so a past player who scans in *leaves* Past players and *appears* in Waiting. That filter also prevents a one-tap add from hitting `UNIQUE(bracket_id, member_id)` as an error. Tap any name for its menu: **Add as paid / Add as unpaid** on a candidate, flip the paid flag on an official entry, **Remove** behind a two-click confirm whose copy distinguishes a registered player (stays in Past players) from a walk-up (gone for good). Names render nickname-primary with `#number · City, ST` underneath, and a shared name with nothing to disambiguate it is called out explicitly. New: `paid/HopperView.tsx`, `paid/hopperGroups.ts` (pure grouping + shared-name flagging), `paid/HopperEntryMenu.tsx`, `paid/HopperIdentityLine.tsx`, `paid/HopperGroup.tsx`; `getBracketRoster` query + `useBracketHopper`/`useBracketRoster` reads and admit/set-paid/eject/add-registered mutation hooks (every write invalidates BOTH reads, since a player moving groups changes both). 7 grouping tests + 8 component tests + 3 new DB tests. Branch `feat/tournament-paid-foundation`.) Prior 2026-09-06 (Tournament PAID foundation — **Unit C2, self-add join**. Players add THEMSELVES to a paid tournament's hopper by scanning a QR / opening a link. New migration `20260905225243_bracket_join.sql`: `brackets.join_token` (DB-generated uuid, backfilled NOT NULL UNIQUE — DISTINCT from the anon view-only `share_token` so a spectator can't join) + `join_bracket_hopper(join_token, via)` SECURITY DEFINER RPC that resolves the joiner from `auth.uid()` (NEVER the token — PF9/PF24), inserts them while `status='setup'` (repeat = no-op), returns `{ok, reason?, bracket_id?, bracket_name?}`. New public route `/brackets/join/:joinToken` → `src/brackets/paid/JoinHopperPage.tsx` (auth handled inside: signed-in → auto-join once; cold scanner → asked to sign in first; renders joining/joined/closed/invalid). New `joinHopper` mutation + `useJoinHopper` hook. 3 DB tests (`bracket.join.db.test.ts`: not_found / not_accepting / not_signed_in). Branch `feat/tournament-paid-foundation`.) Prior 2026-09-05 (Free Tier Tournaments **un-gated for production** — removed the `BRACKETS_ENABLED` launch gate entirely (the finished free tier ships live in every environment; a flag that must be ON everywhere only risks losing the feature to a forgotten env var) and merged latest `main` into the branch. Free-tier recap: single/double-elim bracket any logged-in user runs (names → tap winners), byes, public names-only share link, reset/undo a match, close + 7-day inactivity sweep, 4-state colorblind-safe match styling, manual Playing⇄On-deck sequencing. Branch `feat/tournament-bracket-free-tier`.) Prior 2026-09-04 (Notification controls Unit 3 — the per-chat control. A "Notifications" item in the conversation menu opens `ChatNotificationDialog` (on/off, plus a timing choice for group chats only); the header shows a Muted badge (icon + the word, never colour alone) so the state reads without opening the menu. New pure `resolveChatOverride.ts` decides whether a level ABOVE the chat already blocks it and names the OUTERMOST cause — telling someone their team-chat default is off while their master switch is also off would send them to fix the wrong thing. When a member switches a chat on while something above overrules it, a toast says where to change it rather than the switch implying success. The interval Select only offers values ABOVE the kind default, since anything at or below resolves back via MAX and would visibly do nothing. Quiet hours are deliberately NOT an override — temporary and expected. New `useConversationNotificationPref` (optimistic). 12 more tests. Prior 2026-09-04 (Notification controls Unit 2 — the global settings UI. Settings → Notifications now carries quiet hours (one window, no per-chat escape; offers 22:00–07:00 on first enable, labels an overnight window, and captures the IANA zone from the browser since "22:00" means nothing without knowing whose clock) and a per-conversation-kind list (on/off, plus an interval preset for GROUP kinds only — direct messages get no interval control at all rather than a permanently disabled one). Only kinds live in `push_type_policy` are listed, so there is never a control for a channel that cannot send. New `src/api/queries/notificationPrefs.ts`, `src/api/mutations/notificationPrefs.ts`, `src/api/hooks/useNotificationPrefs.ts` (optimistic), and `src/components/messages/notifications/` (`NotificationPrefsSection` container + prop-driven `QuietHoursSetting`/`KindPrefRow` + shared `notificationKinds.ts`). 12 component tests. Prior 2026-09-04 (Notification controls Unit 1 (schema + resolver) VERIFIED against local Postgres: migration applies cleanly and all 15 DB tests pass. New migration `20260904212604_notification_controls.sql`: `member_notification_prefs` (per-member, per-conversation-kind on/off + interval), `conversation_participants.notification_interval_minutes` + `last_notified_at`, quiet-hours/timezone columns on `members`, `is_in_quiet_hours()` (STABLE not IMMUTABLE — it reads now()), a rewritten `get_push_recipients` implementing the VETO chain (system kind policy AND master switch AND not-quiet-hours AND per-kind default AND per-chat setting AND rate-limit window), and `mark_push_notified()` which the dispatcher calls only after a SUCCESSFUL send. Interval resolves as MAX(kind, chat) so a chat can be quieter but never louder; NULL interval for `direct` means DMs always buzz. Dropped the legacy `is_muted`/`notifications_enabled` columns (verified no app code reads them). `notification_mode` deliberately unchanged — under a veto model 'all' and unset are equivalent, so the nullable migration an earlier plan revision called for was unnecessary. Tests in `src/__tests__/database/notificationControls.db.test.ts` (16, one per veto level plus MAX/DM/stamping) not yet run. Prior 2026-09-04 (Planned the notification-controls work — `docs/plans/2026-09-04-001-feat-notification-controls-plan.md`. Two units gate turning group chats on: per-chat mute (UI-only — `conversation_participants.notification_mode` already exists AND `get_push_recipients` already filters on it, so nothing can currently SET it) and server-side rate limiting (the default-on protection; mute is opt-out and only helps whoever goes looking). Records the verified current state, since the origin brainstorm predates the implementation: only `direct` pushes today — `team_chat`/`captains_chat`/`announcements`/`match_chat` are all false in `push_type_policy`. Prior 2026-09-04 (Message view polish — (1) the conversation header no longer scrolls away: `Messages.tsx` used `h-screen` (100vh), which on mobile Chrome is TALLER than the visible viewport while the address bar shows, so the page itself scrolled and took the header with it; now `h-dvh`, with `flex-shrink-0` on `ConversationHeader` and `min-h-0` on the scroll container so only the message list scrolls. (2) 1:1 DMs no longer repeat the sender name over every incoming bubble — the header already names them, and repeating it read as though someone else might be in the thread; group chats keep it via a new `isDM` prop on `MessageList` that defaults to group behaviour. Prior 2026-09-04 (Push no longer chimes for the conversation you are already reading — `Messages.tsx` now keeps the URL in step with the open thread via a single `selectConversation` helper, because `sw.ts` detects "already viewing" by matching the window path against `/messages/:id` and in-app selection previously left the address at `/messages`. New `src/pages/__tests__/Messages.urlSync.test.tsx` pins the contract, asserting through `isViewingConversation()` itself rather than the path string. Prior 2026-09-04 (Added the repo's first pre-merge CI workflow, `.github/workflows/checks.yml` — until now the only automation was the two deploy workflows, so mistakes could only be caught by breaking a deployed environment. First check: `scripts/check-migration-versions.sh`, which fails on duplicate migration versions; also added as a pre-flight step in both deploy workflows so a bad merge fails with a message naming the offending files instead of an opaque Postgres duplicate-key error mid-deploy. CLAUDE.md gains a "Migration Filenames" section requiring real UTC timestamps. Also: Unblocked the staging deploy — `20260818000000_push_subscriptions.sql` was renamed to `20260817000000_push_subscriptions.sql` because it shared version `20260818000000` with `20260818000000_onboarding_full_captain_name.sql`. `supabase db push` keys `supabase_migrations.schema_migrations` on version, so applying the second one failed with a duplicate-key error, aborting every staging deploy since 2026-08-26 BEFORE the S3 upload — staging had been serving the pre-push bundle for 8 days. Renumbered DOWN rather than up so the Units 7–8 migrations on PR #255 (`20260818000001`, `20260818000002`) still sort after it and need no change; the workflow pushes with `--include-all`, which applies an out-of-order pending migration.) Prior 2026-09-04 (Fixed the "Update Available" prompt — pressing Update Now gave no feedback (the onClick discarded the promise, so the Button never got `isLoading`) and sometimes never updated (the reload was left to a `controllerchange` event that does not fire when no worker is in `waiting`). Now shows an in-flight state and guarantees the reload itself via a 3s fallback; registration errors go to `logger` instead of `console.log`. Gave the component its first tests, which required aliasing `virtual:pwa-register/react` to a new inert stub `src/test/stubs/pwaRegister.ts` in `vitest.config.ts` — that virtual module is generated by vite-plugin-pwa and was unresolvable under vitest, so anything importing it could not be tested at all. Prior 2026-09-04 (Push no longer chimes for the conversation you are already reading — `Messages.tsx` now keeps the URL in step with the open thread via a single `selectConversation` helper, because `sw.ts` detects "already viewing" by matching the window path against `/messages/:id` and in-app selection previously left the address at `/messages`. New `src/pages/__tests__/Messages.urlSync.test.tsx` pins the contract, asserting through `isViewingConversation()` itself rather than the path string. Prior 2026-09-04 (Added the repo's first pre-merge CI workflow, `.github/workflows/checks.yml` — until now the only automation was the two deploy workflows, so mistakes could only be caught by breaking a deployed environment. First check: `scripts/check-migration-versions.sh`, which fails on duplicate migration versions; also added as a pre-flight step in both deploy workflows so a bad merge fails with a message naming the offending files instead of an opaque Postgres duplicate-key error mid-deploy. CLAUDE.md gains a "Migration Filenames" section requiring real UTC timestamps. Also: Unblocked the staging deploy — `20260818000000_push_subscriptions.sql` was renamed to `20260817000000_push_subscriptions.sql` because it shared version `20260818000000` with `20260818000000_onboarding_full_captain_name.sql`. `supabase db push` keys `supabase_migrations.schema_migrations` on version, so applying the second one failed with a duplicate-key error, aborting every staging deploy since 2026-08-26 BEFORE the S3 upload — staging had been serving the pre-push bundle for 8 days. Renumbered DOWN rather than up so the Units 7–8 migrations on PR #255 (`20260818000001`, `20260818000002`) still sort after it and need no change; the workflow pushes with `--include-all`, which applies an out-of-order pending migration.) Prior 2026-08-25 — Units 7–8, dispatcher + trigger (Message push notifications **Units 7–8 — dispatcher + trigger** (feature complete), on stacked branch `feat/message-push-dispatch` off the foundation. **Unit 8:** migration `20260818000002_message_push_dispatch_trigger.sql` — `CREATE EXTENSION pg_net`, single-row `push_dispatch_config` (function_url + shared_secret[seeded empty, set per-env] + enabled), and an exception-wrapped `AFTER INSERT` trigger `on_message_dispatch_push` on `messages` that fire-and-forgets `net.http_post` to the dispatcher for real user messages (skips system messages; never aborts the insert). 4-case db test `messagePushTrigger.db.test.ts` (race-proof via queue+response sum-delta; body is bytea → `convert_from`). **Staging wiring:** `deploy-staging.yml` now un-gates push on staging (`VITE_PUSH_NOTIFICATIONS=true` + `VITE_VAPID_PUBLIC_KEY` build var) and **deploys edge functions** (was missing) so a merge to `main` reaches a working staging after a one-time secret setup (see `docs/ops/push-notifications-secrets.md` "Staging setup"). **Unit 7:** dispatcher. New migration `20260818000001_push_dispatch_recipients.sql` — `get_push_recipients()` SQL function encapsulating the v1 recipient policy (direct-kind-only via `push_type_policy`; non-sender active participants with `notification_mode='all'` + `push_enabled` not-false + a device; block-aware) + 8-case db test `pushDispatchRecipients.db.test.ts`. New edge function `supabase/functions/dispatch-push-notifications/index.ts` — shared-secret (`X-Dispatch-Secret`) auth, rpc recipients, declarative `web_push:8030` payload with capped + profanity-safe preview, `npm:web-push` send, 404/410 pruning, structured log; `config.toml` `verify_jwt=false`. Local spike verified: boots in the Deno edge runtime, `npm:web-push` imports, 401 without secret, clean run with it. (Chose `npm:web-push` over `jsr:@negrel/webpush` — matches our base64url VAPID keys; `VAPID_PUBLIC_KEY` added to the function env since web-push needs both keys.) Prior 2026-08-25 (Message push notifications — Units 1–2 built on branch `feat/message-push-notifications`. Unit 1: schema migration `20260817000000_push_subscriptions.sql` (push_subscriptions table, members.push_enabled, push_type_policy seeded direct-only) + db test. Unit 2: VAPID keypair + DISPATCH_SHARED_SECRET generated into gitignored local env, `.env.example` gains VITE_VAPID_PUBLIC_KEY, ops doc `docs/ops/push-notifications-secrets.md`. Unit 3: deep-linkable conversation route `messages/:conversationId` (`NavRoutes.tsx` + `Messages.tsx` reads the param and opens that thread on cold load; unknown/forbidden id falls back to the list) via new pure helper `src/utils/messages/resolveDeepLinkTarget.ts` + test. Unit 4: custom service worker — flipped `vite-plugin-pwa` `generateSW`→`injectManifest`; new `src/sw.ts` (precache + Supabase NetworkFirst ported 1:1 + `push`/`notificationclick` handlers with suppress-if-viewing + per-conversation tag), pure helper `src/utils/push/notificationPayload.ts` + test, `tsconfig.worker.json` (WebWorker lib) referenced from `tsconfig.json` with `src/sw.ts` excluded from the app tsconfig, added workbox runtime deps (precaching/routing/strategies/expiration/cacheable-response @7.4.0). Build verified: injectManifest emits `dist/sw.js` with 162 precache entries. Unit 5: client subscribe/permission logic — `src/utils/push/pushCapability.ts` (supported/denied/needs-ios-install/unsupported resolver + VAPID base64url↔bytes conversion), `src/utils/push/browserPush.ts` (thin mockable serviceWorker/pushManager/Notification wrappers), `src/api/mutations/pushSubscriptions.ts` (upsert/delete row + set members.push_enabled), `src/api/hooks/usePushSubscription.ts` (subscribe/unsubscribe/heal-on-mount, exported from hooks index) + 12 tests; `VITE_VAPID_PUBLIC_KEY` added to `src/vite-env.d.ts`. Unit 6 (settings toggle): new `src/components/messages/PushNotificationSetting.tsx` (prop-driven per-capability control: on/off Switch, iOS add-to-home-screen nudge, denied/unsupported hints) + test, wired into `MessageSettingsModal.tsx`'s Notifications accordion (replaces the "coming soon" placeholder) via `usePushSubscription` + `useCurrentMember`; `push_enabled` added to the `Member` type. Onboarding: new `src/components/messages/PushOnboardingPrompt.tsx` — a SEPARATE three-way first-run prompt (Turn on = subscribe / No thanks = push_enabled false / Not now = leave NULL and re-ask next Messages visit), shown from `Messages.tsx` after the profanity modal while `push_enabled IS NULL` and capability is supported; persists + invalidates the member cache on an explicit answer. + test. **Gated for the foundation PR:** new `PUSH_NOTIFICATIONS_ENABLED` flag in `src/config/featureFlags.ts` (on in dev, off in prod unless `VITE_PUSH_NOTIFICATIONS=true`) wraps BOTH entry points — the onboarding prompt (`Messages.tsx`) and the Settings toggle (`MessageSettingsModal.tsx`, falls back to "Coming soon") — so users can't subscribe to a channel that can't send until the dispatcher+trigger (Units 7–8) ship; noted in `LIST_FOR_ED.md`. Also regenerated `database.types.ts` with the upgraded Supabase CLI 2.115 to end the phantom-diff drift.) Prior 2026-08-18 (Message push notifications brainstorm — captured `docs/brainstorms/2026-08-18-message-push-notifications-requirements.md`: a scoped, standalone extraction of the 2026-04-21 messaging overhaul's Phase 2. Decision: ship Web Push **loud first** (chime on all chats, honoring only existing mute + a global on/off) to a single friendly test league to surface real pain, then build the per-chat/frequency **preferences panel** and Android **reply-from-notification** as v2. v1 = subscription/permission flow, custom service-worker push handler, server-side dispatch on message insert, universal tap-to-open, and explicit iOS "add-to-home-screen" handling. Greenfield pipeline; the `notification_mode` tri-state column already exists to back v2. No code yet.) Prior 2026-07-12 (League Dues Roster — operators can now see who in a league has/hasn't paid annual dues at a glance instead of clicking players one at a time. New page `src/operator/LeagueDuesPage.tsx` (`/league/:leagueId/dues`): current-season roster + paid/unpaid summary + All/Unpaid filter; rows are `PlayerNameLink`s so dues can be marked paid inline. Backed by `src/api/queries/duesRoster.ts` (league-scoped) + `src/components/operator/DuesRosterList.tsx` (icon+text status, colorblind/dark-mode safe) + a new calendar-year `getDuesYearStatus` in `membershipUtils.ts`. The league page's single "Open Finances" button became a two-button `FinancesCard` (Dues + Payout Calculator), mirroring the Scoring card. `RecordDuesModal` now also invalidates `['leagueDuesRoster']` so the list refreshes after marking paid. Branch `feat/league-dues-roster`.) Prior 2026-06-15 (Bye-not-a-real-match #1+#2 implemented — (#1) new migration `20260615000000_bye_matches_skip_table_assignment.sql` CREATE OR REPLACEs `assign_tables_for_week()` to skip `status='bye'` matches (byes are real-UUID rows now, so the old NULL-only filter handed them a real table); DB test `byeMatchTableAssignment.db.test.ts`. (#2) lineup page shows `ByeWeekNotice.tsx` instead of the builder when the match is a bye — both fixes key on the new shared `src/utils/match/isByeMatch.ts` predicate (also adopted by `manualScoringEligibility.hasTwoRealTeams`); `getMatchWithLeagueSettings` now selects team `status`. (#3) the "My Match" nav surfaces (bottom tab / sidebar / drawer) no longer surface a bye matchup — `classifyTier` in `useMyMatchSurfaces.ts` returns null for an `isByeMatch` row, so a today/past-due bye can't appear as an actionable nav destination. (#4) standings no longer show a bye as "Unknown Team" — `fetchSeasonStandings` (`src/api/queries/standings.ts`) now skips any team id missing from the active-only name map, so bye/withdrawn/forfeited teams get no standings row (their matches still count toward the active opponent); also protects playoff seeding, which reads the same function. Branch `feat/bye-not-a-real-match`.) Prior 2026-06-14 (Bye-not-a-real-match brainstorm — captured requirements at `docs/brainstorms/2026-06-14-bye-not-a-real-match-requirements.md` for the two remaining bye gaps: a bye matchup must not consume a table number, and the bye-side team must not be able to enter the lineup page — both routed through the existing `hasTwoRealTeams()` "real match" definition. Also verified the auto-forfeit `pg_cron` sweep is firing daily in production (3 successful 06:00 UTC runs). Branch `feat/bye-not-a-real-match`.) Prior 2026-06-30: (Schedule ⇄ Matchup decoupling, Phases A+B — fixes the duplicate "Week 14" bug by deriving "Week N" from each week's date POSITION instead of storing it. A: new `deriveWeekLabels` helper (`scheduleDisplayUtils.ts`) + `useWeekLabelsForSeasons` hook; every display surface + the operator edit-lock (`isPlayWeekLocked`, golden-tested in `scheduleManagerLock.test.ts`) read derived labels. B: migration `20260621000000` moves blackout labels to `notes` + collapses `season_end_break`→`blackout`; writers + reflow/toggle/season-length aligned. Phase C (drop `week_name`) HELD for a follow-up PR after staging verification — see `LIST_FOR_ED.md`. Branch `feat/schedule-matchup-decoupling`.) Prior 2026-06-13 (Change Season Length recovered — re-applied the stranded season-length feature (PR #211 merged into a side branch, never reached main) onto current main: operators can lengthen/shorten a season before the week-5 lock — `seasonLengthEdit.ts` (bounds/lock), `applySeasonLengthChange.ts` (append-weeks+generate / guarded-tail-delete), `ChangeSeasonLengthDialog.tsx`, + a `teams.schedule_position` column. Migration renumbered to `20260611000002` to clear a collision with the auto-forfeit-sweep migration. Branch `feat/season-length-change-v2`.) Prior 2026-06-13: (Drop team to BYE — ported the operator "drop a team" feature onto current main after its original PRs (#215 stack) merged into side branches instead of main: added `convertTeamToBye()` (pre-season slot→bye repurpose) to `teams.ts`, replaced the placeholder `handleDeleteTeam` in `team-management/useTeamActions.ts` with the full delete/pre-season-drop/mid-season-drop flow, brought in the `20260611000001_drop_team_mid_season_rpc.sql` migration + two DB tests. Branch `feat/drop-team-to-bye-v2`.) Prior 2026-06-13: (Playoff review page — rewrote `PlayoffSetup.tsx` (`/playoffs`) from a configure-first page into a place-only, review-first finals surface: three states (no-playoffs / set-up / locked), editable place-rule, auto-fill-in at season end honoring the configured pairing, safe reset. Core fixes in `playoffGenerator.ts`: `generatePlayoffBracket` now honors the configured matchup style (`realPairsForStyle`), new `arePlayoffMatchupsPopulated` (trustworthy non-null-team signal) + `resetPlayoffMatchups` (null-not-delete); `PlayoffsCard` label aligned to that signal. Deferred: true server-side auto-populate. Branch `feat/playoff-review-page`.) Prior 2026-06-11: (Auto-forfeit sweep — the league's first scheduled job. Added `supabase/migrations/20260611000000_auto_forfeit_sweep.sql`: `pg_cron` + `sweep_auto_forfeits()` (daily, set-based, all leagues) that forfeits past-due unplayed matches where exactly one team is captainless — the foundation that auto-resolves BYE weeks AND dropped-team weeks. Forfeit scoring deferred. Test `src/__tests__/database/autoForfeitSweep.db.test.ts`. Branch `feat/auto-forfeit-sweep`.) Prior 2026-06-07: (Trigger workshop landed — second standalone module workshop in the Workshops building: `triggers` table + 4 officials, never-throw `loadTrigger`, standalone `validateTrigger`, shared `_shared/ExpressionBuilder` extracted from the allocator's FormulaBuilder, full trigger workshop UI + route. Merged current main in to reconcile the stacked branch. Branch `feat/trigger-room`.) Prior 2026-06-06: (Added `docs/file-split-backlog.md` — ranked refactor backlog of files worth splitting, from a codebase sweep. Branch `docs/file-split-backlog`.) Prior 2026-06-05: (Added Facebook OAuth login alongside Google in `Login.tsx`/`Register.tsx` and shipped a public Privacy Policy at `/privacy` — `about/PrivacyPolicy.tsx` — required for Facebook app review. Branch `feat/fb-login`.) Prior 2026-06-02: (Archived ~63 completed/superseded brainstorm, plan & memory-bank docs to `docs/archive/` + `memory-bank/archive/` after a code+PR triage; active/back-burner/reference docs left in place.) Prior 2026-09-05 (What's New readiness pass — (1) the 9 entries were sitting in a `1.0.0` block DATED TODAY while the app runs 1.8.0, so the page announced a version older than the one you're on; they describe work that is in staging and NOT yet in production, so they now live in the `unreleased` block and render as "In progress / Coming soon" until `scripts/stamp-whats-new.mjs` names a real version at release. With nothing shipped, `latestReleasedVersion()` is null and the nav "New" marker correctly stays off. (2) `__APP_VERSION__` was defined in `vite.config.ts` but NOT `vitest.config.ts`, so rendering `WhatsNewPage` in any test died with "__APP_VERSION__ is not defined" — which is why the page had no render test at all; vitest now mirrors the same `define`. (3) New `src/whatsNew/__tests__/whatsNewAfterUpdate.test.tsx` covers the RECEIVING half of the "Show me what's new" flow (`useShowWhatsNewAfterUpdate` navigates, clears its flag so a later reload doesn't drag you back, uses `replace` so Back works, survives a private window) plus the page render — the two halves are separated by a full page reload, so a mismatched storage key would silently take nobody to the notes and nothing would ever error. 14 tests across the surface. Also: test flakes fixed in `ChangeSeasonLengthDialog.test.tsx` and `ProfanityOnboardingModal.test.tsx` — `await user.click()` awaits the click event, not the handler's promise chain, so assertions on what happens AFTER an awaited call (`onApplied`, `onResolved`) were racing the microtask queue; they now use `waitFor`. Prior 2026-09-05 (BUILT the What's New surface — `/whats-new` (public) shows the current release in full with earlier ones beneath as one-liners linking to `/whats-new/:version`; one component serves both routes. New `src/whatsNew/` (`releases.ts` content file carrying the writing rules in its header, pure `releaseSelectors.ts`, `useWhatsNewSeen.ts`), a per-USER `members.last_seen_whats_new` column (reading on a laptop clears the marker on a phone), and a nav entry on BOTH drawer and sidebar whose marker is the WORD "New" rather than a coloured dot. `__APP_VERSION__` injected from package.json via vite `define` and shown on the page — "am I on the new build?" was unanswerable without DevTools and cost two misdiagnoses on 2026-09-05. Enforcement: `scripts/check-whats-new.mjs` as a 4th CI job (entry OR `[no-changelog]`), and `scripts/stamp-whats-new.mjs` to stamp a release. 13 selector tests; both scripts verified in every direction including that a missing input exits 2 rather than silently passing. Prior 2026-09-05 (FIXED: the app could never self-update. `src/sw.ts` had no SKIP_WAITING message listener — vite-plugin-pwa injects one under `generateSW` but NOT under `injectManifest`, which we switched to in push Unit 4 so we could write our own push handlers. So "Update Now" posted a message nobody listened for, the new worker sat in `waiting` forever, `controllerchange` never fired, and no client could take an update on any device. PR #263 had added a spinner and a 3s forced reload, but that reload landed on the SAME bundle because the new worker never activated — symptom treated, cause missed. Adds the listener plus `clients.claim()` on activate so the reload is served by the new worker, and a source-level guard test (`src/__tests__/unit/serviceWorkerUpdatePath.test.ts`) because every test, typecheck and build passed for the entire time it was broken. Prior 2026-09-04 (Notification controls Unit 3 — the per-chat control. A "Notifications" item in the conversation menu opens `ChatNotificationDialog` (on/off, plus a timing choice for group chats only); the header shows a Muted badge (icon + the word, never colour alone) so the state reads without opening the menu. New pure `resolveChatOverride.ts` decides whether a level ABOVE the chat already blocks it and names the OUTERMOST cause — telling someone their team-chat default is off while their master switch is also off would send them to fix the wrong thing. When a member switches a chat on while something above overrules it, a toast says where to change it rather than the switch implying success. The interval Select only offers values ABOVE the kind default, since anything at or below resolves back via MAX and would visibly do nothing. Quiet hours are deliberately NOT an override — temporary and expected. New `useConversationNotificationPref` (optimistic). 12 more tests. Prior 2026-09-04 (Notification controls Unit 2 — the global settings UI. Settings → Notifications now carries quiet hours (one window, no per-chat escape; offers 22:00–07:00 on first enable, labels an overnight window, and captures the IANA zone from the browser since "22:00" means nothing without knowing whose clock) and a per-conversation-kind list (on/off, plus an interval preset for GROUP kinds only — direct messages get no interval control at all rather than a permanently disabled one). Only kinds live in `push_type_policy` are listed, so there is never a control for a channel that cannot send. New `src/api/queries/notificationPrefs.ts`, `src/api/mutations/notificationPrefs.ts`, `src/api/hooks/useNotificationPrefs.ts` (optimistic), and `src/components/messages/notifications/` (`NotificationPrefsSection` container + prop-driven `QuietHoursSetting`/`KindPrefRow` + shared `notificationKinds.ts`). 12 component tests. Prior 2026-09-04 (Notification controls Unit 1 (schema + resolver) VERIFIED against local Postgres: migration applies cleanly and all 15 DB tests pass. New migration `20260904212604_notification_controls.sql`: `member_notification_prefs` (per-member, per-conversation-kind on/off + interval), `conversation_participants.notification_interval_minutes` + `last_notified_at`, quiet-hours/timezone columns on `members`, `is_in_quiet_hours()` (STABLE not IMMUTABLE — it reads now()), a rewritten `get_push_recipients` implementing the VETO chain (system kind policy AND master switch AND not-quiet-hours AND per-kind default AND per-chat setting AND rate-limit window), and `mark_push_notified()` which the dispatcher calls only after a SUCCESSFUL send. Interval resolves as MAX(kind, chat) so a chat can be quieter but never louder; NULL interval for `direct` means DMs always buzz. Dropped the legacy `is_muted`/`notifications_enabled` columns (verified no app code reads them). `notification_mode` deliberately unchanged — under a veto model 'all' and unset are equivalent, so the nullable migration an earlier plan revision called for was unnecessary. Tests in `src/__tests__/database/notificationControls.db.test.ts` (16, one per veto level plus MAX/DM/stamping) not yet run. Prior 2026-09-04 (Planned the notification-controls work — `docs/plans/2026-09-04-001-feat-notification-controls-plan.md`. Two units gate turning group chats on: per-chat mute (UI-only — `conversation_participants.notification_mode` already exists AND `get_push_recipients` already filters on it, so nothing can currently SET it) and server-side rate limiting (the default-on protection; mute is opt-out and only helps whoever goes looking). Records the verified current state, since the origin brainstorm predates the implementation: only `direct` pushes today — `team_chat`/`captains_chat`/`announcements`/`match_chat` are all false in `push_type_policy`. Prior 2026-09-04 (Message view polish — (1) the conversation header no longer scrolls away: `Messages.tsx` used `h-screen` (100vh), which on mobile Chrome is TALLER than the visible viewport while the address bar shows, so the page itself scrolled and took the header with it; now `h-dvh`, with `flex-shrink-0` on `ConversationHeader` and `min-h-0` on the scroll container so only the message list scrolls. (2) 1:1 DMs no longer repeat the sender name over every incoming bubble — the header already names them, and repeating it read as though someone else might be in the thread; group chats keep it via a new `isDM` prop on `MessageList` that defaults to group behaviour. Prior 2026-09-04 (Push no longer chimes for the conversation you are already reading — `Messages.tsx` now keeps the URL in step with the open thread via a single `selectConversation` helper, because `sw.ts` detects "already viewing" by matching the window path against `/messages/:id` and in-app selection previously left the address at `/messages`. New `src/pages/__tests__/Messages.urlSync.test.tsx` pins the contract, asserting through `isViewingConversation()` itself rather than the path string. Prior 2026-09-04 (Added the repo's first pre-merge CI workflow, `.github/workflows/checks.yml` — until now the only automation was the two deploy workflows, so mistakes could only be caught by breaking a deployed environment. First check: `scripts/check-migration-versions.sh`, which fails on duplicate migration versions; also added as a pre-flight step in both deploy workflows so a bad merge fails with a message naming the offending files instead of an opaque Postgres duplicate-key error mid-deploy. CLAUDE.md gains a "Migration Filenames" section requiring real UTC timestamps. Also: Unblocked the staging deploy — `20260818000000_push_subscriptions.sql` was renamed to `20260817000000_push_subscriptions.sql` because it shared version `20260818000000` with `20260818000000_onboarding_full_captain_name.sql`. `supabase db push` keys `supabase_migrations.schema_migrations` on version, so applying the second one failed with a duplicate-key error, aborting every staging deploy since 2026-08-26 BEFORE the S3 upload — staging had been serving the pre-push bundle for 8 days. Renumbered DOWN rather than up so the Units 7–8 migrations on PR #255 (`20260818000001`, `20260818000002`) still sort after it and need no change; the workflow pushes with `--include-all`, which applies an out-of-order pending migration.) Prior 2026-09-05 (Free Tier Tournaments **un-gated for production** — removed the `BRACKETS_ENABLED` launch gate entirely (the finished free tier ships live in every environment; a flag that must be ON everywhere only risks losing the feature to a forgotten env var) and merged latest `main` into the branch. Free-tier recap: single/double-elim bracket any logged-in user runs (names → tap winners), byes, public names-only share link, reset/undo a match, close + 7-day inactivity sweep, 4-state colorblind-safe match styling, manual Playing⇄On-deck sequencing. Branch `feat/tournament-bracket-free-tier`.) Prior 2026-09-04 (Fixed the "Update Available" prompt — pressing Update Now gave no feedback (the onClick discarded the promise, so the Button never got `isLoading`) and sometimes never updated (the reload was left to a `controllerchange` event that does not fire when no worker is in `waiting`). Now shows an in-flight state and guarantees the reload itself via a 3s fallback; registration errors go to `logger` instead of `console.log`. Gave the component its first tests, which required aliasing `virtual:pwa-register/react` to a new inert stub `src/test/stubs/pwaRegister.ts` in `vitest.config.ts` — that virtual module is generated by vite-plugin-pwa and was unresolvable under vitest, so anything importing it could not be tested at all. Prior 2026-09-04 (Push no longer chimes for the conversation you are already reading — `Messages.tsx` now keeps the URL in step with the open thread via a single `selectConversation` helper, because `sw.ts` detects "already viewing" by matching the window path against `/messages/:id` and in-app selection previously left the address at `/messages`. New `src/pages/__tests__/Messages.urlSync.test.tsx` pins the contract, asserting through `isViewingConversation()` itself rather than the path string. Prior 2026-09-04 (Added the repo's first pre-merge CI workflow, `.github/workflows/checks.yml` — until now the only automation was the two deploy workflows, so mistakes could only be caught by breaking a deployed environment. First check: `scripts/check-migration-versions.sh`, which fails on duplicate migration versions; also added as a pre-flight step in both deploy workflows so a bad merge fails with a message naming the offending files instead of an opaque Postgres duplicate-key error mid-deploy. CLAUDE.md gains a "Migration Filenames" section requiring real UTC timestamps. Also: Unblocked the staging deploy — `20260818000000_push_subscriptions.sql` was renamed to `20260817000000_push_subscriptions.sql` because it shared version `20260818000000` with `20260818000000_onboarding_full_captain_name.sql`. `supabase db push` keys `supabase_migrations.schema_migrations` on version, so applying the second one failed with a duplicate-key error, aborting every staging deploy since 2026-08-26 BEFORE the S3 upload — staging had been serving the pre-push bundle for 8 days. Renumbered DOWN rather than up so the Units 7–8 migrations on PR #255 (`20260818000001`, `20260818000002`) still sort after it and need no change; the workflow pushes with `--include-all`, which applies an out-of-order pending migration.) Prior 2026-08-25 — Units 7–8, dispatcher + trigger (Message push notifications **Units 7–8 — dispatcher + trigger** (feature complete), on stacked branch `feat/message-push-dispatch` off the foundation. **Unit 8:** migration `20260818000002_message_push_dispatch_trigger.sql` — `CREATE EXTENSION pg_net`, single-row `push_dispatch_config` (function_url + shared_secret[seeded empty, set per-env] + enabled), and an exception-wrapped `AFTER INSERT` trigger `on_message_dispatch_push` on `messages` that fire-and-forgets `net.http_post` to the dispatcher for real user messages (skips system messages; never aborts the insert). 4-case db test `messagePushTrigger.db.test.ts` (race-proof via queue+response sum-delta; body is bytea → `convert_from`). **Staging wiring:** `deploy-staging.yml` now un-gates push on staging (`VITE_PUSH_NOTIFICATIONS=true` + `VITE_VAPID_PUBLIC_KEY` build var) and **deploys edge functions** (was missing) so a merge to `main` reaches a working staging after a one-time secret setup (see `docs/ops/push-notifications-secrets.md` "Staging setup"). **Unit 7:** dispatcher. New migration `20260818000001_push_dispatch_recipients.sql` — `get_push_recipients()` SQL function encapsulating the v1 recipient policy (direct-kind-only via `push_type_policy`; non-sender active participants with `notification_mode='all'` + `push_enabled` not-false + a device; block-aware) + 8-case db test `pushDispatchRecipients.db.test.ts`. New edge function `supabase/functions/dispatch-push-notifications/index.ts` — shared-secret (`X-Dispatch-Secret`) auth, rpc recipients, declarative `web_push:8030` payload with capped + profanity-safe preview, `npm:web-push` send, 404/410 pruning, structured log; `config.toml` `verify_jwt=false`. Local spike verified: boots in the Deno edge runtime, `npm:web-push` imports, 401 without secret, clean run with it. (Chose `npm:web-push` over `jsr:@negrel/webpush` — matches our base64url VAPID keys; `VAPID_PUBLIC_KEY` added to the function env since web-push needs both keys.) Prior 2026-08-25 (Message push notifications — Units 1–2 built on branch `feat/message-push-notifications`. Unit 1: schema migration `20260817000000_push_subscriptions.sql` (push_subscriptions table, members.push_enabled, push_type_policy seeded direct-only) + db test. Unit 2: VAPID keypair + DISPATCH_SHARED_SECRET generated into gitignored local env, `.env.example` gains VITE_VAPID_PUBLIC_KEY, ops doc `docs/ops/push-notifications-secrets.md`. Unit 3: deep-linkable conversation route `messages/:conversationId` (`NavRoutes.tsx` + `Messages.tsx` reads the param and opens that thread on cold load; unknown/forbidden id falls back to the list) via new pure helper `src/utils/messages/resolveDeepLinkTarget.ts` + test. Unit 4: custom service worker — flipped `vite-plugin-pwa` `generateSW`→`injectManifest`; new `src/sw.ts` (precache + Supabase NetworkFirst ported 1:1 + `push`/`notificationclick` handlers with suppress-if-viewing + per-conversation tag), pure helper `src/utils/push/notificationPayload.ts` + test, `tsconfig.worker.json` (WebWorker lib) referenced from `tsconfig.json` with `src/sw.ts` excluded from the app tsconfig, added workbox runtime deps (precaching/routing/strategies/expiration/cacheable-response @7.4.0). Build verified: injectManifest emits `dist/sw.js` with 162 precache entries. Unit 5: client subscribe/permission logic — `src/utils/push/pushCapability.ts` (supported/denied/needs-ios-install/unsupported resolver + VAPID base64url↔bytes conversion), `src/utils/push/browserPush.ts` (thin mockable serviceWorker/pushManager/Notification wrappers), `src/api/mutations/pushSubscriptions.ts` (upsert/delete row + set members.push_enabled), `src/api/hooks/usePushSubscription.ts` (subscribe/unsubscribe/heal-on-mount, exported from hooks index) + 12 tests; `VITE_VAPID_PUBLIC_KEY` added to `src/vite-env.d.ts`. Unit 6 (settings toggle): new `src/components/messages/PushNotificationSetting.tsx` (prop-driven per-capability control: on/off Switch, iOS add-to-home-screen nudge, denied/unsupported hints) + test, wired into `MessageSettingsModal.tsx`'s Notifications accordion (replaces the "coming soon" placeholder) via `usePushSubscription` + `useCurrentMember`; `push_enabled` added to the `Member` type. Onboarding: new `src/components/messages/PushOnboardingPrompt.tsx` — a SEPARATE three-way first-run prompt (Turn on = subscribe / No thanks = push_enabled false / Not now = leave NULL and re-ask next Messages visit), shown from `Messages.tsx` after the profanity modal while `push_enabled IS NULL` and capability is supported; persists + invalidates the member cache on an explicit answer. + test. **Gated for the foundation PR:** new `PUSH_NOTIFICATIONS_ENABLED` flag in `src/config/featureFlags.ts` (on in dev, off in prod unless `VITE_PUSH_NOTIFICATIONS=true`) wraps BOTH entry points — the onboarding prompt (`Messages.tsx`) and the Settings toggle (`MessageSettingsModal.tsx`, falls back to "Coming soon") — so users can't subscribe to a channel that can't send until the dispatcher+trigger (Units 7–8) ship; noted in `LIST_FOR_ED.md`. Also regenerated `database.types.ts` with the upgraded Supabase CLI 2.115 to end the phantom-diff drift.) Prior 2026-08-18 (Message push notifications brainstorm — captured `docs/brainstorms/2026-08-18-message-push-notifications-requirements.md`: a scoped, standalone extraction of the 2026-04-21 messaging overhaul's Phase 2. Decision: ship Web Push **loud first** (chime on all chats, honoring only existing mute + a global on/off) to a single friendly test league to surface real pain, then build the per-chat/frequency **preferences panel** and Android **reply-from-notification** as v2. v1 = subscription/permission flow, custom service-worker push handler, server-side dispatch on message insert, universal tap-to-open, and explicit iOS "add-to-home-screen" handling. Greenfield pipeline; the `notification_mode` tri-state column already exists to back v2. No code yet.) Prior 2026-07-12 (League Dues Roster — operators can now see who in a league has/hasn't paid annual dues at a glance instead of clicking players one at a time. New page `src/operator/LeagueDuesPage.tsx` (`/league/:leagueId/dues`): current-season roster + paid/unpaid summary + All/Unpaid filter; rows are `PlayerNameLink`s so dues can be marked paid inline. Backed by `src/api/queries/duesRoster.ts` (league-scoped) + `src/components/operator/DuesRosterList.tsx` (icon+text status, colorblind/dark-mode safe) + a new calendar-year `getDuesYearStatus` in `membershipUtils.ts`. The league page's single "Open Finances" button became a two-button `FinancesCard` (Dues + Payout Calculator), mirroring the Scoring card. `RecordDuesModal` now also invalidates `['leagueDuesRoster']` so the list refreshes after marking paid. Branch `feat/league-dues-roster`.) Prior 2026-06-15 (Bye-not-a-real-match #1+#2 implemented — (#1) new migration `20260615000000_bye_matches_skip_table_assignment.sql` CREATE OR REPLACEs `assign_tables_for_week()` to skip `status='bye'` matches (byes are real-UUID rows now, so the old NULL-only filter handed them a real table); DB test `byeMatchTableAssignment.db.test.ts`. (#2) lineup page shows `ByeWeekNotice.tsx` instead of the builder when the match is a bye — both fixes key on the new shared `src/utils/match/isByeMatch.ts` predicate (also adopted by `manualScoringEligibility.hasTwoRealTeams`); `getMatchWithLeagueSettings` now selects team `status`. (#3) the "My Match" nav surfaces (bottom tab / sidebar / drawer) no longer surface a bye matchup — `classifyTier` in `useMyMatchSurfaces.ts` returns null for an `isByeMatch` row, so a today/past-due bye can't appear as an actionable nav destination. (#4) standings no longer show a bye as "Unknown Team" — `fetchSeasonStandings` (`src/api/queries/standings.ts`) now skips any team id missing from the active-only name map, so bye/withdrawn/forfeited teams get no standings row (their matches still count toward the active opponent); also protects playoff seeding, which reads the same function. Branch `feat/bye-not-a-real-match`.) Prior 2026-06-14 (Bye-not-a-real-match brainstorm — captured requirements at `docs/brainstorms/2026-06-14-bye-not-a-real-match-requirements.md` for the two remaining bye gaps: a bye matchup must not consume a table number, and the bye-side team must not be able to enter the lineup page — both routed through the existing `hasTwoRealTeams()` "real match" definition. Also verified the auto-forfeit `pg_cron` sweep is firing daily in production (3 successful 06:00 UTC runs). Branch `feat/bye-not-a-real-match`.) Prior 2026-06-30: (Schedule ⇄ Matchup decoupling, Phases A+B — fixes the duplicate "Week 14" bug by deriving "Week N" from each week's date POSITION instead of storing it. A: new `deriveWeekLabels` helper (`scheduleDisplayUtils.ts`) + `useWeekLabelsForSeasons` hook; every display surface + the operator edit-lock (`isPlayWeekLocked`, golden-tested in `scheduleManagerLock.test.ts`) read derived labels. B: migration `20260621000000` moves blackout labels to `notes` + collapses `season_end_break`→`blackout`; writers + reflow/toggle/season-length aligned. Phase C (drop `week_name`) HELD for a follow-up PR after staging verification — see `LIST_FOR_ED.md`. Branch `feat/schedule-matchup-decoupling`.) Prior 2026-06-13 (Change Season Length recovered — re-applied the stranded season-length feature (PR #211 merged into a side branch, never reached main) onto current main: operators can lengthen/shorten a season before the week-5 lock — `seasonLengthEdit.ts` (bounds/lock), `applySeasonLengthChange.ts` (append-weeks+generate / guarded-tail-delete), `ChangeSeasonLengthDialog.tsx`, + a `teams.schedule_position` column. Migration renumbered to `20260611000002` to clear a collision with the auto-forfeit-sweep migration. Branch `feat/season-length-change-v2`.) Prior 2026-06-13: (Drop team to BYE — ported the operator "drop a team" feature onto current main after its original PRs (#215 stack) merged into side branches instead of main: added `convertTeamToBye()` (pre-season slot→bye repurpose) to `teams.ts`, replaced the placeholder `handleDeleteTeam` in `team-management/useTeamActions.ts` with the full delete/pre-season-drop/mid-season-drop flow, brought in the `20260611000001_drop_team_mid_season_rpc.sql` migration + two DB tests. Branch `feat/drop-team-to-bye-v2`.) Prior 2026-06-13: (Playoff review page — rewrote `PlayoffSetup.tsx` (`/playoffs`) from a configure-first page into a place-only, review-first finals surface: three states (no-playoffs / set-up / locked), editable place-rule, auto-fill-in at season end honoring the configured pairing, safe reset. Core fixes in `playoffGenerator.ts`: `generatePlayoffBracket` now honors the configured matchup style (`realPairsForStyle`), new `arePlayoffMatchupsPopulated` (trustworthy non-null-team signal) + `resetPlayoffMatchups` (null-not-delete); `PlayoffsCard` label aligned to that signal. Deferred: true server-side auto-populate. Branch `feat/playoff-review-page`.) Prior 2026-06-11: (Auto-forfeit sweep — the league's first scheduled job. Added `supabase/migrations/20260611000000_auto_forfeit_sweep.sql`: `pg_cron` + `sweep_auto_forfeits()` (daily, set-based, all leagues) that forfeits past-due unplayed matches where exactly one team is captainless — the foundation that auto-resolves BYE weeks AND dropped-team weeks. Forfeit scoring deferred. Test `src/__tests__/database/autoForfeitSweep.db.test.ts`. Branch `feat/auto-forfeit-sweep`.) Prior 2026-06-07: (Trigger workshop landed — second standalone module workshop in the Workshops building: `triggers` table + 4 officials, never-throw `loadTrigger`, standalone `validateTrigger`, shared `_shared/ExpressionBuilder` extracted from the allocator's FormulaBuilder, full trigger workshop UI + route. Merged current main in to reconcile the stacked branch. Branch `feat/trigger-room`.) Prior 2026-06-06: (Added `docs/file-split-backlog.md` — ranked refactor backlog of files worth splitting, from a codebase sweep. Branch `docs/file-split-backlog`.) Prior 2026-06-05: (Added Facebook OAuth login alongside Google in `Login.tsx`/`Register.tsx` and shipped a public Privacy Policy at `/privacy` — `about/PrivacyPolicy.tsx` — required for Facebook app review. Branch `feat/fb-login`.) Prior 2026-06-02: (Archived ~63 completed/superseded brainstorm, plan & memory-bank docs to `docs/archive/` + `memory-bank/archive/` after a code+PR triage; active/back-burner/reference docs left in place.)
...
| `docs/brainstorms/2026-08-18-message-push-notifications-requirements.md` | Requirements for standalone message push notifications (scoped extraction of the overhaul's Phase 2) | Ship-loud-first Web Push; controls deferred to v2 |
...
| `docs/brainstorms/2026-09-08-coin-flip-component-requirements.md` | Reusable coin flip: two participants in, one winner out; knows nothing about matches, leagues, breaks or ties | Planned (`docs/plans/2026-09-08-001-...`) |
...
| `docs/brainstorms/2026-06-14-bye-not-a-real-match-requirements.md` | Requirements: a bye matchup is not a real match — exclude it from table assignment + block the bye-side team from the lineup page | Both fixes route through the existing `hasTwoRealTeams()` definition; completes the 2026-06-09 bye design; auto-forfeit sweep already done; branch `feat/bye-not-a-real-match` |
...
| `docs/brainstorms/2026-06-14-schedule-matchup-decoupling-requirements.md` | Requirements for decoupling the schedule (dated typed slots: regular/blackout/playoffs) from matchups (chart owns round order); derive week numbers from regular-week position instead of storing `week_name` — kills the week-numbering drift/collision bug class | Three week types (season_end_break folds into blackout "Week Off"); blackout label needs `week_name→label` backfill; control-flow-vs-display sweep; load-bearing invariant = regular dates monotonic in chart order |
| `docs/brainstorms/2026-09-04-tournament-paid-foundation-requirements.md` | Requirements for the Tournaments PAID-tier FOUNDATION (roadmap #1) — real players on `member_id`, organizer roster, QR/link/search self-add → hopper → official list, tier-gate "Premium features" checklist + $0 `PaymentCardForm` checkout, reusable saved setup | Player-run (no org); tournaments DISPOSABLE (only setup/roster/run-count persist); walk-ups = tournament-scoped (NOT app placeholders, no merge-code changes); scoring/handicap/money/venue = forward hooks; future idea: Tournament Series (~#8) |
| `docs/brainstorms/2026-09-08-self-scoring-primer.md` | PRIMER (not the brainstorm) for **self-scoring / individual races** — hands a fresh session Ed's stated vision, the five verified code findings, and the open questions, so the `/ce:brainstorm` doesn't restart from scratch | Feature is BOTH tournament + league (individual-race leagues MUST be covered, not ported later); break order is an existing Pairings dial with an extension point; `match-format` already has `race_to_n`; confirmation system is built but keyed on member ids (walk-ups have none); scoring room `ScoreMatch.tsx` is team/lineup-shaped |
| `docs/brainstorms/2026-09-09-self-scoring-requirements.md` | REQUIREMENTS for **self-scoring individual races** (successor to the primer) — the race as a standalone primitive (per-side goal numbers, games APPENDED never pre-created, gated on "all confirmed and no goal met"), its own storage with parents pointing AT it, a second smaller scoring room reusing the prop-driven confirmation layer, who-may-score stored as a LIST, **three kinds of human** (A registered / B phone+ticket / C no phone — once a ticket counts as identity the scoring layer stops caring which), scanning the QR issues a ticket BEFORE any name is typed, an organizer **Validate** action giving a large single-use 6-digit code that binds a walk-in's ticket to a name he typed in, a match going **in progress is what creates its race**, and a new "Scoring" setup tab (break rule / race + game per bracket side / single-player policy / advance policy, the last two sharing one three-position shape) | Acceptance test = the SAME race runs a league night of N individual races (no port later); `matches` needs season_id + season_week_id so fake-teams-of-one is dead; `ScoreMatch.tsx` + `match_games` NEVER edited; auto-advance is safe because `reopen_bracket_match` exists; notifications / venue-tables / handicap races / modular scoreboard all OUT of scope |
| `docs/plans/2026-09-09-001-feat-self-scoring-individual-races-plan.md` | PLAN for self-scoring individual races — 11 units in five phases, opening with a **league-night walkthrough before the schema exists** (R32's acceptance test run while its output can still change columns for free), then the race primitive standalone, identity, bracket integration, the Scoring tab, and the room | Two P0s the review caught: new `public` tables are born **anon-WRITABLE** via baseline DEFAULT PRIVILEGES (so the race tables need explicit REVOKE, and tickets live in a non-exposed schema), and the automatic advance must run in a **nested caught block** because `advance_bracket_winner` RAISEs — otherwise a rollback eats the rack a player just confirmed. Also: reopen writes to THREE matches (winner, loser-drop, grand-final reset); `add_late_entry` PERFORMs reopen and discards its result; `deriveDissents` silently drops rows whose `side` isn't 'home'/'away' |
| `docs/plans/2026-09-09-002-league-race-walkthrough.md` | PARKED LEAGUE REFERENCE — **not scope, and no tournament unit depends on it.** One league night walked against the designed race to answer one narrow question: is the race primitive being built in a shape a league could never reuse? **It passes** | The self-scoring plan is SOLELY tournaments; the league is not "the race, five times" — lineup sequencing, league scoring and non-predetermined matchups are their own much larger effort (Ed, 2026-09-10). One carry-forward, now in Unit 9: the room is addressable PER RACE and assumes no single race exists, because a league night is several at once. Everything else here is a note to a future league brainstorm |
| `/docs/plans/` | **CE implementation plans** | Output of `/compound-engineering:ce-plan` |
| `docs/plans/2026-09-04-001-feat-tournament-paid-foundation-plan.md` | Implementation plan for the Tournaments PAID-tier FOUNDATION (roadmap #1) — 10 units / 4 phases (A tier gate + paid backbone, B identity registered-link/walk-up, C roster+hopper+self-add, D setup + run-count) | Reviewed by 7 personas + reshaped with Ed; tournaments disposable; walk-ups tournament-scoped (no shared-merge surgery); builds on free-tier (now in main); **Phase A BUILT (A1–A3)** — schema + per-player `payment_methods` card-on-file + reusable `PaymentMethodSetup` + create-page Premium-features checklist ($1 each/$5 cap) + $0 charge-at-checkout seam ("Start & pay $X"); branch `feat/tournament-paid-foundation` |
| `docs/plans/2026-06-14-001-refactor-schedule-matchup-decoupling-plan.md` | Implementation plan for the schedule ⇄ matchup decoupling (derive week numbers from regular-week position, drop stored `week_name`) | 7 units in 3 deploy-ordered phases (A read-paths+sweep / B backfill-labels-to-`notes`+collapse `season_end_break`→blackout / C drop `week_name`) + docs unit; live-season-safe (run controlled renumber SQL before Phase A; monotonic-date invariant gate; grep-gate reader inventory); origin `2026-06-14-schedule-matchup-decoupling-requirements.md`; implement on a branch off fresh main (this work predates it) |
| `docs/plans/2026-08-18-001-feat-message-push-notifications-plan.md` | Implementation plan for v1 loud-first message push notifications (Web Push); hardened after a 5-persona review | 8 units: `push_subscriptions`+`members.push_enabled`, VAPID+dispatch-shared-secret, deep-link `messages/:conversationId` route, `vite-plugin-pwa` generateSW→injectManifest custom `src/sw.ts` (suppress-if-viewing + per-conversation tag), client subscribe/permission + heal-on-mount, onboarding+settings+iOS-nudge UI, `dispatch-push-notifications` edge fn (shared-secret auth, corrected `notification_mode='all'` recipient SQL, blocked-user exclude, capped preview, `jsr:@negrel/webpush` + declarative `web_push:8030`), DB trigger + `pg_net` (CREATE EXTENSION + exception-wrapped) + `push_dispatch_config`; v1 pushes **personal DMs + group chats only** (`direct` kind) via a data-driven `push_type_policy` table (later phases flip a row to enable team/captain/announcement/match kinds; per-user `notification_mode` = the future user console); origin `2026-08-18-message-push-notifications-requirements.md` |
| `docs/plans/2026-09-08-001-feat-coin-flip-component-plan.md` | Implementation plan for the reusable coin flip component | 5 units: pure flip logic + injected random source, the component and its four-state machine (idle → calling → flipping → result), CSS coin animation, an external-control seam so a caller can supply the result, catalog/TOC. No schema, no persistence, no realtime. Break-first assignment and tie resolution are explicitly deferred to their owning Modules; origin `2026-09-08-coin-flip-component-requirements.md` |
| `docs/archive/plans/2026-04-17-001-feat-official-rulebook-reader-plan.md` | Implementation plan for the Official Rulebook Reader | 6 units, active branch `feature/official-rulebook-reader` |
...
| `docs/ops/push-notifications-secrets.md` | Secrets & key management for message push notifications — VAPID keypair + VAPID_SUBJECT + DISPATCH_SHARED_SECRET, where each lives per env, generation commands, rotation runbook | Unit 2 of the push-notifications plan; public key is build-time, private+secret are Supabase secrets / gitignored local env |
...
| `scripts/check-whats-new.mjs` | **CI guard: a PR must EITHER add a What's New entry OR declare `[no-changelog]` in its description.** You can't forget, you can only decide — and a wrong opt-out is one visible line, caught by a human in review. Deliberately NOT "changed src/ ⇒ write an entry": that's wrong most of the time (refactors, test fixes, dependency bumps), and a check that cries wolf gets routed around until it protects nothing. Reads its inputs from FILES rather than shell arguments, since a PR description is arbitrary user text. |
| `scripts/stamp-whats-new.mjs` | Run when cutting a release. Renames the accumulated `unreleased` block in `src/whatsNew/releases.ts` to the given version, dates it (LOCAL date, not `toISOString()` — an evening release west of Greenwich would otherwise be dated tomorrow), and opens a fresh empty block. Refuses rather than guessing if the file's shape has changed. |
| `scripts/check-migration-versions.sh` | **CI guard: fails if two files in `supabase/migrations/` share a version number.** `schema_migrations` is keyed on version alone, so a collision merges cleanly in git (filenames differ) but can never both apply — it surfaces only at deploy, aborting it before the S3 upload. Run by `.github/workflows/checks.yml` on every PR and as a pre-flight step in both deploy workflows. Takes an optional dir arg (default `supabase/migrations`). |
...
- `convertTeamToBye.db.test.ts` - **Drop team to BYE (pre-season).** Exercises `convertTeamToBye()` against local Supabase: reshapes an active team to the canonical bye shape (status='bye', captain cleared, name 'BYE', roster released), and asserts all three guards reject (non-active team, season already has a bye, any match already played).
- `dropTeamMidSeason.db.test.ts` - **Drop team mid-season (atomic RPC).** Exercises `drop_team_mid_season()`: withdraws a team that has already played (past results preserved) and replaces it with a fresh BYE on every remaining match, asserting upcoming opponents get bye weeks and the operation is atomic. Requires local Supabase + the 20260611000001 migration.
- `applySeasonLengthChange.db.test.ts` - **Change season length (apply).** DB-backed coverage of `applySeasonLengthChange()`: lengthen appends weeks and generates their matches; shorten deletes only the unplayed tail (guarded). Requires local Supabase.
- `scheduleDecoupleMigration.db.test.ts` - **Schedule decouple migration (B1).** Verifies the `week_type` CHECK accepts regular/blackout/playoffs and rejects the dropped `season_end_break`, and that a blackout stores its label in `notes`. Requires local Supabase.
- `teamSchedulePosition.db.test.ts` - **schedule_position persistence.** Verifies the `teams.schedule_position` column (migration 20260611000002) is written at matchup creation so season-length changes can re-pair correctly. Requires local Supabase.
- `byeMatchTableAssignment.db.test.ts` - **BYE matches get no table.** Builds a league + venue (3 tables) + a week of three matches (real / real-vs-BYE / real) and calls `assign_tables_for_week()`, asserting the real matches get tables 1 and 2 and the BYE match gets none (and burns no slot). Requires local Supabase + the 20260615000000 migration.
...
- `brackets.lifecycle.db.test.ts` - **Tournament bracket tool / Unit 3** — lifecycle tests for the write RPCs (`start_bracket` + `advance_bracket_winner`), driven via raw `pg`: tree persists with pointers resolved + bracket goes live; advance propagates winner→next and readies the next match; terminal completion flips the bracket to `complete`; the guard makes a duplicate/conflicting advance a no-op; a non-participant winner is rejected; double-elim loser drops into the losers bracket; grand-final reset activates on an LB-champ win and is removed on a WB-champ win. 7 tests.
- `brackets.reopen.db.test.ts` - **Tournament bracket tool / reopen (undo)** — `reopen_bracket_match`: clears a decided match's winner (→ ready) + pulls the advanced player back out of the next match (→ pending); the downstream guard refuses when a later match was already played; reopening the terminal match flips the bracket complete→live; no-op on a not-complete match. 4 tests.
- `brackets.sweep.db.test.ts` - **Tournament bracket tool / Unit 7** — close-tombstone + inactivity sweep: `sweep_stale_brackets` hard-deletes closed + past-idle brackets and cascades to children, while keeping a recently-active live bracket AND a just-touched setup bracket (the "setup trap" — never sweep an organizer mid-configuration); a truly-abandoned setup bracket (idle past threshold) IS swept. 5 tests.
- `brackets.schema.db.test.ts` - **Tournament bracket tool / Unit 1** — schema verification for `brackets`/`bracket_participants`/`bracket_matches`: default columns (status=setup, seeding=seeded, uuid share_token), CASCADE delete of children, format/status/side CHECK rejections, unique share_token + unique (bracket_id, seed), realtime publication membership + `REPLICA IDENTITY FULL`, and the `get_bracket_share` RPC (names-only, `{found:false}` on unknown token, no created_by/member_id leak). 10 tests.
- `brackets.paid.schema.db.test.ts` - **Tournament PAID foundation / Unit A1** — the paid columns on `brackets`: free-tier inserts unaffected (tier defaults free, premium_features empty, no card), a paid tournament persists with premium features + game_type (card lives on the player, not here), the tier CHECK rejects unknown values, the tier↔premium_features invariant (a premium feature on a free bracket is rejected / allowed when paid), and the inactivity sweep stays tier-agnostic (both tiers disposable). 6 tests.
- `bracket.hopper.db.test.ts` - **Tournament PAID foundation / Unit C1** — hopper + roster: admitting a registered player (hopper→official) auto-adds a sticky roster row (trigger); eject leaves the roster intact; one registered identity per bracket (UNIQUE); multiple walk-ups (member_id NULL) allowed; walk-up admission creates NO roster row (registered-only). 5 tests.
- `bracket.join.db.test.ts` - **Tournament PAID foundation / Unit C2** — the self-add join RPC (`join_bracket_hopper`) branch logic: unknown token → `not_found`; a valid token on a bracket past setup → `not_accepting`; a valid setup token with no session → `not_signed_in` (proves anon cannot self-add). 3 tests (authed happy path covered by the mutation wiring + manual pass).
- `payment_methods.schema.db.test.ts` - **Tournament PAID foundation / per-player card-on-file** — `payment_methods`: a card saves with defaults (is_default true), at most one DEFAULT per member (partial-unique), a second NON-default card is allowed, the member FK is enforced, and `brackets.payment_method_id` nulls out when the card is deleted (SET NULL, so a paid tournament never dangles). 5 tests.
...
- `dbTestUtils.ts` - Supabase/pg clients + `executeSql` for the `db` test project
- `onboardingFixtures.ts` - Builds a placeholder-captain (or staff-captained) team with roster room for the onboarding-cascade DB suites, so they stop hunting the shared seed for shapes it does not contain
...
- `LeagueDuesPage.tsx` - **Dues Roster** page (`/league/:leagueId/dues`, operator-gated). Lists the league's current-season roster with each player's annual-dues status (calendar-year), a paid/unpaid summary, and an All / Unpaid-only filter. Rows are `PlayerNameLink`s so dues can be marked paid inline. Reached from the Finances card's Dues button. Query: `api/queries/duesRoster.ts`; rows: `components/operator/DuesRosterList.tsx`.
...
**Playoffs**
- `PlayoffSetup.tsx` - Playoff **review** page (`/playoffs`). Place-only (never team names/standings): three states — no-playoffs (terminal "doesn't run playoffs") / set-up (editable place-rule via `PlayoffBracketPreviewCard` + status line + tucked-away format edit) / locked (matchups set → view-on-schedule + Reset). Fills in automatically when the season completes (honors the configured pairing); manual override + safe reset. Tests: `PlayoffSetup.test.tsx`, `playoffGenerator.pairing.test.ts`, `__tests__/database/playoffMatchups.db.test.ts`.
...
- `SeasonSchedulePage.tsx` - Season matchups page; renders **derived** week labels (`deriveWeekLabels`, no stored `week_name`). Operator-only **"Export for LMS"** button downloads the schedule as a two-sheet .xlsx matching CSI/FargoRate LMS's import (see `/utils/lmsExport/`).
...
- `OnboardingAttentionCard.tsx` - LO's combined onboarding to-do on `LeagueDetail`: captains-still-needing-their-link + pending join requests in ONE collapsible card. **Loud** (accent border + count badges, expanded) when there's work, **muted** (collapsed, "All players onboarded") when not. Captains list inlined (shows the captain's FULL name via the 20260818000000 RPC change); join requests reuse the shared `JoinRequestList`. Copy-link flashes "Copied!" for ~2s then reverts (always re-clickable). Replaced the separate `OnboardCaptainsList` + inline `JoinRequestList`.
- `OnboardingAttentionCard.test.tsx` - 3 tests: muted "all onboarded" (no badges) when no work; conspicuous count badge + full captain name + always-present join-request surface; Copy link writes `/join/:token` and flashes "Copied!".
...
- `ByeWeekNotice.tsx` - Shown on the lineup page in place of the lineup builder when the match contains the BYE team. "You have a bye this week — nothing to do" + Back to Schedule; icon+text (no color-only). Prevents the bye-side player from building a lineup against the roster-less bye.
...
- `DuesRosterList.tsx` - Renders the per-player rows of the league **Dues Roster** (`LeagueDuesPage`). Each row is a `PlayerNameLink` (mark dues paid inline via its popover) + a dues-status pill shown as **icon + text label** (`Paid 2026` / `Expired 2024` / `Never Paid`) — never color alone, semantic tokens for dark mode.
- `FinancesCard.tsx` - **Finances** card on the league page (on `SectionCard`; two slim icon-left buttons mirroring the Scoring card): **Dues** → `/league/:id/dues` (annual membership dues roster) and **Payout Calculator** → `/league/:id/finances`. Replaced the former single "Open Finances" `DashboardCard`.
...
- `LmsEnteredCheckbox.tsx` - The "Entered / Not entered" checkbox an operator ticks after hand-entering a match into CSI/FargoRate LMS. Rendered in BOTH places that job happens — the manual-scoring match picker (the at-a-glance backlog) and the printable LMS results sheet (tick it and hit Next, no trip back). State is shared league-wide, so two operators can't double-enter. Reads done/not-done by word as well as by the box (colorblind-safe), and flips instantly while the write is in flight. Tested in `__tests__/LmsEnteredCheckbox.test.tsx` (6 cases).
...
- `api/mutations/paymentMethods.ts` - **PAID card-on-file writes.** `upsertDefaultPaymentMethod` — saves the player's DEFAULT card (from `PaymentCardForm`'s mock token) to `payment_methods`, replacing the existing default in place (one-default-per-member). Only token + last4/brand stored; raw PAN/CVV never leave the form.
- `api/queries/paymentMethods.ts` - **PAID card-on-file read.** `getDefaultPaymentMethod(memberId)` → the member's default saved card, or null (seeds the create flow: show the card form vs. reuse the card on file).
- `api/hooks/usePaymentMethods.ts` - TanStack hooks: `useDefaultPaymentMethod` (read, enabled once a memberId exists) + `useSaveDefaultPaymentMethod` (upsert + invalidate).
- `api/mutations/brackets.ts` - **Tournament bracket tool / Unit 3 + 7 (+ PAID A1/A2).** Write ops: `sweepStaleBrackets` (Unit 7 — best-effort `sweep_stale_brackets` RPC call, swallows errors), `createBracket` (setup status; DB-generated share_token; sweeps stale brackets FIRST — cleanup-on-create; **paid:** accepts `premiumFeatures`/`gameType`/`paymentMethodId`, sets tier=paid when any feature is checked), `setParticipants` (resolves seed order by mode — random is Fisher–Yates FROZEN here so a persisted bracket is never re-derived — then replace-inserts seed 1..N), `startBracket` (runs the pure engine then persists via the `start_bracket` RPC atomically), `advanceWinner` (guarded advance via `advance_bracket_winner` RPC; returns whether it advanced), `closeBracket` (status='closed' tombstone, NOT a hard delete). Seeding-mode resolution + engine call live here; the RPCs own the atomic/guarded DB work.
- `api/queries/brackets.ts` - **Tournament bracket tool / Unit 3.** `getBracket(id)` = authed organizer view (bracket + participants + matches via parallel selects); `getBracketShare(shareToken)` = the PUBLIC read via the `get_bracket_share` RPC (names only; `{found:false}` on unknown token) — kept in the same file (no single-export share file). Row + share-view types exported here.
- `api/hooks/useBrackets.ts` - **Tournament bracket tool / Unit 3 + 8.** TanStack Query hooks: `useBracket`/`useBracketShare` reads (staleTime 0 — live during play; `useBracketShare` also polls 15s while live as an anon-realtime fallback), `useBracketsByCreator` (index list, Unit 8), + `useCreateBracket`/`useSetParticipants`/`useStartBracket`/`useAdvanceWinner`/`useCloseBracket` mutations that invalidate the `queryKeys.brackets.*` keys.
...
#### Tournament Bracket UI (`/brackets/`)

- `BracketsIndexPage.tsx` - **Brackets index (Unit 8).** The `/brackets` landing surface behind the gated "Brackets" nav entry: lists the current member's active brackets (setup/live/complete, closed excluded) as links into each with a status badge, a "New bracket" action, and a first-run empty-state CTA. Test: `BracketsIndexPage.test.tsx`. **Now TWO lists:** tournaments you're running and tournaments **you're playing in** — the second fixes a dead end where a player who joined by QR had no route back from inside the app (keep the tab open, or go find the code again). Joined rows link to the PLAYER's view (`/brackets/join/:joinToken`), not the organizer's, and badge "Waiting" when that's their standing. Headings only appear when both lists exist, and someone playing in a tournament they didn't create no longer gets the first-run pitch.
- `PublicBracketPage.tsx` - **Public bracket share page (Unit 6).** The no-auth `/brackets/share/:shareToken` page: reads names-only data via the `get_bracket_share` RPC (verified anon-callable), reuses `BracketTree` read-only, stays live via realtime + a 15s polling fallback (anon realtime unproven). Unknown/closed/swept token → a funnel-friendly "This bracket has ended" state with a "Create your own bracket" CTA. Test: `PublicBracketPage.test.tsx`.
- `BracketView.tsx` - **Organizer live bracket view (Unit 5).** The authed `/brackets/:bracketId` page: renders the live tree, tap-to-advance a ready match's winner (AlertDialog confirm → `advanceWinner`; a no-op advance toasts "already decided"), champion banner + Close action when complete, read-only when not `live`. Data-derived (recomputes from fetched rows) + live via `useBracketRealtime`. Test: `BracketView.test.tsx`.
- `BracketTree.tsx` - **Bracket tree renderer (Unit 5).** Lays out rounds as horizontally-scrolling columns; double-elim uses a tabbed Winners/Losers/Grand-Final surface (v1 mobile layout), single-elim shows just the winners tree. Pure presentation from the view-model; backs both the organizer (interactive) and public (read-only) views.
- `MatchCell.tsx` - One match cell: two slots (name / "—" TBD), winner bolded; a ready match's filled slots are tappable buttons in organizer mode.
- `bracketViewModel.ts` - **Pure render model (Unit 5).** `buildBracketView(participants, matches)` → names resolved per slot, winner flags, matches grouped by side+round; `isSlotPickable`; `championName` (terminal-match winner, preferring an activated reset). Consumed by both views; structurally accepts DB rows AND the share-RPC shape. Test: `bracketViewModel.test.ts`.
- `useBracketRealtime.ts` - Subscribes to this bracket's `bracket_matches` (+ the `brackets` row) filtered by `bracket_id` and invalidates the given query key. Mirrors `useMessagingRealtime`; relies on `REPLICA IDENTITY FULL` for filtered UPDATE events.
- `CreateBracketFlow.tsx` - **Create-bracket flow orchestrator (Unit 4 + PAID A2).** Lightweight linear flow (details → participants → review) for an authed organizer; on submit runs create → setParticipants → startBracket then navigates to `/brackets/:id`. **Paid:** loads the player's card on file, renders the `PremiumFeaturesSection` on the details step, saves a verified card to `payment_methods` on first premium-feature enable, and passes `premiumFeatures`/`gameType`/`paymentMethodId` to `createBracket`. On failure the bracket stays in `setup` (retryable) + toasts. Test: `CreateBracketFlow.test.tsx`. **PAID C3 fork:** with `real_players` checked there is no player list to type — the flow stops after Details (stepper hidden, button reads "Create & add players"), creates the tournament in `setup`, and hands off to `paid/BracketSetupPage`, which owns Start AND the checkout charge for that path.
- `paid/premiumFeatures.ts` - **PAID A2 catalog.** Plain constant list of premium features (real players, self-scoring, handicap races, venue/tables, alerts) — key/label/blurb/priceCents — that drives the checklist. Helpers `getPremiumFeature`, `totalPriceCents`, `formatPrice`. v1: listed so the checkout works end-to-end; most aren't functional yet (their engines are later brainstorms); prices are illustrative placeholders (Jack sets real pricing). Test: `premiumFeatures.test.ts`. `hasPremiumFeature(features, key)` is the **gate every feature reads** — each one is sold separately, so each must gate separately (sign-up links must not imply the entry-fee tracker, or vice versa); it also absorbs the nullable `premium_features` so a free tournament can't slip through a call site. `chargeBreakdown(keys)` itemises a charge for the checkout card — lines, subtotal, the cap's discount, and a total pinned by test to equal `totalPriceCents`; an unknown key (a retired feature) is skipped rather than rendered as a blank row.
- `paid/PremiumFeaturesSection.tsx` - **PAID A2** — the "Premium features" checklist on the create page. Each feature is a checkbox + blurb + $1 price; turning one ON with **no card yet** opens the generic `PaymentMethodDialog` (once), otherwise enables immediately; OFF is immediate. Shows a **"payment method established"** indicator, the preview total ($5 cap), and "not charged now — only when you start". Test: `PremiumFeaturesSection.test.tsx`.
- `paid/participantKind.ts` - **PAID B2** — `participantKind(entry)`: `registered` (member_id set) vs `walkup` (no member) — the branch point later features read. Derived, not stored.
- `paid/participantIdentity.ts` - **PAID B1** — `resolveParticipantIdentity(entry, member?)`: a registered player → nickname-primary + player number + home (same-name disambiguators); a walk-up → typed name only. Pure + guarded (never throws). Test: `participantIdentity.test.ts`.
...
- `paid/JoinHopperPage.tsx` - **PAID C3** — the **player's live tournament page** (`/brackets/join/:joinToken`), where a scanned QR / opened link lands. Not a one-shot "you're in" confirmation: joining happens on arrival and is announced with a toast, and the page itself is a live view (realtime on the hopper) of the room filling up. Two tabs — **Players** always, **Bracket** the moment the organizer starts. A signed-out visitor still SEES the tournament (the read is names-only) and is offered sign-in that returns here with `?redirect`; a `name_taken` result sends them to their profile. Test: `JoinHopperPage.test.tsx`.
- `paid/PlayerTournamentView.tsx` - **PAID C3** — the Players tab: where the viewer stands, then who's in and who's waiting. Deliberately COMPACT (rows, not cards) — it's read one-handed on a phone in a bar, and a card per fact pushed the player lists off screen. **Entry-fee status appears ONLY for the viewer's own entry**; the page hangs on a wall, so listing who still owes would make it a debt board. Test: `PlayerTournamentView.test.tsx`.
- `paid/TournamentRulesBar.tsx` - **PAID C3** — the tournament's rules as one wrapping line of chips, directly under the name: game, single/double elimination, how many losses knocks you out, and the grand-final rule when it applies. Second on the page because "what am I walking into" is the next question after "right tournament?" — and on a phone it has to cost a line, not a card. Test: `TournamentRulesBar.test.tsx`.
- `gameTypes.ts` - The games a tournament can be played as — **single source**. Previously a list inside `DetailsStep` plus a second, drifting copy of the labels inside `tournamentRules` (which rendered "9-ball" where the create page said "9-Ball"). The create page, the organizer's info tab and the player-facing rules line now all read this. `gameTypeLabel` falls back to the raw value rather than "Unknown", since the column is free-form in v1.
- `paid/AddMyNameCard.tsx` - **PAID C3** — what a signed-out player sees, in the order that serves them: **"You're not signed in"** + Sign in (carrying `?redirect` back here), then a short case for registering, then **"Or play as a guest"** — a name up to 12 characters. Guest is last but never buried; the walk-in who'll never install an app must still get on the list. The register button carries **no** redirect on purpose: `Register` doesn't navigate on success and its OAuth handoff returns to the app root, so promising a trip straight back would be a lie — the copy says to scan again instead. Rejections render inline beside the box, not as a toast that vanishes mid-read. Test: `AddMyNameCard.test.tsx`.
- `paid/walkupMemory.ts` - **PAID C3** — the per-tournament note in a walk-up's own browser (`bracket-walkup:<joinToken>`), so returning to the same tournament recognises them. Keyed per token, so a name entered for one tournament never appears on another. A **convenience, not truth**: the page re-checks it against the live list every visit and drops it if the organizer removed them. Every access try/catch-wrapped — storage throws outright in some private modes. Test: `walkupMemory.test.ts`.
- `paid/tournamentRules.ts` - **PAID C3** — `tournamentRules(bracket)`: the tournament's rules as label/value lines for a player (format, **how many losses puts you out**, the grand-final reset when it applies, the game). Omits an unset field entirely rather than showing a blank — a blank reads as a missing answer, no line reads as "not part of this tournament". Test: `tournamentRules.test.ts`.
- `paid/HopperView.tsx` - **PAID C3** — the organizer's setup screen for a paid tournament: a sticky count bar over three STACKED groups — **In the tournament** (admitted; what Start seeds) → **Waiting to be added** (scanned/linked/added candidates) → **Past players** (sticky roster, one tap to add). Stacked not tabbed because admitting bounces between the groups. Reads `useBracketHopper` + `useBracketRoster`; writes via admit / set-paid / eject / add-registered, all of which invalidate both reads. Test: `HopperView.test.tsx`. Carries the **"Add anyone still waiting when I start"** choice, because it is a standing rule about THIS list rather than a checkout option — shown even when nobody is waiting yet, and off by default.
- `paid/hopperGroups.ts` - **PAID C3** — `buildHopperGroups(entries, roster)`: splits the hopper by `status` into the official + waiting groups (official in seed order, waiting in arrival order), resolves every row's display identity, and flags names shared across ANY of the three groups so the view insists on the disambiguators. Pure; the no-duplicates guarantee it depends on comes from `get_bracket_roster`. Test: `hopperGroups.test.ts`. Past-player rows carry a `key` (member id, or `walkup:<name>` for a remembered walk-up, which has no id).
- `paid/HopperEntryMenu.tsx` - **PAID C3** — a tappable hopper row + its action menu. Waiting rows offer **Add as paid / Add as unpaid** (admit); official rows flip the paid flag; both offer **Remove** behind a two-click confirm whose copy comes from `removalConsequence`. Candidate rows show how the player arrived (Scanned in / Joined by link / Added by you). **All money language is gated on `trackEntryFees`** (`payment_tracker`): without it, admitting is a single "Add to tournament" and there are no Paid/Unpaid badges or Mark-as-paid item — the stored flag just defaults to unpaid, unseen.
- `paid/removalConsequence.ts` - **PAID C3** — the one true sentence for the Remove confirm, which has three answers: a registered player stays in Past players; an ADMITTED walk-up's name stays (the walk-up roster remembered it); a walk-up who was only ever waiting was never saved and would have to be re-typed. Pure + own file so the promise can't drift from what the roster triggers actually keep. Test: `removalConsequence.test.ts`.
- `paid/PastPlayerMenu.tsx` - **PAID C3** — the past-player row's action menu, so every name on the setup screen behaves the same way (tap for a menu). **Add to this tournament** re-adds them; **Forget this player** is the housekeeping valve on an otherwise sticky list, behind a two-click confirm whose copy is explicit that it changes the saved list and nothing about any tournament they've played. Test: `PastPlayerMenu.test.tsx`.
- `paid/HopperIdentityLine.tsx` - **PAID C3** — one player's name block, shared by all three groups: nickname-primary in bold with `#number · City, ST` underneath, a `walk-up` tag for accountless entrants, and an explicit warning when a name is shared on-screen and there is nothing to tell them apart.
- `paid/HopperGroup.tsx` - **PAID C3** — presentational section wrapper for one hopper group: heading + live count, and per-group empty copy that says what would fill it.
- `paid/AddRegisteredPlayer.tsx` - **PAID C3** — the organizer's search-add: find a player **by name or player number** and put them in the waiting room. Wraps the app's existing `MemberSearchCombobox` rather than growing a second search. **Registered players only** and **"All" chip only** — placeholders belong to a league's team structure, and My Org / State / Staff are league scopes a tournament organizer (a player with no org) can't use. Excludes anyone already in the hopper, since the database would refuse them and an always-failing option is worse than none. Test: `HopperView.test.tsx`.
- `paid/AddWalkupForm.tsx` - **PAID C3** — the organizer's type-a-name entry on the setup screen. A typed name creates a **walk-up** (no account; the name is the whole identity) in the waiting room, alongside the two self-service paths (QR / join link). Keeps the text on a failed add so nothing is lost, and swallows the rejection because the caller owns the toast. Duplicate names are allowed on purpose — two people really can both be Slim, and the screen flags a shared name rather than refusing it. Searching for a REGISTERED player by name is a separate piece, not built. Test: `AddWalkupForm.test.tsx`.
- `paid/BracketSetupPage.tsx` - **PAID C3** — the setup screen (`/brackets/:bracketId/setup`) a "Real players & sign-up" tournament sits on between creation and Start, while players join by QR/link. Hosts `HopperView`, copies the **join link** (`join_token`, never the view-only `share_token`), and owns Start as a three-step hand-off: `finalize_bracket_hopper` → `start_bracket` (the same call the free flow makes) → `chargeForStart` LAST, so a failed start can't leave a charged-but-not-started tournament. An already-started bracket redirects to its bracket view. Test: `BracketSetupPage.test.tsx`. Layout: a **setup card whose tabs sit at ITS bottom** (Info / Players — they belong to the card, not the screen), and **Start & pay as a separate card below it**, so starting is reachable from whichever tab the organizer is on. **Tab-budget note:** nearly every premium feature configures BEFORE the bracket is drawn (venue, tables, alerts, handicaps, payouts), so this one tab set is where crowding lands — mobile takes 4 comfortably and 5 at most, so combine as features arrive rather than adding one per feature. **Two guards in front of Start, in order:** (1) if players are still waiting and the sweep choice is off, a three-way confirm — add them and start / start without them / go back — since a bracket can't be redrawn and leaving someone out must be a decision, not an omission; (2) `ConfirmStartDialog`, the final look at the FINAL count and the exact charge. Order matters: the waiting answer changes the count, so the confirm comes second to show a number that's actually true. The waiting warning's buttons read by weight: the two real choices share a row at equal width (**Start without them** in the new `warning` variant, **Add them** primary), with **Go back** smaller and plainer underneath.
- `paid/BracketInfoTab.tsx` - **PAID C3** — the organizer edits the tournament while it's still in setup: name, game, format, and the grand-final rule (offered only for double elimination). These were set once on the create page and then unreachable, which is wrong for a tournament that deliberately sits in setup all evening — a name gets typed in a hurry and the game often isn't decided until people turn up. Save is disabled until something differs, and the draft is NOT reset by a background refresh (the page has realtime, which would otherwise delete half-typed edits). Locked after start, enforced by the mutation's `status='setup'` guard, not just by hiding the form. Test: `BracketInfoTab.test.tsx`.
- `paid/bracketDestination.ts` - **PAID C3** — `bracketDestination(bracket)` / `usesHopperSetup(bracket)`: which page a tournament opens at, in ONE place so the index and the bracket view can't disagree. A sign-up tournament in `setup` opens on its **hopper** (it sits there on purpose while players scan in); everything else — free tier, and paid without sign-up — opens on the bracket. Both halves are required: no feature means no hopper to show, and a started tournament has no use for the setup screen. Test: `bracketDestination.test.ts`.
- `paid/LateEntryDialog.tsx` - **PAID** — seat a latecomer in an unused bye. Reached from the **Add player** action on a bye cell in the live bracket. Blunt about the consequence, because it un-decides a match somebody was already told they won: "Slim has a bye. Adding someone here gives them an opponent, so Slim plays this round after all." Refusals render inline beside the box, not as a vanishing toast.
- `paid/JoinQrPoster.tsx` - **PAID C3** — the printable / big-screen join sign (`/brackets/:bracketId/qr`, organizer-only). A full PAGE, not a dialog, because one page prints as one sign to tape around the bar and the same page thrown on a TV is already the right shape. Available from creation (`join_token` is DB-generated on insert). QR forced **black-on-white** rather than theme colours — it has to survive printing and scanning across a dim room, and a themed QR in dark mode is unreadable both ways. `print:hidden` chrome + `window.print()`, matching `LmsResultsSheet`. Test: `JoinQrPoster.test.tsx`. Also offers **SVG / PNG downloads** so the organizer can drop the code into their own flyer.
- `paid/qrExport.ts` - **PAID C3** — saves the join QR as a file for an organizer's own flyer. **SVG** (vector — stays crisp blown up to poster size, what design tools want) and **PNG** at 1024px (for tools that reject SVG), rasterized via an off-screen canvas with an explicit white fill so a transparent export can't become an unscannable code on a dark flyer. `svgToString` adds the `xmlns` a standalone .svg needs to open at all (inline SVG in HTML doesn't need it) and clones rather than mutating the displayed node. Right-click is NOT a substitute — browsers only offer "copy image" on an `<img>`. Test: `qrExport.test.ts`.
- `paid/joinUrl.ts` - **PAID C3** — `joinUrl(joinToken)`: the one builder for the player-facing join URL, now shown in three places (copied from setup, encoded in the QR, and printed underneath it as typed-fallback text). If those drifted, the fallback would send people somewhere the QR doesn't. Encodes `join_token`, never the view-only `share_token`.
- `paid/StartTournamentPanel.tsx` - **PAID C3** — the Start control: a live "Starting with N players" count (reflecting the waiting-list sweep choice), the itemised charge, and the Start / Start & pay button. The sweep choice itself lives with the waiting list it describes, not here. Test: `StartTournamentPanel.test.tsx`.
- `paid/ChargeBreakdownPanel.tsx` - **PAID C3** — the collapsed accordion on the checkout card ("2 features · $2.00"), wrapping the shared `ChargeReceipt`. Closed by default: the price on the button is the headline, and the itemisation is for whoever wonders what it covers. Test: `ChargeBreakdownPanel.test.tsx`.
- `paid/ChargeReceipt.tsx` - **PAID C3** — the itemised charge, laid out like a receipt: items left with amounts in a right-hand column, a rule, then **Total pushed over beside its amount** and set a step larger (a total styled like the rows above reads as one more thing being charged). Tabular figures so prices line up; **Subtotal + discount lines when the $5 cap bites**, so the arithmetic reconciles on the page. Shared by the checkout card's accordion AND the final confirm, so the money glanced at and the money approved come from one component — two receipts computed separately is how a checkout drifts into showing two numbers.
- `paid/ConfirmStartDialog.tsx` - **PAID C3** — the last tap before anything irreversible. Start draws a bracket that can't be redrawn AND charges a card, so it confirms the two facts worth checking: the FINAL player count (after any waiting-list decision, so it can differ from the card behind it) and the exact charge. The confirming button names the amount — "Pay $2.00 and start" — rather than a bare "Confirm", so nobody taps a charge blind.
...
- `generateBracket.ts` - **Bracket generation engine (Unit 2).** `generateBracket(participantCount, {format, grandFinalReset})` → the in-memory match tree (`GeneratedMatch[]`). `generateSingleElim` builds the winners tree as exactly N−1 matches (a bye is NOT a match — the top seed starts in round 2) with `next_match` pointer wiring. Pure + deterministic (works on seed positions only; seeding-mode order is resolved upstream once). Dispatches to `doubleElim.ts` for double-elim. Tests: `generateBracket.test.ts` (19). Types: `src/types/bracket.ts`.
- `doubleElim.ts` - **Double-elimination generation (Unit 2b).** `generateDoubleElim(N, reset)` → winners + losers bracket + grand final (+ conditional reset). WB round r loser drops to LB round 2r-2 (cross-seeded to reduce rematches); LB alternates major (meets WB drops) / minor (LB survivors) rounds. Byes handled by a PRODUCER MODEL + `resolve()` contraction (a half-bye match passes its one real player through; fully-phantom → downstream bye), so bye "matches" never appear and the output is exactly 2N−2 rows (+1 for reset). Pure/deterministic; "works, not perfect" cross-seeding.
...
- `clipboard.ts` - `copyText(text)` — copy that works OUTSIDE a secure context. `navigator.clipboard` exists only on HTTPS/localhost, so reaching a dev server by its LAN address (to test on a phone) or an embedded webview makes it throw — exactly where sharing a link matters most. Falls back to a throwaway `<textarea>` + `execCommand`, and returns false rather than claiming success so the caller can offer the text another way. Extracted from `ShareAppCard`, which had the only correct implementation; `ShareLinkSection` and both bracket copy buttons now use it. Test: `clipboard.test.ts`.
- `membershipUtils.ts` - Membership utilities. `getMembershipStatus` (rolling-365-day) + `getDuesYearStatus` (**calendar-year** status — `Paid YYYY` / `Expired YYYY` / `Never Paid`, timezone-safe via `parseLocalDate`; used by the dues roster + any paid/unpaid summary so those surfaces agree).
...
- `duesRoster.ts` - `fetchLeagueDuesRoster(leagueId)` — current-season roster (players on active/upcoming-season teams) of one league with each member's `membership_paid_date`, for the Dues Roster page. League-scoped (dues sanctioning is per league).
...
- `useLmsEntryMutations.ts` - **`useSetMatchLmsEntered`** — sets/clears `matches.lms_entered_at`, the marker for "this match's results are already hand-typed into CSI/FargoRate LMS." Optimistic: writes the new value into every cache holding the match (single-match detail + its sub-keys, and the nested `WeekSchedule[]` season-schedule lists) so the tick survives navigating straight to the next match, then reconciles `onSettled`. Rolls back on failure.
...
| `supabase/migrations/20260611000001_drop_team_mid_season_rpc.sql` | **Drop team mid-season** — `drop_team_mid_season(p_team_id, p_season_id)` SECURITY DEFINER RPC: atomically withdraws a team that has already played (past results preserved) and replaces it with a fresh BYE on every REMAINING match, so upcoming opponents get bye weeks. Companion to the pre-season `convertTeamToBye()` client path. |
| `supabase/migrations/20260611000002_add_team_schedule_position.sql` | **Change Season Length** — adds `teams.schedule_position`, persisted at matchup creation so a season can be re-paired when lengthened/shortened. Renumbered from `20260611000000` on recovery to clear a collision with `20260611000000_auto_forfeit_sweep.sql`. |
...
| `supabase/migrations/20260615000000_bye_matches_skip_table_assignment.sql` | **BYE matches consume no table.** CREATE OR REPLACEs `assign_tables_for_week()` to also skip matches where either team is `status='bye'` — the original only skipped NULL-team matches, but byes are now real `teams` rows with real UUIDs, so they were being handed a real table. NULL checks kept (playoff-TBD). Original migration stays immutable (already in prod). |
...
| `supabase/migrations/20260621000000_schedule_decouple_labels_and_types.sql` | **Schedule decouple — Phase B1.** Backfills blackout/season-end-break labels into the `notes` column, collapses `season_end_break` rows into `blackout`, and narrows the `week_type` CHECK to regular/blackout/playoffs (one transaction). `week_name` stays (NOT NULL, now unread) until Phase C drops it. Part of the derive-week-numbers-from-position refactor (see `docs/plans/2026-06-14-001-refactor-schedule-matchup-decoupling-plan.md`). |
| `supabase/migrations/20260818000000_onboarding_full_captain_name.sql` | **Onboarding list shows the captain's full name.** CREATE OR REPLACEs `get_league_teams_for_onboarding` so `captain_name` is the captain's first + last name (was nickname-first `member_display_name`, e.g. "Mike J.") — the LO needs the real name to know who each join link is for. Only that one field changes. |
...
| `supabase/migrations/20260904160417_tournament_brackets.sql` | **Tournament bracket tool — Free Tier v1 (whole feature, consolidated).** One migration = the entire schema + server surface. **Tables:** `brackets`, `bracket_participants`, `bracket_matches` (the match tree as pointer-columned rows: `next_match_id`/slot for the winner, `loser_next_match_id`/slot for double-elim drops); non-enumerable `uuid` `share_token`; child rows CASCADE on bracket delete; nullable PAID-AWARE `member_id` hook (unused in v1); `brackets` + `bracket_matches` on `supabase_realtime` with `REPLICA IDENTITY FULL`. **RPCs:** `get_bracket_share(uuid)` (anon names-only public-read boundary while RLS off, mirrors `get_team_join_view`); `start_bracket(uuid, jsonb)` (atomically persists the engine tree — seed→participant + engine-key→match-uuid — and goes `live`); `advance_bracket_winner(uuid, uuid)` (guarded advance: acts only when still `ready`+unwon, propagates winner→next + loser→loser_next, activates/removes the conditional grand-final reset, readies newly-full matches, completes the bracket, bumps `last_activity_at`); `sweep_stale_brackets(int default 7)` (inactivity janitor — cascade-deletes closed + idle brackets, called at create time). The three write RPCs are `authenticated`-only (`REVOKE EXECUTE … FROM PUBLIC, anon` — Postgres/Supabase auto-grant to anon, so revoked since the public share route is a real anon surface); `get_bracket_share` stays anon. Tests: `brackets.schema` / `brackets.lifecycle` / `brackets.sweep` `.db.test.ts`. |
| `supabase/migrations/20260905163726_bracket_paid_foundation.sql` | **Tournament PAID foundation — Phase A, Units A1 + A3 (paid schema + charge seam).** Adds to `brackets`: `tier` (free\|paid, CHECK), `premium_features` (text[], the organizer's checklist), `game_type`, and the **charge-at-checkout seam** `charged_at` + `charge_amount_cents` (recorded at Start; $0 mock, Jack's Stripe charge slots in). Also adds `bracket_participants.entry_fee_paid` (organizer-asserted paid/unpaid for the `payment_tracker` feature — cash collected outside the app). A CHECK (`brackets_premium_implies_paid_check`) enforces the tier↔premium_features invariant (any feature checked ⇒ tier=paid). **The card-on-file does NOT live here** — it's a per-player asset in `payment_methods`; the FK `brackets.payment_method_id` (added in the payment_methods migration) points at the card to charge. **No sweep change** — tournaments stay disposable in BOTH tiers. All columns nullable/defaulted so free brackets are unaffected. Test: `brackets.paid.schema.db.test.ts`. |
| `supabase/migrations/20260905220235_bracket_hopper_roster.sql` | **Tournament PAID foundation — Phase C, Unit C1 (hopper + roster schema).** `bracket_hopper` (candidate pool: registered = `member_id` set, walk-up = `member_id` NULL + `display_name`; `status` hopper\|official; `paid_status`; `added_via` search\|link\|qr; `seed`; UNIQUE(bracket_id, member_id) — NULLs exempt so multiple walk-ups OK) + `bracket_roster` (organizer's sticky REGISTERED past players; UNIQUE(organizer, player)). Trigger `add_to_bracket_roster_on_admission` auto-adds a registered player to the roster on hopper→official (sticky; never removed on eject; walk-ups excluded). `bracket_hopper` on realtime + REPLICA IDENTITY FULL. Test: `bracket.hopper.db.test.ts`. |
| `supabase/migrations/20260905221350_bracket_hopper_read.sql` | **Tournament PAID foundation — Phase C, Unit C1 (hopper read RPC).** `get_bracket_hopper(bracket_id)` — SECURITY DEFINER, column-projected read of a bracket's candidates with registered players' member fields joined (nickname, player number, home) for display/disambiguation; walk-ups return null member fields. Authenticated-only (revoke anon; caller=created_by authz deferred to the RLS pass). |
| `supabase/migrations/20260905225243_bracket_join.sql` | **Tournament PAID foundation — Phase C, Unit C2 (self-add join).** Adds `brackets.join_token` (DB-generated uuid, backfilled, NOT NULL, UNIQUE — the QR/link token, DISTINCT from the anon view-only `share_token`) + `join_bracket_hopper(join_token, via)` SECURITY DEFINER RPC: the authenticated caller adds THEMSELVES to the hopper (member resolved from `auth.uid()`, never the token) while `status='setup'`; repeat join is a no-op (ON CONFLICT DO NOTHING). Returns `{ok, reason?, bracket_id?, bracket_name?}` (`not_found`/`not_accepting`/`not_signed_in`). Authenticated-only (revoke anon). Test: `bracket.join.db.test.ts`. |
| `supabase/migrations/20260906141649_bracket_roster_read.sql` | **Tournament PAID foundation — Phase C, Unit C3 (roster read RPC).** `get_bracket_roster(bracket_id)` — SECURITY DEFINER, column-projected read of the organizer's sticky **past players**, joined to `members` for nickname/number/home display. The organizer is derived from `brackets.created_by` (never passed in), and the result **excludes anyone already in this bracket's hopper** — that filter is what keeps the hopper screen's three groups duplicate-free and stops a one-tap add from hitting `UNIQUE(bracket_id, member_id)`. Unknown bracket → `[]`, never an error. Authenticated-only (revoke anon). Tests: `bracket.hopper.db.test.ts`. |
| `supabase/migrations/20260906150028_bracket_hopper_finalize.sql` | **Tournament PAID foundation — Phase C, Unit C3 (start-time conversion).** `finalize_bracket_hopper(bracket_id, include_waiting)` — the seam between the hopper (who might play) and the bracket (who IS playing): optionally admits everyone still waiting as unpaid, assigns contiguous seeds 1..N over the official list in arrival order, then replaces `bracket_participants` from it — carrying `member_id`, `display_name`, and `paid_status` → `entry_fee_paid` (COALESCEd, since that column is NOT NULL and an unflagged entry is simply not paid). Returns the count for the tree generator. Setup-only (a second call on a live bracket raises), refuses fewer than 2 players, and leaves **`start_bracket` untouched** — after this runs, the paid flow starts through exactly the same code path as the free one. Authenticated-only (revoke anon). Test: `bracket.finalize.db.test.ts`. |
| `supabase/migrations/20260906151140_bracket_walkup_roster.sql` | **Tournament PAID foundation — Phase C, Unit C3 (remembered walk-ups).** `bracket_walkup_roster` (organizer + typed name, UNIQUE per organizer on `lower(btrim(display_name))` so "Slim"/"slim" are one regular) + an admission trigger mirroring the registered-roster one — a walk-up is remembered when ADMITTED, never on eject. Rewrites `get_bracket_roster` to return registered members AND remembered walk-ups as ONE list (`member_id` set vs NULL), each half still excluding anyone already in this bracket's hopper — walk-ups matched by trimmed case-insensitive name, the only identity they have. **Creates no global member or placeholder**: a remembered walk-up is a private note in one organizer's list, so the walk-ups-are-tournament-scoped decision holds. Carries a **reserved, always-NULL `handicap` column** — free text, since the handicap SYSTEM is the `handicap_races` feature's decision; nothing writes it yet, it is there so remembering one costs no migration later. Test: `bracket.walkupRoster.db.test.ts`. |
| `supabase/migrations/20260906152223_bracket_roster_forget.sql` | **Tournament PAID foundation — Phase C, Unit C3 (roster housekeeping).** `forget_bracket_roster_entry(member_id, display_name)` removes ONE person from the calling organizer's remembered past players — a registered player by id, or a walk-up by case-insensitive name. The past-players list is sticky by design (an eject never removes anyone), so this is the deliberate way out; **single-target on purpose — no bulk clear**, the blast radius is wrong for a list built up over months. The organizer is resolved from `auth.uid()`, never passed in, which is the safety property while RLS is off. Returns false (not an error) when there was nothing to remove, so a double tap is harmless. Test: `bracket.rosterForget.db.test.ts`. |
| `supabase/migrations/20260906171456_bracket_hopper_unique_name.sql` | **Tournament PAID foundation — Phase C, Unit C3 (one name per tournament).** Unique index on `(bracket_id, lower(btrim(display_name)))` — first come, first served, case- and trim-insensitive. **Deliberately reverses** the earlier "walk-up de-dup is the organizer's judgment" position: that was right for the free tier (which has no hopper and where a human reads the bracket), and wrong once automation identifies a player by name — self-scoring, table alerts and handicap lookups all break identically on two Tim Ps. Enforced in the DATABASE because a UI check can't decide a race between two simultaneous joins. `join_bracket_hopper` gains a `name_taken` outcome (and an `already_in` no-op answered FIRST, so a re-scan never reports the player's own name as taken). Test: `bracket.join.db.test.ts`. |
| `supabase/migrations/20260906172524_bracket_player_view.sql` | **Tournament PAID foundation — Phase C, Unit C3 (player view).** `get_bracket_player_view(join_token)` — the THIRD read, because neither existing one fits: `get_bracket_hopper` carries the PII that lets an organizer tell same-name players apart (can't reach a poster link), and `get_bracket_share` reads `bracket_participants`, which don't exist until Start. Returns names-only waiting + official lists, the tournament rules, participants + matches once started, and **`me`** — the caller's own row resolved from `auth.uid()`, the ONLY place a paid flag appears. **Anon-callable on purpose** (a walk-up has no account); the projection is the authorization boundary, same posture as `get_bracket_share`. Tests: `bracket.join.db.test.ts`. |
| `supabase/migrations/20260906181438_bracket_walkup_self_add.sql` | **Tournament PAID foundation — Phase C, Unit C3 (walk-up self-add).** `add_self_as_walkup(join_token, display_name)` — an accountless player types a name on the tournament page and lands in the waiting room. **Deliberately opens an ANONYMOUS write**, reversing the authenticated-only hopper posture: the waiting room is a *proposal* the organizer must admit from, so abuse is tidying rather than corruption, and requiring an account would lose exactly the walk-in this feature exists for. Guards, all in the DB because an input box is bypassable: setup-only, `real_players` required, a **128-entry cap**, a **12-character** name limit, and one-name-per-tournament reported as `name_taken`. The length limit lives in the function, NOT on the column — organizer-typed names and registered nicknames write the same column and aren't bound by the self-entry box's rule. Tests: `bracket.join.db.test.ts`. |
| `supabase/migrations/20260906204115_my_tournaments.sql` | **Tournament PAID foundation — "tournaments I'm in".** `get_my_tournaments()` returns the tournaments the CALLER is playing in (from their `bracket_hopper` row), excluding closed ones and ones they created. Returns each **`join_token`**, because a player's home for a tournament is the join page — deliberately NOT the `share_token`, which is the spectator view. Member resolved from `auth.uid()`; an anonymous caller gets `[]` rather than an error. A walk-up can't appear here and can't be made to: there's no account to look them up by, so their only route back is the link their own browser remembered. Test: `bracket.myTournaments.db.test.ts`. |
| `supabase/migrations/20260906205201_organizer_add_registered.sql` | **Tournament PAID foundation — organizer search-add.** `add_registered_to_hopper(bracket_id, member_id)`. **An RPC rather than a client insert because the display name must be derived the SAME WAY `join_bracket_hopper` derives it** (nickname → full name → 'Player'): a name picked in the browser could differ from the one that player gets by scanning the code, and since a name may appear only once per tournament, that mismatch would surface as a phantom collision nobody could explain. Setup-only, registered-only (a placeholder belongs to a league), `already_in` as a no-op answered before the name check, and `name_taken` reported rather than a constraint error. Test: `bracket.organizerAdd.db.test.ts`. |
| `supabase/migrations/20260905172511_payment_methods.sql` | **Per-player card-on-file** (tournament paid foundation, Phase A). New `payment_methods` table: one saved card per row, **owned by `member_id`**, reusable for anything the player pays for (tournaments, dues, ...) — matches Stripe's model (charges reference a card by id). Cols: `stripe_customer_id`, `stripe_payment_method_id` (token), `card_last4`, `card_brand`, `nickname` (optional label, for >1 card later), `verified_at`, `is_default` (partial-unique: one default per member). Adds `brackets.payment_method_id` FK (ON DELETE SET NULL) = the card charged at Start. Mock today; Jack's Stripe install backs the token fields. The LO org card (`organizations`) is left as-is (future consolidation points it here). Test: `payment_methods.schema.db.test.ts`. |
| `supabase/migrations/20260910175307_races_sandbox.sql` | **RACE tables** — `races` (two players, a goal PER SIDE so a handicapped race is not a special case, break rule, game type, NO parent pointer), `race_games` (one row per game actually played — APPENDED, never pre-created), `race_confirmations` (many-eyes, mirrors `game_confirmations`) | `race_games` mirrors `match_games` **column-for-column ON PURPOSE** — `home_action`/`away_action`, `winner_player_id`, achievement flags, `winner_value`/`loser_value`, `confirmed_by_home`/`away`, `vacate_requested_by` — so the existing ScoringDialog / ConfirmationDialog / deriveDissents read a race game with NO shim, and a league can adopt races without a second scoring implementation. Carries a NULL `winner_team_id` because a league race DOES sit under two teams. Unique (race_id, game_number) makes a double-append impossible. REVOKE ALL + GRANT SELECT, not just the write verbs — the default grant to anon includes TRUNCATE |
| `supabase/migrations/20260910181344_race_engine.sql` | **RACE ENGINE** — `create_race` (the two players, a goal EACH, break rule, game type), `start_race` (name who breaks, write game 1), `record_race_game` (enter a result + vouch), `confirm_race_game` (the other player agrees → official → race finishes OR grows its next game), `vacate_race_game` (wipe a result, un-finish the race) | **A race is LINEAR** — both players are at the table the whole time, so games can't be played or entered out of order (the round-robin reason for out-of-order scoring doesn't exist here). A result goes only on the earliest UNSETTLED game; a wipe comes only off the LATEST game with a result. To fix game 3 of 5 you reverse 5, then 4, then 3, and vacate sweeps the trailing empty rows with it. That linearity is what keeps the score a plain count. The append lives SERVER-side inside the confirming transaction, under `FOR UPDATE` on the race row — both phones hit the gate at the same instant, so a client-side append would write two game 6s. If the opponent enters a DIFFERENT result instead of confirming, the first player's vouch is cleared (they never saw this result). All functions revoked from PUBLIC **and** `anon` (two separate default grants); `race_advance` revoked from `authenticated` too — it never checks who is calling |
| `src/race/types.ts` | Race row types + the `RaceSeat` / `RaceView` view-models | `RaceSeat` is the shape the room is HANDED — who the two people are, which side, what to call them, and their target — resolved by the HOST (a tournament from bracket seats, a league from a lineup + two teams). The room derives none of it |
| `src/race/useRaceData.ts` | Fetches races / race_games / race_confirmations + the realtime subscription | staleTime 0 + refetch on mount/reconnect, mirroring league live-scoring. Realtime is an ACCELERANT — any event refetches, payloads are never merged, so a missed message delays agreement rather than losing it |
| `src/race/useRaceView.ts` | Assembles everything the room renders from | Computes `activeGameNumber` (earliest unsettled — the only game a result may go on) and `vacatableGameNumber` (last with a result — the only one that may be reversed), mirroring the engine's linear rule. Only SETTLED games move the scoreboard |
| `src/race/useRaceActions.ts` | The four race RPCs as mutations + `useCreateRace` | Tables are SELECT-only for the browser, so this is a thin caller with no direct-write path to drift from. RPCs answer `{ok, reason}` rather than throwing — most refusals are ordinary situations, shown as sentences |
| `src/race/ScoreRace.tsx` | **The race room** — a COMPONENT that takes a race id | Not a page that finds one: it drops into a game room, a tournament, or a future league night unchanged. Reuses `ScoringDialog` + `ConfirmationDialog` as-is — a race collecting results its own way would be a second scoring implementation |
| `src/race/RaceGamesList.tsx` | The game rows — and where a player MEETS the linear rule | Reverse renders on exactly ONE row (the last game with a result). Tapping a name scores that game. Status by icon + words, never colour alone |
| `src/race/RaceScoreboard.tsx` | Two players, what each won, what each NEEDS | Shows BOTH targets always — in a handicapped race they differ (7 vs 4), and a single-target scoreboard would be quietly wrong for one player |
| `src/race/FirstBreakerPrompt.tsx` | The one question before a race starts: who breaks? | However it was decided at the table already happened; this writes it down. After it the break rule takes over and nobody is asked again |
| `src/race/ScoreRacePage.tsx` | Thin route wrapper — turns a URL into a race id | Keeping URL-reading HERE is what lets the room be hosted anywhere |
| `src/race/StartRacePage.tsx` | **DISPOSABLE** stand-in for starting a race | The real entrance is the game room, which calls the same `create_race`. Exists so a race can be played before that room is built |
| `src/race/__tests__/RaceGamesList.test.tsx` | Tests that Reverse appears on exactly one game | The button placement IS the rule as far as a player is concerned — on a middle game the server would refuse and they'd see an unexplainable failure |
