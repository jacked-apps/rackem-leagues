...
| `database/dev_relink_lo_login.sql` | DEV-ONLY: after cloning prod (which omits the Supabase-managed `auth` schema), repoint `members.user_id` to the LOCAL auth login owning the same email, so your login resolves to its member + org-staff roles. Idempotent. Run automatically at the end of `pnpm clone-prod`; this is the standalone manual version. |
