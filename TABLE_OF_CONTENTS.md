...
| `docs/plans/2026-09-09-002-league-race-walkthrough.md` | UNIT 0 OUTPUT for self-scoring — one 5v5 individual-race league night walked against the designed race BEFORE any migration exists, so what it finds is still free to change. **R32 passes**: the loop, confirm flow and three tables serve a league night with no second implementation | But the league can't HOLD that night — `game_generation` is SRR/DRR only and `game_count = lineup_size²`, so 5v5 gives 25 races, never 5; needs a third variant in the LOCKED Team Geometry doc. `prep_match` PRE-CREATES one `match_games` row per pairing, so a race fills one pairing row and team standings need zero change; `match_games.race_id` pointer, a fifth `forfeited` race status, and a widened `swap_player_in_lineup` cascade are the only additions |
| `/docs/plans/` | **CE implementation plans** | Output of `/compound-engineering:ce-plan` |
