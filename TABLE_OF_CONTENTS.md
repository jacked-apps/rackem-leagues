...
> **Last Updated**: 2026-09-19 (Game Room Unit 8 — the HOUSE model: seats belong to the host across every room they own (3 distinct guests present, dial `room_guest_seats()`), not to the room; `house_seats` is the only seat math; `RoomSeats` = `{devices, guests, seats, open}`; copy says "guest seats open". Migration `20260919182053_room_house_seats.sql`; 5 new db tests. Also today: websocket released after 2 min hidden; sweep daily 07:00 UTC.)
...
- `rooms.join.db.test.ts` - **Game Room / Units 1 + 8** — `join_room` (not_found / not_shared / rejoin same device / second device = new row); HOUSE seats: 3 distinct people fill the house, 4th `full` with the picture; owner's 2nd device free, guest's 2nd device = same person; guests in room A count against room B, same guest in both = one, 4th refused at either door; a host-friend is just a guest with their own house untouched; `is_host` = owns; aged-out guest frees their house seat; heartbeat refresh + room bump threshold; not_in_room; not_signed_in. Four test clients + a raw "stranger" row for the 4th guest; `beforeEach` empties the house
...
| `supabase/migrations/20260919182053_room_house_seats.sql` | **Game Room / Unit 8 — the HOUSE model.** Seats belong to the HOST, across every room they own: `room_guest_seats()` dial (3), `house_seats(owner)` = distinct people present in any of the owner's rooms (owner excluded) — the ONLY seat math; `room_seats(room)` reshaped to `{devices, guests, seats, open}`; `create_room` (is_host = owns), `join_room` (owner free; already-counted guest free; else house must have a seat; per-owner advisory lock), `set_room_shared` (no is_host rewrite) replaced | Closes the leak where one paid host could stand at 20 doors and leave 40 people on sockets; hosts no longer stack; room count irrelevant to cost → no cap needed. Ed 2026-09-19 |
