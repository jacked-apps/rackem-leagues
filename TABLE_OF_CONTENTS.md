...
> **Last Updated**: 2026-06-04 (Per-Game Allocator Room **Unit 6 SHIPPED**: workshop room UI. New page `/operator/scoring-workshop/per-game-allocator` (lazy-loaded, withOperator-gated) shows officials (read-only) + user's saved variations; clone-as-template + edit + delete flows. `SideEditor.tsx` exposes the four peer side-kinds (fixed / state-bag value / range / formula) with R11's state-bag read as a first-class choice. `saveTimeGuard.ts` is the first guard layer — runs `validatePerGameAllocator` AND a synthetic 5-game dry-run through `evaluatePointsSystem`; refuses to save anything that throws or NaN's. Dashboard link from OperatorDashboard.tsx via a Wrench-icon card. 7 new save-time-guard tests covering happy paths + Unit 3 args-shape rejection cases. Build green. Branch `feat/per-game-allocator-workshop`.)
