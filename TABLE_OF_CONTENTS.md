...
> **Last Updated**: 2026-09-16 (BRAINSTORMED the Game Room — `docs/brainstorms/2026-09-16-game-room-requirements.md`, superseding the 09-10 doc on branch feat/game-room. Decisions: games bring tables, room subscribes from a list; every game table has room_id cascade + realtime publication + full replica identity (else deletes never sync); presence = rows with heartbeat; seats derived, cap = members present × 4; one stored cell shared/not; wipe on new game; room is the foundation for tournament self-scoring; first tenant = two-phone coin flip. Interim host gate on members.role until Ed's per-hat designations table lands.)
