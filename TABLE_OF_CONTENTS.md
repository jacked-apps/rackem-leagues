...
> **Last Updated**: 2026-09-16 (**FIXED: a late entrant was locked out of their own tournament page.** `add_late_entry` wrote only `bracket_participants`, but every player-facing read goes through `bracket_hopper` — so the latecomer was invisible to their own phone, `JoinHopperPage` took that as "the organizer removed me" and wiped their stored name, and the re-add was refused because the bracket is live. It now writes an `official` hopper row, ADMITTING an existing never-admitted sign-up row rather than colliding with the table's two unique indexes, and keeping the player's own spelling because the page matches by exact string. New `name_waiting` refusal for a genuine collision with someone else's row.) Previously 2026-09-15: (FIXED the two db tests red on main since #280 made CI run them: `appendConfirmation.db.test.ts` built its `baseResult` fixture before `earlyEight` was added to `ConfirmationResult` (#276), so the insert wrote `undefined` → DB default `false`, and the exact-duplicate guard compared `false === undefined` → not a dup → a second identical vouch row. Test file is excluded from tsconfig, which is why typecheck never flagged the missing field; production callers are all typed and pass it. Also restored the `src/components/coinflip/` entries that fell out of the TOC when #279 merged, and trimmed the sandbox line's reference to the cut supplied-face prop.)
...
| `supabase/migrations/20260916144323_late_entry_writes_hopper_row.sql` | **FIX: a late entrant can see their own tournament.** `add_late_entry` wrote `bracket_participants` only, but `get_bracket_player_view` builds its lists from `bracket_hopper` — so a latecomer was invisible to their own phone, `JoinHopperPage` read that as "the organizer removed me" and WIPED their remembered name, and `add_self_as_walkup` then refused (`not_accepting` — the bracket is live). Playing in a tournament, locked out of its page | Now writes an `official` hopper row too. UPDATE-or-insert, not a plain insert: `bracket_hopper` carries unique indexes on (bracket_id, lower(btrim(display_name))) and (bracket_id, member_id) that the participants-only guards never had to satisfy — a never-admitted sign-up would have hit an unhandled unique violation. That person is the LIKELIEST latecomer, so their row is admitted rather than refused. A walk-up keeps THEIR OWN spelling (the page matches by exact string, so admitting "Rocket" as "rocket" locks them out anyway). Someone else's row → new `name_waiting` reason, distinct from `name_taken` |
