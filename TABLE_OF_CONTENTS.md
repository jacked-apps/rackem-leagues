...
> **Last Updated**: 2026-06-04 (Per-Game Allocator Room **Unit 4 SHIPPED**: runtime safety net wrapping the `evaluateAllocator` call in `runtime.ts` mirroring `fireTrigger`. A workshop-authored variation that throws mid-match is caught, logged with composition name + game index, and that game's points contribution is skipped — the W/L tick already happened above the catch, so it's preserved. The two ground rules (page renders, W/L recorded) now have a four-layer guarantee: save-time guard (Unit 6, pending), read-time validator (Unit 2/3, done), snapshot freeze (Unit 5, pending), runtime backstop (Unit 4, DONE). 4 new safety-net tests; 2,225 points-system tests pass total, no regressions. Branch `feat/per-game-allocator-workshop`.)
