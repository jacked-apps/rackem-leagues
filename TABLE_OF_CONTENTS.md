...
| `docs/brainstorms/2026-05-21-scoreboard-module-design-requirements.md` | ROUGH: scoreboard = slots per side filled by modules that read the state bag + render labeled values; LO-customizable; stress-tests the naming layer | Rough draft — partially superseded by `2026-06-03-scoreboard-module-requirements.md` |
| `docs/brainstorms/2026-06-03-scoreboard-module-requirements.md` | Phase-1 design building on May 21 rough draft: 8 spots per side in 3-2-3 layout, side-agnostic-mirror, `{curly_brace}` template syntax against state bag, label char limits, reader-never-recomputer with crash-wrapped render. Locked decisions captured; 8 open questions surfaced for Ed (variable-naming convention, fallback shape, global-var prefix, per-system default spot sets, format hints, etc.). Branch `feat/scoreboard-module-brainstorm` |
