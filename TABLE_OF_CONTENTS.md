...
> **Last Updated**: 2026-09-16 (**Individual races — schema, engine and room BUILT** on `feat/self-scoring-individual-races`, rebased onto main after #275 merged. A race is two players and a goal EACH (unequal goals = the handicapped case, not a special path), with games APPENDED one at a time — the next game is written only when every existing one is agreed and neither player has reached their number. **A race is LINEAR**: both people are at one table the whole time, so a result goes only on the earliest unsettled game and a wipe comes only off the LAST game with a result — to fix game 3 of 5 you reverse 5, then 4, then 3, and vacate sweeps the trailing empty rows with it. The append runs SERVER-side inside the confirming transaction under `FOR UPDATE`, because both phones hit the gate at the same instant and a client-side append would write two game 6s. `race_games` mirrors `match_games` COLUMN-FOR-COLUMN on purpose (incl. a NULL `winner_team_id`) so the existing `ScoringDialog` / `ConfirmationDialog` / `deriveDissents` read a race with no shim and a league can adopt races without a second scoring implementation. The room is a COMPONENT taking a race id — it drops into the coming game room, a tournament, or a league night unchanged. Gated `!isProduction` on the routes; `/race/new` is a DISPOSABLE stand-in for the game room, which calls the same `create_race`.) Previously 2026-09-15: (FIXED the two db tests red on main since #280 made CI run them: `appendConfirmation.db.test.ts` built its `baseResult` fixture before `earlyEight` was added to `ConfirmationResult` (#276), so the insert wrote `undefined` → DB default `false`, and the exact-duplicate guard compared `false === undefined` → not a dup → a second identical vouch row. Test file is excluded from tsconfig, which is why typecheck never flagged the missing field; production callers are all typed and pass it. Also restored the `src/components/coinflip/` entries that fell out of the TOC when #279 merged, and trimmed the sandbox line's reference to the cut supplied-face prop.)
