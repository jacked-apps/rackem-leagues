...
- `paid/ChargeBreakdownPanel.tsx` - **PAID C3** — a collapsed accordion on the checkout card: "2 features · $2.00", opening to a **receipt** — items left with amounts in a right-hand column, a rule, then **Total pushed over to sit beside its amount** (a full-width total row read as one more thing being charged). Tabular figures so the prices line up, and the total is set a step LARGER as well as bolder — weight alone is a weak signal at this size. **The $5 cap gets Subtotal + discount lines when it bites** — six features at $1 add to $6 while the button says $5, and a list that doesn't reconcile reads like a bug even when the cheaper number is the honest one; uncapped, a subtotal equal to the total is just noise, so it's omitted. Backed by pure `chargeBreakdown()` in `premiumFeatures.ts`, tested to always agree with `totalPriceCents`. Test: `ChargeBreakdownPanel.test.tsx`.
