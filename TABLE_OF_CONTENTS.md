...
| `supabase/migrations/20260904160417_bracket_tables.sql` | **Tournament bracket tool — Free Tier v1 (Unit 1).** Creates `brackets`, `bracket_participants`, `bracket_matches` (the match tree as pointer-columned rows: `next_match_id`/slot for the winner, `loser_next_match_id`/slot for double-elim drops). `share_token` is a non-enumerable `uuid`; child rows CASCADE on bracket delete; `member_id` on participants is a nullable PAID-AWARE hook (unused in v1). Both `brackets` + `bracket_matches` added to `supabase_realtime` with `REPLICA IDENTITY FULL`. Adds `get_bracket_share(uuid)` SECURITY DEFINER RPC (granted to anon) — the names-only public-read boundary while RLS is off, mirroring `get_team_join_view`. Test: `src/__tests__/database/brackets.schema.db.test.ts`. |
| `supabase/migrations/20260904160748_sweep_stale_brackets.sql` | **Bracket inactivity janitor — Free Tier v1 (Unit 7).** `sweep_stale_brackets(p_idle_days int default 7)` SECURITY DEFINER (authenticated-only; `REVOKE … FROM PUBLIC, anon`): hard-deletes (cascade) any bracket that is `status='closed'` OR idle past the threshold, returns the count. Called opportunistically at `createBracket` time (cleanup-on-create) — zero scheduling infra for the ephemeral tool. Test: `src/__tests__/database/brackets.sweep.db.test.ts`. |
| `supabase/migrations/20260904160517_bracket_rpcs.sql` | **Bracket write RPCs — Free Tier v1 (Unit 3).** Two SECURITY DEFINER functions, `authenticated`-only (Unit 6 hardening: `REVOKE EXECUTE … FROM PUBLIC, anon` — Postgres/Supabase auto-grant new functions to anon, so the write path was anon-callable; now denied since the public share route is a real anon surface): `start_bracket(uuid, jsonb)` persists the engine-generated match tree atomically (seed→participant + engine-key→match-uuid resolution) and flips the bracket to `live`; `advance_bracket_winner(uuid, uuid)` is the guarded advance — only acts when the match is still `ready`+unwon (concurrency/idempotency guard), propagates winner→next and loser→loser_next, activates/removes the conditional grand-final reset, readies newly-full matches, completes the bracket when the terminal resolves, and bumps `last_activity_at`. Returns whether it advanced. Test: `src/__tests__/database/brackets.lifecycle.db.test.ts`. |
