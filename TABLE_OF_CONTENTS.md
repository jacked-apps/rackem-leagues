...
| `supabase/migrations/20260517000002_messaging_phase1_unit19_editable_titles.sql` | **Messaging Phase 1 / Unit 19** — adds `conversations.title_user_edited_at TIMESTAMPTZ NULL`. Stamped to `NOW()` by the `updateConversationTitle` mutation whenever a captain renames their team chat. The future Unit 15 auto-rename trigger (entity name → chat title propagation) MUST check `title_user_edited_at IS NULL` before updating, so a captain's custom title is never overwritten by the auto-rename machinery. NULL = auto-managed (default); non-null = user-owned. Schema-only migration, no constraints. |
