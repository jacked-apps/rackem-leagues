...
> **Last Updated**: 2026-06-04 (Per-Game Allocator ROOM plan: `docs/plans/2026-06-04-002-feat-per-game-allocator-room-plan.md` — first room of the Scoring System Workshop building. 9 units: DB schema + officials + tamper trigger, loader, validator hardening, runtime safety net around allocator call, snapshot extension + live-path swap in `match-adapter.ts`, workshop room UI with save-time guard, league pick UI with apply-time preview, 17-Point smoke test through LIVE scoring path, TOC. Built against the foundational framing brainstorm. Branch `feat/per-game-allocator-workshop`.)
