...
- `brackets.paid.schema.db.test.ts` - **Tournament PAID foundation / Unit A1** — the paid columns on `brackets`: free-tier inserts unaffected (tier defaults free, premium_features empty), a paid tournament persists with premium features + game_type + a card on file, the tier CHECK rejects unknown values, the tier↔premium_features invariant (a premium feature on a free bracket is rejected / allowed when paid), and the inactivity sweep stays tier-agnostic (both tiers disposable). 6 tests.
...
| `supabase/migrations/20260905163726_bracket_paid_foundation.sql` | **Tournament PAID foundation — Phase A, Unit A1 (paid schema).** Adds to `brackets`: `tier` (free\|paid, CHECK), `premium_features` (text[], the organizer's checklist), `game_type`, and verify-at-setup payment fields (`payment_token`/`card_last4`/`card_brand`/`payment_verified_at`). A CHECK (`brackets_premium_implies_paid_check`) enforces the tier↔premium_features invariant (any feature checked ⇒ tier=paid). **No sweep change** — tournaments stay disposable in BOTH tiers (the "paid persists" idea was reversed; durable footprint = per-player setup + roster + run-count, later units). All columns nullable/defaulted so free brackets are unaffected. Test: `brackets.paid.schema.db.test.ts`. |
