...
- `CreateBracketFlow.tsx` - **Create-bracket flow orchestrator (Unit 4).** Lightweight linear flow (details → participants → review) for an authed organizer; on submit runs create → setParticipants → startBracket then navigates to `/brackets/:id`. On failure the bracket stays in `setup` (retryable) + toasts. No persistence (ephemeral). Test: `CreateBracketFlow.test.tsx`.
- `useCreateBracketForm.ts` - Local form-state hook for the create flow: fields (name/format/reset/seedingMode), participant add/remove/up-down-reorder, step nav, derived validation (name, 2–64 count, duplicate soft-warning). `MAX_PARTICIPANTS = 64`.
