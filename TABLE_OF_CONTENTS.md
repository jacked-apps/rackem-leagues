...
| `docs/plans/2026-09-04-001-feat-tournament-paid-foundation-plan.md` | Implementation plan for the Tournaments PAID-tier FOUNDATION (roadmap #1) — 10 units / 4 phases: tier gate ($0 `PaymentCardForm` checkout) + paid backbone (A), real identity + player-owned placeholders (B), roster/hopper/self-add (C), reusable setup + durable tagged results (D) | Origin `2026-09-04-tournament-paid-foundation-requirements.md`; builds on free-tier PR #264; key net-new = generalize org-scoped placeholder machinery to a player owner; kind derived from `user_id`; RLS/authorization deferred to pre-launch pass |
...
| `docs/brainstorms/2026-09-04-tournament-paid-foundation-requirements.md` | Requirements for the Tournaments PAID tier FOUNDATION (roadmap #1) — tier gate as a "Premium features" checklist + $0 checkout (reuse `PaymentCardForm`: verify-at-setup / charge-at-start), real players on `member_id`, organizer roster of "past players", QR/link/search self-add → hopper → official list (tap-name menu: eject/set-paid/add-unpaid), same-name via nickname+number, reusable saved setup, durable tagged results | Player-run (not org); goal=registered / fallback=full placeholder (player-owned); per-tournament pricing, first free; paid persists vs free purges; scoring/handicap/money/venue = forward hooks to their own brainstorms. Companion to the free-tier brainstorm (rides with PR #264) |
| `/docs/plans/` | **CE implementation plans** | Output of `/compound-engineering:ce-plan` |
