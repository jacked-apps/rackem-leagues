...
| `supabase/migrations/20260607000001_threshold_officials_seed.sql` | **Threshold workshop seed** — the REAL thresholds converted to the saveable shape (like the allocator seeds real allocators): Empty Starter (blank formula), **3v3 — Games to win** and **5v5 — Games to win** finish lines (`chart_lookup`, mirrored home_away, with the chart's rows EMBEDDED from the seeded global charts so a clone is self-contained and editable inline). DB test: `src/__tests__/database/threshold-officials-seed.db.test.ts`. |
