...
> **Last Updated**: 2026-05-17 (Messaging unread-announcement pinning + tappable mark-as-read badge. `ConversationList.tsx` pre-sorts so unread announcements render above all other active rows; the unread-count badge is now a tappable `role="button"` span that calls `useUpdateLastRead` and stops propagation so it doesn't also open the chat. New `__tests__/ConversationList.unread-pin-and-mark.test.tsx` (7 cases). On `messaging-unread-pin-and-mark-read` branch, stacked on `messaging-phase1-polish`.)
