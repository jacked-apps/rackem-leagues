...
- `rooms.sweep.db.test.ts` - **Game Room / Unit 2** — 25 h idle swept / 1 h kept; cascade takes phones + widgets and leaves other rooms; window is a dial (2 h survives default, not a 1-hour window); idempotent second call returns 0; `cron.job` has exactly one hourly `game-room-sweep`; not callable by clients
...
| `supabase/migrations/20260916184405_game_rooms_sweep.sql` | **Game Room / Unit 2** — `sweep_stale_rooms(p_idle_hours default 24)` deletes rooms whose `last_activity_at` is older than the window (cascade takes phones + game rows), returns the count; scheduled HOURLY by pg_cron as `game-room-sweep` (a 24 h window swept daily would let rooms live 48 h) | Keyed on state → idempotent + self-healing; revoked from anon AND authenticated (pg_cron runs as owner); mirrors the auto-forfeit sweep |
