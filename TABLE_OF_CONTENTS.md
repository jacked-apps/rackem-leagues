...
| `supabase/migrations/20260607000001_threshold_officials_seed.sql` | **Threshold workshop seed** — three read-only formula officials the LO clones: Empty Starter (single, const 0), Head start by handicap gap (home_away, mirrored `this_side_team_handicap - other_side_team_handicap`), Three-quarter mark (single milestone `game_count * 0.75`). Chart-view official lands with the chart editor. DB test: `src/__tests__/database/threshold-officials-seed.db.test.ts`. |
