...
- `DissentFlag.tsx` - **Many-eyes Layer-2 / Unit 5.** Calm, team-visible inline notice (shadcn `Alert` `warning` variant) shown on the dissenter's team when a recorded vouch differs from the official result. Names the dissenter + agreeing confirmers; never blocks scoring, never auto-changes a result — conversation prompt only; correction is vacate-and-rescore. UI copy lives in constants at the top of the file for easy iteration.
