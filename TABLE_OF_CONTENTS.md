...
> **Last Updated**: 2026-09-10 (REPAIRED the `db` test project — 11 failures across 8 files, now green and green twice running — and MADE CI RUN IT, which is the part that stops it recurring. None of the failures came from recent work: CI ran only the `unit` project, so these tests executed only when somebody ran them locally, and had been rotting unseen for months. Three causes. (1) SCHEMA DRIFT: `match_games.vacate_requested_by` became a varchar(4) side marker while the test still wrote a player uuid (22001); `messages` gained edit/delete time-limit triggers while the RLS test still edited the OLDEST seeded message (P0001) — impossible to pass. (2) FIXTURES THE SEED CANNOT HOLD: games are only written at lineup lock and messages only exist once somebody uses the app, so a seeded database has neither; four suites hunted for shapes that were never there. They now BUILD what they need — new `src/test/onboardingFixtures.ts` for placeholder-captain teams, and inline match/game and conversation/message fixtures elsewhere — and tear it down. (3) TESTS MUTATING SHARED SEED ROWS: members.rls rewrote a random real member’s email, which fires `ensure_placeholder_invite_token` and left a cancelled invite behind, poisoning that member so the NEXT run collided on `unique_pending_invite` (23505) — surfacing in whichever file touched it next, not the one at fault. Every one of these suites now owns its data. The pg_net push assertion counted total queue+response rows, which pg_net’s GC could cancel out; it now uses a high-water-mark id and inspects the request inside a rolled-back transaction. NEW `db-tests` job in checks.yml boots a trimmed Supabase (Postgres + Kong + PostgREST + Auth) and runs the project, plus a `pnpm test:db` script. It seeds from `database/dev_starting_point.sql`, the maintained dev seed — deliberately NOT `supabase/seed.sql`, which references dropped columns (`is_muted`, `team_format`) and fails partway, leaving a half-populated database; that file is removed by PR #278.)
