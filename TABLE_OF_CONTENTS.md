...
> **Last Updated**: 2026-06-03 (Added handicap-entry-module plan: `docs/plans/2026-06-03-002-feat-handicap-entry-module-plan.md` — Phase 1 module from the UI modularity audit. Replaces 14 of 18 lineup-side peeks at `handicap_type` with per-system `handicapEntry` config (inputKind, range, displayFormat, columnHeader, source). Locks Fargo-as-manual-entry-with-dials reframing. Branch `feat/handicap-entry-module`. Parallel work: LO Manual Scoring starting on second machine.)
...
| `docs/plans/2026-06-03-002-feat-handicap-entry-module-plan.md` | Implementation plan for the handicap-entry module (first Phase-1 module from the UI modularity audit) | 3 units; adds `handicapEntry: HandicapEntryModule` field to the existing HandicapSystem interface (inputKind, range, displayFormat, columnHeader, source dials); refactors HandicapCell, HandicapSummary, useHandicapCalculations, useLineupValidation, MatchLineup to read from the module instead of branching on handicap_type; locks Fargo-as-manual-entry-with-dials reframing (Fargo today = manual module with Fargo dials; future API-backed = same module, source='api'); origin 2026-06-03 UI-modularity-audit brainstorm; branch `feat/handicap-entry-module` |
