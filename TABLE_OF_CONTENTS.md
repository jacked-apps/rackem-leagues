...
> **Last Updated**: 2026-09-16 (BUILT Game Room Unit 1 — `supabase/migrations/20260916171612_game_rooms.sql`: rooms + room_phones, the plug-in contract check, seats/presence/heartbeat, host controls, reads; 28 db tests across four files via REAL authenticated clients. Two test-infra repairs surfaced by doing that: (1) `supabase/seed_test_users.sql` carried a fabricated bcrypt hash that never matched `test-password-123` — nobody had ever signed in from a db test; now hashed by pgcrypto at seed time. (2) `dbTestUtils.createTestClient` shared one localStorage session slot under jsdom, so the second signed-in client silently became the first user; now isolated per client.)
