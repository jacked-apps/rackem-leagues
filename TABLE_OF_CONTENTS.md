...
- `paid/JoinHopperPage.tsx` - **PAID C2** — the self-add landing page (`/brackets/join/:joinToken`) a player reaches by scanning a paid tournament's QR / opening its join link. Public route (auth handled inside): a signed-in player is added to the hopper once (via `useJoinHopper` → `join_bracket_hopper`, resolving THEM from the session, never the token); a cold scanner is asked to sign in first; renders joining / joined / closed / invalid states. **The cold scanner's sign-in link carries `?redirect` back to this page** — without it they sign in, land on the dashboard, and never join, so the code on the wall silently does nothing. Login validates it via `getSafeRedirectPath` (same-origin paths only). Test: `JoinHopperPage.test.tsx`.
