...
- `paid/HopperEntryMenu.tsx` - **PAID C3** — a tappable hopper row + its action menu. Waiting rows offer **Add as paid / Add as unpaid** (admit); official rows flip the paid flag; both offer **Remove** behind a two-click confirm whose copy comes from `removalConsequence`. Candidate rows show how the player arrived (Scanned in / Joined by link / Added by you).
