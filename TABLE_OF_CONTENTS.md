...
| `docs/brainstorms/2026-06-07-threshold-workshop-requirements.md` | Threshold Workshop — third Scoring System work room (after allocator + trigger). A threshold = one agnostic resolver `home+away → number` (state setter, generic `threshold_N` key, display-only label); two authoring views (formula OR chart) that compile to the same resolver; home/away mirror toggle; declares array/scalar, workshop validates. Reuses chart-editor UI + chart data layer from `origin/lo-manual-scoring` and `ExpressionBuilder` from the prior rooms; module-grain lesson from abandoned `origin/feat/threshold-math-modular` | Ready for planning → `/ce:plan`; salvage branches noted in frontmatter |
...
| `docs/plans/2026-06-07-001-feat-threshold-workshop-plan.md` | Implementation plan for the Threshold Workshop — third Scoring System work room (after allocator + trigger). 8 units in 2 phases: new `thresholds` table + never-throw loader; threshold-scoped `evaluate_expression` op (formula view) + generalized `chart_lookup` op + chart write-CRUD (chart view); room shell/list/route/home-card; guided editor (mirror toggle, input-side shaping, formula view); unified chart editor (cherry-pick + collapse 4→1 from `origin/lo-manual-scoring`); save-time guard (expectedSize drift-check + dry-run); seed officials + TOC. Library-authoring only — apply-to-league deferred to the assembly room. Reuses the existing `threshold_charts` data layer + the shared `ExpressionBuilder`. Origin: `2026-06-07-threshold-workshop-requirements.md`. Stacks on `feat/trigger-room` |
...
- `threshold/ChartView.tsx` - chart sub-view: an **inline editable lookup table**. The chart's rows live embedded in the threshold's own definition (clone-to-own, like the allocator embeds its config); edit cells / add / remove rows, pick the output column; emits a `chart_lookup` definition with the edited chart. Empty state lets you start from a global chart (copies its rows in).
- `threshold/BuiltInView.tsx` - read-only "built-in calculation" view: shows the ACTUAL source code that runs (via `builtinCode`) so the LO can see the tiny program — these are conditional programs (if/then), not one-line formulas. Read-only for now; an editable code editor (gated to technical users) is a later step.
- `threshold/builtinCode.ts` - `builtinCode(operationKind)` returns the real running source via `Function.prototype.toString()` (the deep tiny-program for the formula ops, the op `compute` for the rest). No hand-written representation, so it can't drift.
- `threshold/__tests__/BuiltInView.test.tsx` - 3 cases: 3v3 formula shows its real `midpoint`/`Number.isInteger` program; simpler built-ins show their compute; the view renders the Fargo code.
...
- `threshold-row-loader.ts` - **Threshold workshop — never-throw loader (Unit 1).** `loadThreshold(id)` reads a `thresholds` row, rebuilds the resolvable `ThresholdRow` from its stored `{operationKind, operationArgs}` via `buildThresholdRow` (registry = source of truth, no drift), returns a `LoadedThreshold` (row + label/description/expansionMode) or `null`.
- `__tests__/threshold-row-loader.test.ts` - 9 cases: happy path (metadata re-derived) + null description + not-found / supabase error / throw / malformed definition / unregistered op / bad expansion_mode.
...
- `operations/games-needed-formula.ts` - **Threshold workshop** — wires the byte-exact games-needed FORMULA charts as threshold ops: `games_needed_3v3_formula` (points) + `games_needed_5v5_formula` (percentage), size-agnostic. The formula counterpart to the size-locked chart.
- `operations/fargo-games-won.ts` - **Threshold workshop** — `fargo_games_won` op: derives games-to-win from lineup Fargo ratings (the Fargo win-threshold, alongside `fargo_start_points_for_side`). Never-throws.
- `operations/register-all.ts` - imports every threshold operation (side-effect registration); the workshop loader + save-time guard import it so any official's op is registered.
- `operations/__tests__/wired-formula-ops.test.ts` - 8 cases: 3v3 formula byte-exact (10/9/8 at gap 0), 5v5 formula, fargo-games-won, encodings, empty-lineup null, registration.
...
| `supabase/migrations/20260607000001_threshold_officials_seed.sql` | **Threshold workshop seed** — 8 learning templates, all HANDICAP-DERIVED (every real package threshold uses home+away handicaps), named encoding-first (points / percentage / fargo): games-to-win as a **chart** (editable, rows embedded) AND a **formula** (any size) for points + percentage; **Fargo start-points** AND **Fargo games-to-win**; the points lower-edge; and a blank Empty Starter. (No read-a-pref / prefs-milestone — those were stubs from percent-5-man, not real.) DB test: `src/__tests__/database/threshold-officials-seed.db.test.ts`. |
| `supabase/migrations/20260609000000_threshold_expected_handicap_type.sql` | **Threshold workshop — input declaration.** Adds `thresholds.expected_handicap_type` (points / percentage / fargo, nullable) — the author-set dial declaring WHAT the threshold takes in, which selects the input guard rail automatically (the author never writes a guard). Backfills the seeded officials with their real types. |
| `supabase/migrations/20260607000000_threshold_room.sql` | **Threshold workshop Unit 1** — `thresholds` table (id, `name` [generic resolvable key], `label` [display], description, scope, `author_id` FK → `members(id)`, `definition` JSONB `{operationKind, operationArgs}`, `expansion_mode` [single/home_away/per_pairing], timestamps) + CHECK constraints (scope, expansion_mode, author-required-for-user) + BEFORE UPDATE/DELETE tamper trigger on `scope='official'`. Operation metadata is re-derived at load via `buildThresholdRow` (not stored). Seeds deferred to Unit 8; library-authoring only. See `docs/plans/2026-06-07-001-feat-threshold-workshop-plan.md`. Schema test: `src/__tests__/database/threshold-room-schema.test.ts`. |
