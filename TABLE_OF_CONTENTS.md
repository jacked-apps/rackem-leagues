...
> **Last Updated**: 2026-09-19 (Game Room: `useRoomRealtime` releases the websocket after a tab has been hidden 2 min (`HIDDEN_RELEASE_MS`, = server presence grace) and reopens on return — a backgrounded DESKTOP tab no longer holds a Supabase connection until the 24 h sweep; phones already lost it to OS suspension. 3 tests.)
...
- `rooms.join.db.test.ts` - **Game Room / Unit 1** — `join_room` (not_found / not_shared / rejoin same device / second device = new row), seats (1 host → 4 devices, 5th `full`; host's 2nd device takes a seat, adds no allowance), presence ages out after the grace window, `room_heartbeat` bumps the room row only when stale
