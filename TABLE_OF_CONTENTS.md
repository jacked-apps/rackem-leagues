...
- `paid/hopperGroups.ts` - **PAID C3** — `buildHopperGroups(entries, roster)`: splits the hopper by `status` into the official + waiting groups (official in seed order, waiting in arrival order), resolves every row's display identity, and flags names shared across ANY of the three groups so the view insists on the disambiguators. Pure; the no-duplicates guarantee it depends on comes from `get_bracket_roster`. Test: `hopperGroups.test.ts`.
- `paid/HopperEntryMenu.tsx` - **PAID C3** — a tappable hopper row + its action menu. Waiting rows offer **Add as paid / Add as unpaid** (admit); official rows flip the paid flag; both offer **Remove** behind a two-click confirm whose copy says what survives (a registered player stays in Past players; a walk-up is gone). Candidate rows show how the player arrived (Scanned in / Joined by link / Added by you).
