...
- `paid/PlayerTournamentView.tsx` - **PAID C3** — the Players tab: where the viewer stands, who's in, who's waiting, and the rules. **Entry-fee status appears ONLY for the viewer's own entry** — the page is reachable from a code taped to a wall, so listing who still owes money would make it a debt board. Test: `PlayerTournamentView.test.tsx`.
