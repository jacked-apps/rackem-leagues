...
- `shadowAuditRunningTotals.ts` - **Strand-B cutover shadow audit** — runs the NEW modular Points System engine (`src/systems/points-system/match-adapter.ts`) alongside the legacy `computeMatchRunningTotals` on each scoring mutation, feeding the frozen prep snapshot (snapshotted threshold columns + LOCKED `match_lineups` ratings/team-bonus), and logs any divergence. Pure observation: never writes the match row, never throws, never blocks. Removed at the flip.
