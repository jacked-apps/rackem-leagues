...
> **Last Updated**: 2026-05-17 (Messaging smoothness pass — 6 polish items in one bundle on `messaging-smoothness-pass` branch, stacked on `messaging-unread-pin-and-mark-read`. (1)+(2) "You: ..." / "Jack: ..." preview prefixes via new denormalized `conversations.last_message_sender_id` column + updated trigger + backfill migration `20260517000005_messaging_smoothness_last_message_sender.sql`. (3) Browser tab title shows unread count via new `src/hooks/useDocumentTitleUnread.ts` + `DocumentTitleUnreadSyncer` mounted in App.tsx. (4) Smart auto-scroll in MessageList — only force-scrolls if user was near bottom OR sent the message themselves; new `src/utils/scrollHelpers.ts` helpers. (5) "Unread messages" divider in MessageList above first message newer than the chat-open snapshot of `currentUserLastRead` (captured in a ref by MessageView so the mark-as-read mutation doesn't erase the anchor). (6) Right-click / 500ms-long-press context menu on message bubbles via new `src/components/messages/messageview/BubbleContextMenu.tsx` (Popover + Copy item only for now). New tests: ConversationList.preview-prefix (8), MessageList.unread-divider (4), scrollHelpers (8). Full suite green: 82/82.)
...
- `messaging-phase1-unit15-auto-rename.rls.test.ts` - **Messaging Phase 1 / Unit 15** — DB-backed coverage of the two auto-rename triggers. (1) `teams.team_name` rename propagates to the matching team_chat title; non-name UPDATEs don't fire the trigger; user-edited titles (title_user_edited_at IS NOT NULL) are preserved. (2) `leagues.division` rename propagates to every captains_chat in every season of that league; NULL division falls back to day_of_week; user-edited captains_chat titles are preserved. Restores all mutated state in `afterAll` so the test is repeatable.
- `messaging-phase1-unit14-season-end.rls.test.ts` - **Messaging Phase 1 / Unit 14** — DB-backed coverage of the season-end `cannot_leave` release trigger: precondition that captain has `cannot_leave=TRUE` on team_chat for an active season, flipping season to `completed` releases the flag on both team_chat AND captains_chat for that captain, other seasons untouched, non-completion status changes (e.g. `active → cancelled`) don't fire, idempotent re-fire on already-completed season is a safe no-op. Restores original status in `afterAll`. 5 cases.
...
- `__tests__/ConversationList.unread-pin-and-mark.test.tsx` - **Messaging Phase 1 / Unread-pin + mark-as-read** — RTL test for two related contracts: (1) unread announcement rows pin above more-recent non-announcement chats; a *read* announcement sorts by recency like everything else. (2) the unread-count badge is a tappable mark-as-read button — clicking it calls `useUpdateLastRead` with `{conversationId, userId}`, does NOT also open the conversation (stopPropagation), the row body still opens the chat, the badge does not render when unreadCount is 0 or for past-member rows. 7 cases.
- `__tests__/ConversationList.preview-prefix.test.tsx` - **Messaging smoothness pass / preview-prefix** — RTL test for the iMessage-style "You: " / "Jack: " preview prefix in the conversation list. 8 cases: "You: " when current user sent; "Jack: " in group chats when other sent; no prefix in DMs when other sent (row title already names them); "You: " in DMs when current user sent; no prefix for system messages (sender NULL); no prefix when sender name missing (defensive); "No messages yet" with no prefix when preview is null; "Jack: " also applies to announcements channels.
...
- `messageview/MessageList.tsx` - Scrollable message list extracted from `MessageView`. Owns the `Message` interface (exported), loading + empty states, **smart auto-scroll** (only force-scrolls when user was near bottom OR sent the message themselves; uses `scrollHelpers.ts` to find the scroll parent without coupling to MessageView), the system-message branch (`is_system → MessageBubble isSystem`), the Unit 8 inline-failed-send rendering, AND the smoothness-pass "Unread messages" divider above the first message newer than `unreadAnchorAt`. Each non-system bubble is wrapped in `<BubbleContextMenu>` so right-click / long-press surfaces a Copy action.
- `messageview/useOutgoingMessages.ts` - Local-state hook for optimistic outgoing messages (Unit 8 inline pattern). Tracks `{ clientId, content, status: 'sending' | 'failed', errorMessage?, createdAt }` entries; exposes `addPending` / `markPending` / `markFailed` / `remove` / `removeByMatch`. Used by `MessageView` to thread an optimistic bubble into `MessageList` and convert it to a retryable failed bubble when the mutation rejects.
- `messageview/BubbleContextMenu.tsx` - **Messaging smoothness pass** — wraps a message bubble in a shadcn `Popover` that opens on right-click (`onContextMenu` preventDefault) OR a 500ms long-press (`onPointerDown` timer cancelled by move/up). Single "Copy" item copies bubble content to the clipboard and toasts "Copied" (or "Copy failed" on clipboard-API errors). Designed to grow — Reply / React / Edit / Delete slot in here later without touching `MessageBubble`. System bubbles intentionally NOT wrapped (no value in copying narration like "Jack joined the team").
- `messageview/__tests__/MessageList.unread-divider.test.tsx` - **Messaging smoothness pass / unread-divider** — RTL test for the "Unread messages" divider: appears above the first message newer than the anchor; absent when anchor is null; absent when no message is newer than the anchor; appears at the very top when all messages are newer. 4 cases.
...
- `useDocumentTitleUnread.ts` - **Messaging smoothness pass** — syncs `document.title` with the current user's unread message count. `useDocumentTitleUnread()` hook reads `useCurrentMember` + `useUnreadMessageCount` (polling, not realtime) and prefixes `(N) ` to the captured baseline title when N > 0. Captures baseline once on first effect so it doesn't compound on every poll. `<DocumentTitleUnreadSyncer />` mount-only component wraps the hook for use near the app root. Mounted inside `UserProvider` in `App.tsx` so the hook can resolve the current user, outside the router so it doesn't remount on navigation.
...
- `scrollHelpers.ts` - **Messaging smoothness pass** — DOM helpers for MessageList's smart-auto-scroll: `findScrollParent(el)` walks up to the nearest ancestor with `overflow-y: auto|scroll` (so MessageList can find its scroll container without coupling to MessageView via a prop); `isNearBottom(el, threshold=150)` returns whether a scroll container is within `threshold` px of its bottom (drives the "don't yank the user mid-scroll-up" decision).
- `__tests__/scrollHelpers.test.ts` - **scroll-helper tests** (8 cases, jsdom env) — `isNearBottom` at bottom / inside default threshold / outside / honors custom threshold; `findScrollParent` for `overflow-y: auto` ancestor / `overflow-y: scroll` ancestor / no-scrollable-ancestor / null input.
...
| `supabase/migrations/20260517000001_messaging_phase1_unit18_short_titles.sql` | **Messaging Phase 1 / Unit 18** — shortens auto-managed chat titles for mobile readability and moves the long org/league context name into the read-only banner. New title patterns: `team_chat` = `<team.name>` (drop "— Team Chat"), `captains_chat` = `"Captains — <division|day_of_week>"` (reorder; drop "Chat"), `season_announcements` = `"League Announcements"` (universal), `org_announcements` = `"Global Announcements"` (universal). Two parts in one migration: (1) retroactive UPDATE on existing auto-managed conversation rows so dev DBs match without a `db:reset`; (2) CREATE OR REPLACE `auto_create_season_conversations` with the new patterns so all future season activations use them. Matching JS helpers in `src/api/mutations/autoConversations.ts` updated in the same commit. |
| `supabase/migrations/20260517000002_messaging_phase1_unit19_editable_titles.sql` | **Messaging Phase 1 / Unit 19** — adds `conversations.title_user_edited_at TIMESTAMPTZ NULL`. Stamped to `NOW()` by the `updateConversationTitle` mutation whenever a captain renames their team chat. The Unit 15 auto-rename triggers (entity name → chat title propagation) check `title_user_edited_at IS NULL` before updating, so a captain's custom title is never overwritten by the auto-rename machinery. NULL = auto-managed (default); non-null = user-owned. Schema-only migration, no constraints. |
| `supabase/migrations/20260517000003_messaging_phase1_unit15_auto_rename_propagation.sql` | **Messaging Phase 1 / Unit 15** — two `AFTER UPDATE` triggers that propagate entity renames to matching chat titles. (1) `teams.team_name UPDATE` → updates the team's `team_chat.title` to the new name. (2) `leagues.division/day_of_week UPDATE` → updates `"Captains — <division\|day>"` on every `captains_chat` belonging to that league's seasons (joins through seasons; one league can have multiple captains chats, one per season). Both triggers skip rows where `title_user_edited_at IS NOT NULL` per Unit 19. `SECURITY DEFINER`, explicit `search_path`, REVOKE PUBLIC/authenticated — same security shape as Unit 5 triggers. Org/season renames were originally in scope but became moot after Unit 18 made the announcement titles universal ("Global Announcements" / "League Announcements" — no embedded entity name to update). |
| `supabase/migrations/20260517000004_messaging_phase1_unit14_season_end_release_cannot_leave.sql` | **Messaging Phase 1 / Unit 14** — mirror of Unit 4's activation trigger. `AFTER UPDATE OF status ON seasons WHEN (OLD.status = 'active' AND NEW.status = 'completed')` → flips `cannot_leave=FALSE` for every participant in this season's team chats AND captains chat. Chats themselves stay intact; the change just lets captains (and anyone else with `cannot_leave=TRUE`) leave / archive the chat now that the season is over. Single UPDATE wrapped in `SECURITY DEFINER`, same security shape as Unit 5 triggers. Idempotent re-fire. |
| `supabase/migrations/20260517000005_messaging_smoothness_last_message_sender.sql` | **Messaging smoothness pass** — adds denormalized `conversations.last_message_sender_id UUID REFERENCES members(id) ON DELETE SET NULL` so the conversation-list UI can render iMessage-style "You: ..." / "Jack: ..." preview prefixes without an N+1 lookup against `messages`. Updates the existing `update_conversation_last_message` (INSERT) and `update_conversation_on_message_delete` (soft-delete) triggers to maintain the column alongside the existing `last_message_preview` / `last_message_at`. Idempotent backfill via `UPDATE conversations SET last_message_sender_id = m.sender_id FROM (SELECT DISTINCT ON (conversation_id) … FROM messages WHERE is_deleted=FALSE ORDER BY conversation_id, created_at DESC) m`. System messages keep NULL (no human sender; UI treats NULL as "no prefix"). |
