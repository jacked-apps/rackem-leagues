...
- `AppDrawer.tsx` - Slide-in drawer with secondary nav (profile, settings, operator-org switcher, sign-out) + a **My Match section** pinned at the TOP: small count chips ("Live N" / "Makeup N") that reveal each group's list on tap (Live auto-opens when there's a live/tonight match; otherwise collapsed to just chips). Drawer is the home for nav items that don't fit on the sidebar/tab bar.
