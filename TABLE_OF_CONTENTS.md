...
> **Last Updated**: 2026-06-04 (Per-Game Allocator Room **Unit 5 SHIPPED**: live-path swap + snapshot freeze. `ResolvedSystemConfig` gains `per_game_allocator_id` + optional `per_game_allocator` (resolved object). `populateMatchSnapshotIfNeeded` loads the variation at match-start and embeds the resolved object — historical replay is stable (R9). `match-adapter.ts` `buildComposition` accepts an optional override that replaces the prepackaged composition's allocator slot (triggers untouched, name suffixed `__custom_<name>`). `engineRunningTotals.ts` threads it through. `updateMatchRunningTotals` reads the snapshot's embedded variation and passes it down. `pickPointsSystem` parity swap kept consistent. Preset fast path skipped when override is present. 7 new tests in snapshot-and-swap.test.ts including a 17-point variation that exercises the formula path end-to-end. 2,449 points+systems tests pass; build green. Branch `feat/per-game-allocator-workshop`.)
...
| `supabase/migrations/20260604000000_per_game_allocator_room.sql` | **Scoring System Workshop — Per-Game Allocator Room Unit 1** — `per_game_allocators` table (id, name, description, scope, author_id, winner_side JSONB, loser_side JSONB, timestamps), CHECK constraints for scope + author_id pairing, BEFORE UPDATE/DELETE tamper trigger blocking modification of `scope='official'` rows, `preferences.per_game_allocator_id` UUID FK (ON DELETE RESTRICT), extended `resolved_league_preferences` view with the new COALESCE'd column, seeded 4 official rows (Percent 5-Man, 10-Point, 17-Point, Empty Starter) byte-equivalent to the current TS factory defaults. See `docs/plans/2026-06-04-002-feat-per-game-allocator-room-plan.md`. |
