...
> **Last Updated**: 2026-09-19 (Game Room Unit 8 — the HOUSE model: seats belong to the host across every room they own, and A SEAT IS A SCREEN (one present phone row = one websocket, the host's own included — "if they count it we count it"); 4 seats, dial `room_house_seats()`; `house_seats` is the only seat math; `RoomSeats` = `{devices, used, seats, open}`; copy says "N seats open". Migration `20260919182053_room_house_seats.sql`; 6 house db tests. Also today: websocket released after 2 min hidden; sweep daily 07:00 UTC.)
...
| `docs/brainstorms/2026-09-10-game-room-requirements.md` | SUPERSEDED (2026-09-16) — the first game-room brainstorm: two members, challenge-from-profile, event log with room-enforced per-event confirmation, server-decided outcomes | Kept for the reasoning; three ideas carried into the 09-16 doc |
| `docs/brainstorms/2026-09-16-game-room-requirements.md` | REQUIREMENTS for the **Game Room** — a generic, perishable, multi-phone container that games plug into. Games bring their own tables (each with `room_id` cascade + published + full replica identity); the room subscribes from a list on the room row and invalidates `['room', roomId, table]`; presence is rows with a heartbeat; seats derived from members present × 4; free = same room with the door shut (no channel); wipe on new game; room is the future foundation for tournament self-scoring (R4 is a fence, not a wall). First tenant: the two-phone coin flip. Reviewed by 4 personas 2026-09-16 | Interim host gate = `members.role` in (league_operator, developer); Ed is designing a one-row-per-hat designations table separately |
| `docs/plans/2026-09-16-001-feat-game-room-plan.md` | PLAN for the Game Room (Deep, 7 units in 4 phases): A) `rooms` + `room_phones` schema/RPCs/publication + pg_cron sweep; B) data layer + guest identity (Supabase anonymous sign-in, `isLoggedIn` excludes anonymous), `useRoomRealtime` over the table list, room screens + game registry; C) two-phone coin flip (server-decided face via `throw_room_coin`, frozen call via `call_room_coin`, controlled `CoinFlip` props); D) non-prod gate + doors + docs. Reviewed by 3 personas 2026-09-16, 44 fixes applied; then guests changed to registered-members-only (no anonymous sessions) | Acceptance boundary: room = `src/rooms/**` minus `games/**` + the RPCs; Ed's race plugs in after with zero room changes |
| `/docs/plans/` | **CE implementation plans** | Output of `/compound-engineering:ce-plan` |
...
- `rooms.schema.db.test.ts` - **Game Room / Unit 1** — publication + replica identity for `rooms`/`room_phones`, cascade + unique (room, device), every branch of `room_validate_tables` (names what is missing), anon has no EXECUTE on any room RPC and authenticated has none on the helpers
- `rooms.create.db.test.ts` - **Game Room / Unit 1** — `create_room` via REAL authenticated clients: player → free room ok / shared → `not_a_host`; operator → shared ok + `is_host`; every bad-table reason; nothing written on refusal; raw SQL → `not_signed_in`
- `rooms.join.db.test.ts` - **Game Room / Units 1 + 8** — `join_room` (not_found / not_shared / rejoin same device / second device = new row); HOUSE seats (a seat = a SCREEN, host's included): host + 3 screens fill the 4-seat house, 5th `full` with the picture; host's own 2nd–4th screens are seats ("thirty tablets get no pass"); guest's 2nd screen = 2nd seat, same screen re-opening = free; screens in room A count against room B, ONE device in both rooms = TWO seats, 5th refused at either door; a host-friend is just a screen with their own house untouched; `is_host` = owns; aged-out screen frees its seat; heartbeat refresh + room bump threshold; not_in_room; not_signed_in. Four test clients + a raw "stranger" row; `beforeEach` empties the house
- `rooms.host.db.test.ts` - **Game Room / Unit 1** — `set_room_game` wipes exactly this room's rows + a refused switch leaves the old game intact; `set_room_settings` no wipe; host-only guards; `set_room_shared` opens the door in place; `close_room` cascades and reads say `{found:false}`; `get_room` ≡ `get_room_by_token`
- `rooms.sweep.db.test.ts` - **Game Room / Unit 2** — 25 h idle swept / 1 h kept; cascade takes phones + widgets and leaves other rooms; window is a dial (2 h survives default, not a 1-hour window); idempotent second call returns 0; `cron.job` has exactly one daily (07:00 UTC) `game-room-sweep`; not callable by clients
- `roomCoinFlips.db.test.ts` - **Game Room / Unit 6** — the flip's trust boundary on the real DB: table passes the plug-in contract (`create_room` accepts it; published; replica full); call → throw sets heads|tails ONCE (second throw `already_thrown`); throw before call `no_call`; caller throwing `not_flipper`; flipper calling `not_caller`; `bad_call`; call changeable until the throw, frozen after; start refuses `same_phone` / `phone_not_in_room` / `not_in_flip`; room delete cascades flips; anon revoked
- `rooms.queries.db.test.ts` - **Game Room / Unit 3** — the client query functions against the real DB: `getMyRooms` joined select (once per room, newest first, host flag), `getRoom`/`getRoomByToken` → state, then `null` after close
- `roomsFixtures.ts` - Shared fixtures for the room tests: `room_test_widgets` (meets the whole plug-in contract) + four tables that each break one rule; `rpc()` / `createRoomAs()` helpers; not a test file
...
- `dbTestUtils.ts` - Supabase/pg clients + `executeSql` for the `db` test project. Test clients are session-isolated (`persistSession:false` + a per-user `storageKey`) — under jsdom two clients otherwise share one localStorage slot and the second sign-in silently overwrites the first
...
#### Game Room (`/rooms/`) — Units 3+ (the room the games plug into)
- `deviceId.ts` - Mint-once per-browser id in localStorage (`game-room:device-id`) — a SEAT is a DEVICE because Supabase bills per socket; a refresh or second tab is the same device, a tablet is a new one. Throwing storage → one stable id for the page load
...
- `useRoomRealtime.ts` - ONE channel per device over the room's table list. Tab hidden past `HIDDEN_RELEASE_MS` (2 min = server presence grace) → channel released, reopened on return (a backgrounded desktop tab stops holding a connection; a visible screen is never touched): `rooms` (id filter) + `room_phones` (room_id) → invalidate `rooms.detail` EXACTLY (phones live inside it); each listed game table → its own `rooms.table` key. Drops heartbeat-only UPDATEs. EVERY SUBSCRIBED (first included) invalidates the detail PREFIX (realtime never replays). `rooms` DELETE → `roomGone`. Table list change = teardown + `removeQueries` old table keys + rebuild. `shared=false` = no socket, status `live`. Status via `classifySubscribeEvent`
- `useRoomRealtime.test.ts` - 14 tests on a fake channel + real QueryClient: bindings/filters, per-table invalidation, heartbeat noise dropped, `roomGone`, table-list rebuild (cache dropped, no rebuild on same list), every-SUBSCRIBED prefix invalidation, binding-mismatch → `error`; hidden past the window releases + return re-subscribes and refetches, a short hide or a visible idle tab never touches the channel
...
- `games/registry.test.ts` - Unknown key → undefined + fallback name; every entry honours the contract (1–3 unique, non-reserved tables; Play + isReady present; a game with no Setup is always ready)

##### Coin flip tenant (`/rooms/games/coinflip/`) — Unit 6, the first game
- `definition.ts` - `COIN_FLIP: GameDefinition` — key `coin_flip`, table `room_coin_flips`, `Play = RoomCoinFlip`, no `Setup` (who calls is per flip), `isReady` always true. The registry entry
- `RoomCoinFlip.tsx` - The `Play` screen: latest flip row → controlled `<CoinFlip>` (viewer = my phone; caller/flipper = the row's phones; call/face = the record). Taps → RPCs; the room's channel brings the row back to both phones. No row / "Flip again" → `CoinFlipSetup`. `key={flip.id}` remounts per flip
- `RoomCoinFlip.test.tsx` - No row → setup with "they call, I throw" default + swap; alone → door copy; caller phone gets heads/tails → `call_room_coin`, flipper waits; flipper gets throw → `throw_room_coin`, caller waits; a face → same winner on both phones; Flip again → panel until a new row
- `CoinFlipSetup.tsx` - Pick the other phone (RadioGroup; `away` tagged), "I call, they throw" switch, "Flip for it". Alone in the room → "A flip takes two phones" pointer at the door
- `coinFlipApi.ts` - The game's own reads/writes (the room never imports it): `getLatestRoomFlip` (from `room_coin_flips`), `startRoomCoinFlip` / `callRoomCoin` / `throwRoomCoin` RPC wrappers, `CoinFlipRefusal` union, `COIN_FLIP_TABLE`
- `useRoomCoinFlip.ts` - TanStack hooks keyed with `queryKeys.rooms.table(roomId, 'room_coin_flips')` — the one rule that makes a game live in a room. Writes invalidate the same key
- `coinFlipRefusalCopy.ts` - Every flip refusal as a sentence (mostly races: "The coin has already landed.")
...
- `CoinFlip.tsx` - The reusable coin flip component: two participants in, one winner out. Two modes on one state machine (idle → calling|assigned → flipping → result): CALLED (one player calls, the OTHER throws) and QUICK (app assigns faces, shows the assignment as its own beat, then flips). Winner announced by name, never by color alone. CONTROLLED path (Game Room Unit 6): `controlled={{ call, face, onCall, onThrow, onFlipAgain? }}` renders FROM an external record — phase derived from the record, taps reported up, face never tossed locally, no shuffle, random never consulted; mounting with a face shows the result at once, a face ARRIVING spins. Without `controlled`, unchanged
...
- `CoinFlip.test.tsx` - Component tests across both modes and both roles; fake timers with fireEvent (userEvent deadlocks under fake timers); + 7 controlled-path tests (record → phase, taps reported up, random untouched, same winner on both devices, Flip again → new record)
...
- `api/queries/rooms.ts` - **Game Room / Unit 3 reads.** `getRoom(id)` / `getRoomByToken(token)` → the `room_state` jsonb (row + derived seats + phones with SERVER-computed `is_present`) or `null` once gone (`{found:false}` is never an error); `getMyRooms(memberId)` — rooms the member has a device in, one entry per room, via `rooms` ⟵ `room_phones!inner` (proven against the real DB in `rooms.queries.db.test.ts`)
- `api/mutations/rooms.ts` - **Game Room / Unit 3 writes.** Thin wrappers over the room RPCs: `createRoom`, `joinRoom`, `roomHeartbeat`, `setRoomGame`, `setRoomSettings`, `setRoomShared`, `closeRoom`. Results are `{ok:true,…} | RoomRefused` with a typed `reason` union so pages can phrase every refusal; only transport throws
- `api/hooks/useRooms.ts` - **Game Room / Unit 3 hooks.** `useRoom` (polls every 15 s as the realtime FALLBACK while the room exists, stops at `null`), `useRoomByToken`, `useMyRooms`, and the write hooks, each invalidating `rooms.detail(id)` — the PREFIX of every game-table key
- `api/hooks/useRooms.test.ts` - Wiring tests: enabled gate, poll on/off, refusal returned as data, invalidation of the room prefix
...
| `supabase/migrations/20260916171612_game_rooms.sql` | **Game Room / Unit 1** — `rooms` + `room_phones` (one row per DEVICE; a seat is a socket), both published + REPLICA IDENTITY FULL; dials as functions (`room_table_cap` 3, `room_seats_per_host` 4, `room_presence_grace` 2 min, `room_activity_threshold` 5 min); `room_validate_tables` (the plug-in contract check: reserved? exists? uuid room_id? cascade FK to rooms? published? full replica?); `room_seats` (distinct hosts present × 4 − devices present); RPCs `create_room` / `join_room` (FOR UPDATE + seat check, `not_shared` on free rooms) / `room_heartbeat` / `set_room_game` (validate → WIPE old tables by room_id → update) / `set_room_settings` (no wipe) / `set_room_shared` / `close_room` (cascade) / `get_room` / `get_room_by_token` (`{found:false}` once gone). Interim host gate `room_member_is_host` = role in (league_operator, developer) | Every caller resolved auth.uid() → members; nothing granted to anon; helpers revoked from authenticated too |
| `supabase/migrations/20260916184405_game_rooms_sweep.sql` | **Game Room / Unit 2** — `sweep_stale_rooms(p_idle_hours default 24)` deletes rooms whose `last_activity_at` is older than the window (cascade takes phones + game rows), returns the count; scheduled DAILY at 07:00 UTC by pg_cron as `game-room-sweep`, an hour after `auto-forfeit-sweep` — with the 24 h window a room lives 24–48 h after the last phone leaves (Ed, 2026-09-19: tidying, not a deadline; was hourly) | Keyed on state → idempotent + self-healing; revoked from anon AND authenticated (pg_cron runs as owner); mirrors the auto-forfeit sweep |
| `supabase/migrations/20260916200041_room_coin_flips.sql` | **Game Room / Unit 6** — the first tenant table `room_coin_flips` (room_id cascade, caller/flipper phone FKs cascade, `call`/`face` heads|tails checks, two-phones check; published + REPLICA IDENTITY FULL) + RPCs: `start_room_coin_flip(room, caller_phone, flipper_phone)` (caller must be one of them), `call_room_coin(flip, call)` (caller phone only; changeable until thrown, then `already_thrown`), `throw_room_coin(flip)` (flipper phone only; needs a call; DATABASE picks the face from `random()` exactly once) | The face is never client-supplied — two phones cannot disagree. `room_phone_is_mine` helper revoked from everyone; RPCs authenticated-only |
| `supabase/migrations/20260919182053_room_house_seats.sql` | **Game Room / Unit 8 — the HOUSE model.** Seats belong to the HOST across every room they own, and A SEAT IS A SCREEN: `room_house_seats()` dial (4), `house_seats(owner)` = present phone rows (one row = one websocket, the owner's own included) across all the owner's rooms — the ONLY seat math; `room_seats(room)` reshaped to `{devices, used, seats, open}`; `create_room` (is_host = owns), `join_room` (same device same room = refresh; every other screen needs a seat; per-owner advisory lock), `set_room_shared` (no is_host rewrite) replaced | Closes the leak where one paid host could stand at 20 doors and leave 40 people on sockets, or hand 30 tablets to a league; hosts no longer stack; room count irrelevant to cost → no cap. Ed 2026-09-19: "if they count it we should count it" |
| `supabase/seed.sql` | Full local dev DB dump — auto-applied on `supabase db reset`. **Local only, never runs against production.** |
| `supabase/seed_test_users.sql` | 4 synthetic test auth users (player/operator/captain/owner; password hashed by pgcrypto at seed time — password `test-password-123`). **Dev-only — run manually via `docker exec ... psql`.** |
