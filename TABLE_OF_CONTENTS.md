...
- `paid/AddMyNameCard.tsx` - **PAID C3** — the two doors for a player with no account: type a name (up to 12 characters) or sign in. The typed-name door is the point of the feature — the walk-in who will never install an app still gets on the list from a code on the wall. Rejections render inline next to the box they'd retype in, not as a toast that vanishes mid-read. The limit is mirrored here for feedback but ENFORCED by the RPC. Test: `AddMyNameCard.test.tsx`.
