...
> **Last Updated**: 2026-05-24 (Win Calculator Module BUILT + cut over live — new `src/systems/win-calculator/` (singular): pure judge `decideWinner(state, config)` (chip-first `edge` override → ordered games/points comparators each most/met-goal → no-winner residue), code-defined `buildWinCalcConfig(win_condition)`, 32 tests incl. characterization parity; wired authoritative in `src/components/scoring/MatchEndVerification.tsx` with legacy kept as fallback + divergence auditor; the dead wrong-architecture scaffolding `src/systems/win-calculators/` (plural, metric-stack/edge) deleted and its inert required `winCalculator` field dropped from the `SystemModule` interface + all 4 assignment sites. Earlier: Win Calculator v2 RATIFIED — the v2 draft swapped into the locked `docs/league-system/modules/win-calculator.md` (pure judge: winner-chip `edge` override + 2 comparators games/points each most/met-goal + order filter; tie = no-winner residue handed to the runtime); the `win-calculator-v2.md` draft file removed. Also: team-geometry + 5v5-games-needed trimmed of Win-Calc-internals over-reach (gate-2 reconciliation); tiebreak + pairings firing-ownership deferred to their own v2s. Earlier: Win Calculator implementation plan added — `docs/plans/2026-05-23-001-feat-win-calculator-module-plan.md`: pure-judge module (2 comparators games+points, each most/met-goal, order filter, win-chip override, no tie slot), code-defined config, full live cutover phased; built against + validates the v2 draft (lists 2 required v2 trims). Earlier: Locked-doc revision protocol added — `docs/league-system/revision-protocol.md` (v2-draft-alongside + surgical-diff + 3-cold-read ratification gate). Win Calculator v2 DRAFT added — `docs/league-system/modules/win-calculator-v2.md`, the pure-judge + comparator-switch replacement for the locked win-calculator.md, rebuilt as a surgical diff of v1 (6 deltas); 2026-05-18 brainstorm marked superseded. Earlier: Strand-B points cutover begun: match-adapter (engine→4 totals) + legacy parity gate (1374 cases), then shadow-audit wired into updateMatchRunningTotals — new engine runs alongside legacy on real mutations and logs divergence, legacy still writes. Earlier: Trigger v2 enstated as canonical; corpus-wide benchmark→threshold standardization + synonym note; EOGA deprecated from canon (end-of-match scoring is a match_end trigger); per-game allocator confirmed a distinct primitive; concept-analogies.md canonized/locked. Earlier: Points System all four sub-mechanisms A/B/C/D audited + locked data-driven: triggers, threshold output enrichment, per-game allocator (SideConfig + formula registry), end-of-match aggregate (data-driven operation). Added 10-Point rename, 17-Point + behind-boost compositions, complex-trigger.md design concept. Earlier: Merged main into docs/league-system-l1 — Modular Scoring System framework + Module extraction (Team Geometry / Match Format / Handicap Systems / Threshold Charts / Handicap Mechanisms A→D). Main absorbed: Messaging Phase 1, Navigation IA, Scoring Modal rework, Fargo auto-agree start-points, modular league system v2, unified scoreboard.)
...
| `docs/league-system/modules/win-calculator.md` | Win Calculator Module — pure judge: winner-chip (`edge`) override + 2 comparators (games/points, each most/met-goal) + order filter; tie = no-winner residue handed to the runtime | LOCKED |
...
| `docs/plans/2026-05-23-001-feat-win-calculator-module-plan.md` | Implementation plan for the Win Calculator Module (pure judge) | 7 units across 3 phases (build judge → parity → live cutover + remove dead scaffolding); 2 comparators (games+points, each most/met-goal) + order filter + win-chip override, no tie slot; code-defined serializable config; built against the v2 draft, since ratified into win-calculator.md; branch `docs/win-calculator-endgame-brainstorm` |
