...
- `paid/StartTournamentPanel.tsx` - **PAID C3** — the Start control: a live "Starting with N players" count, the Start / Start & pay button, and the **"Also add the N still waiting, as unpaid"** checkbox. Default OFF and always named with its count — the waiting room is usually people standing there wanting to play, but a QR on a flyer also collects the curious, and a start can't be undone. Hidden when nobody is waiting. Test: `StartTournamentPanel.test.tsx`.
