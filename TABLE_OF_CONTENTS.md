...
> **Last Updated**: 2026-06-06 (Trigger room work in progress on `feat/trigger-room` — second standalone work room of the Scoring System Workshop building. Merged `feat/per-game-allocator-workshop` (PR #179) in as the base since the trigger room reuses the allocator's ExpressionBuilder + workshop home page. Trigger room Units 1-3 shipped: DB schema + 4 seeded officials + tamper trigger; never-throw loader; standalone `validateTrigger` extracted from `validatePointsSystem` with optional write-target whitelist.) Prior 2026-06-05: Allocator workshop merged main → workshop branch (brings in glossary, doc archive, onboarding, DB suite cleanup, scoring-end fix). 2026-06-02: Archived ~63 completed/superseded brainstorm, plan & memory-bank docs to `docs/archive/` + `memory-bank/archive/`.
