...
> **Last Updated**: 2026-06-03 (Threshold-math-modular branch COMPLETE. Pinned 7 core architectural principles at top of CLAUDE.md; rewrote plan v4 around the empty-state-bag model. All 5 units landed: chain-runtime engine, seed modules, threshold modules, `chain: Module[]` field on every SystemModule, and `useMatchPreparation` rewired to call `runSystemChain` then read named threshold keys from the resulting bag — zero literal handicap-type strings in code. Deleted dead helpers: `calculateHandicapThresholds.ts`, `getTeamHandicapBonus.ts`, and their characterization tests. 3561 tests passing across systems/hooks/utils. Branch `feat/threshold-math-modular`.)
