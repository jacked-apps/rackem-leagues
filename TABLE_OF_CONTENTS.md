...
| `scripts/clone-prod-to-local.mjs` | **Clone prod public data → local DB** (`pnpm clone-prod`, needs `PROD_DB_PASSWORD`). One-way: read-only `supabase db dump --data-only` of prod, loaded into the local `supabase_db_*` Docker container only (load target discovered from the running container, never reaches prod). Truncates local public + loads with FK/triggers off. Public-only (no `public→auth` FKs); keep logging in locally as usual. Output `prod_data.sql` is gitignored (real user data). |
