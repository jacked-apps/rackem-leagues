...
- `threshold/BuiltInView.tsx` - read-only "built-in calculation" view: explains a dedicated-math threshold (Fargo, points/percentage formula, read-a-pref, milestone) when cloned — its math isn't editable in the arithmetic builder (only charts are cell-editable).
