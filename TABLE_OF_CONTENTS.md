...
- `paid/ChargeBreakdownPanel.tsx` - **PAID C3** — a collapsed accordion on the checkout card: "2 features · $2.00", opening to the itemised list and what's charged at start. **The $5 cap gets its own line when it bites** — six features at $1 add to $6 while the button says $5, and a list that doesn't reconcile with the total reads like a bug even when the cheaper number is the honest one. Backed by pure `chargeBreakdown()` in `premiumFeatures.ts`, which is tested to always agree with `totalPriceCents`. Test: `ChargeBreakdownPanel.test.tsx`.
