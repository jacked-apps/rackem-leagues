...
- **Re-read PR #98 immediately before /compound-engineering:ce-work.** PR #98 is OPEN at plan time. If review feedback materially reshapes snapshot keys, threshold-trio columns, or calculator registry signatures, this plan needs revision. Flag any drift before starting.
...
| **PR #98 reshape during review.** PR #98 is OPEN at plan time; review feedback could rename columns, change snapshot keys, or alter calculator registry signatures. | Pre-`/compound-engineering:ce-work` step: re-read PR #98 and flag drift. Plan's deferred-to-implementation question explicitly calls this out. |
