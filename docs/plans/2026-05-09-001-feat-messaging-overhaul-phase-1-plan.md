...
- **Sync with Jack on the schema delta.** Quick conversation, write the outcome to `LIST_FOR_JACK.md`. Topics: (1) `messages.sender_id` becomes nullable — mobile types regen + null-safe rendering; (2) new `is_system` column with a strict CHECK constraint — mobile must NOT INSERT `is_system = true` from any client path; (3) new columns on `members` (`notifications_paused_until`, `profanity_onboarding_completed_at`, `deleted_at`) — mobile reads should tolerate; (4) new `notification_mode` column — mobile reads it preferentially over `is_muted` once Phase 2 lands. **This is not "deferred to a separate track" — it's a Unit 0 prerequisite for the trigger work in Unit 4.**
...
- Keep `is_muted` and `notifications_enabled` columns in place — they will be dropped in a Phase 2 cleanup migration once no code references them. **During the deprecation window, `notification_mode` is authoritative**; new code reads it, old code may still read `is_muted` until Phase 2's web client refactor lands. Mobile must regenerate types and read `notification_mode` before its next release.
...
| **Mobile partner reads break on new columns** | All schema changes are additive (no column removed, no NOT-NULL added on populated rows without backfill). Mobile reads should tolerate. Coordinate via `LIST_FOR_JACK.md` or a Slack note. |
...
- **Mobile partner coordination** — note in `LIST_FOR_JACK.md` that the messaging schema is being extended on the messaging-system-overhaul branch; mobile reads should tolerate (additive only) but flag the new `is_system`, `notification_mode`, `cannot_leave`, `archived_at` columns so mobile can plan when to read them.
