...
(Why this matters: the kind tells you what to expect. A composition-pattern System has N orthogonal axes; legal configurations are the constrained Cartesian product across them. Each axis is set independently; the *variant* is the resulting triple, not a packaged option pulled off a shelf. The two-scope bundling is a deliberate anti-conflation move — see [§Architectural intent](#architectural-intent-two-scopes-one-module-distinct-failure-modes).)
...
- **Forfeit policy** — what happens when a team can't field a lineup. Forfeits produce match outcomes (forfeit win/loss) that feed standings, but the rule for *what counts as a forfeit and its associated penalties* is a separate Forfeit Policy concern.
