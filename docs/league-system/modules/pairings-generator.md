...
- **Tiebreaker pairings** — when [Win Calculator](win-calculator.md)'s metric stack ends with all metrics tied and fires the [Tiebreak Mechanisms](tiebreak-mechanisms/README.md) chain, several Tiebreak Mechanism variants (e.g., `single_round`, `single_game`, `race_to_n`) need a pairing set for their extra-play games. Either a future variant of this Module (re-invoked with mini-match parameters) or a peer Tiebreaker Pairings Generator handles those slots. Today, the runtime appends hardcoded tiebreaker game slots (games 19-21 for 3v3 best-of-3); the Step-3+ refactor formalizes this as a distinct generator concern, likely consumed by the relevant Tiebreak Mechanisms at runtime.
