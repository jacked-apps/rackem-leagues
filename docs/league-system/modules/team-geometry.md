...
- **[Win Calculator](win-calculator.md)** — reads `game_count` parity (even vs odd) to inform whether match-night ties are mathematically possible. Odd game counts can't tie at the games metric; even counts can. This informs Win Calc's stack — a league with odd game counts has no need for the [Tiebreak Mechanisms](tiebreak-mechanisms/README.md) chain to fire on games-based wins.
