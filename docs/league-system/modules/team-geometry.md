...
| `lineup_size` consistent with downstream Modules' calibration — Pairings Generator's round-robin algorithm is universal (works for any lineup_size); the formula-shape [Points Games-Needed Formula](threshold-charts/3v3-games-needed.md) and [Percentage Games-Needed Formula](threshold-charts/5v5-games-needed.md) are parameterized by `game_count` and produce valid output for any team size | runtime evaluation in each downstream Module | no failure mode at the Team Geometry layer — any valid lineup_size produces a valid downstream chain |
