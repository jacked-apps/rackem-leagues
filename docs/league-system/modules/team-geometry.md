...
- **[Win Calculator](win-calculator.md)** — does not read `game_count` directly at runtime; its runtime inputs are accumulated game/points data plus Handicap Mechanism benchmarks. At config time, however, `game_count` parity (even vs odd) informs the LO's metric precedence stack composition: odd game counts can't tie at the games metric, so a league with odd `game_count` has no need to include `edge` (and thus fire the [Tiebreak System](tiebreak-system/README.md) chain) as a fallback for games-based ties. Even counts CAN tie at the games metric, making `edge` a useful fallback for those leagues.
