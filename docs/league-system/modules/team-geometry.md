...
| `lineup_size` consistent with [Pairings Generator](pairings-generator.md)'s currently-implemented variants — currently embedded in the hardcoded game-order tables | enforced implicitly by `src/utils/gameOrder.ts` — only the shipped triples are wired | unwired triples (e.g., 4v4 DRR) fall through to runtime warn-and-zero per Principle 10 graceful degradation |

The last row is the bridge to the Implementation status section: today, only the *three currently-shipped triples* are end-to-end wired through code. A new triple would compose validly at the Team Geometry layer but stall downstream at [Pairings Generator](pairings-generator.md) (because the game-order generation is partly hardcoded inside per-Scoring-System code, awaiting the Step-2 extraction of Pairings Generator as a centralized Module). This is a known implementation gap, not a Module-level violation.
