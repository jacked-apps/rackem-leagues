...
(Why this matters: the kind tells you what to expect inside. A parallel-pattern System has N orthogonal axes; legal configurations are the Cartesian product across them, gated by inter-axis validation. There is no "Team Geometry preset" — there's a `(lineup_size, max_roster_size, game_generation)` triple per league.)
...
| `lineup_size` and `game_generation` immutable post-season-lock (NOT `max_roster_size`) | `lock_tier1_preferences()` Postgres trigger applied to those two columns; `max_roster_size` is NOT in the Tier 1 lock set and may be updated mid-season | UPDATE on locked preferences blocked at DB layer for the two season-stable axes only |
| `lineup_size` consistent with downstream Modules' calibration — Pairings Generator's round-robin algorithm is universal (works for any lineup_size); the formula-shape [Points Games-Needed Formula](../modules/threshold-charts/3v3-games-needed.md) and [Percentage Games-Needed Formula](../modules/threshold-charts/5v5-games-needed.md) are parameterized by `game_count` and produce valid output for any team size | runtime evaluation in each downstream Module | no failure mode at the Team Geometry layer — any valid lineup_size produces a valid downstream chain |

The last row reflects the architectural commitment from the formula-shape Threshold Chart variants and the universal Pairings Generator: any valid `lineup_size` produces a valid downstream chain. The current code has team-size-specific shortcuts (hardcoded `gameOrder.ts` table for 3v3 DRR; inline computation for 5v5 SRR) that the Step-2 refactor will replace with the universal Pairings Generator implementation per its own blueprint. These shortcuts are implementation conveniences for the currently-shipped triples; they do not represent an architectural limitation on which triples are supported.
...
- **[Standings & Tiebreakers](standings-tiebreakers.md)** — reads `game_count` for two distinct purposes: (1) per-team game-win counts normalize against `game_count` for season-level standings (a team that wins 11 of 18 carries differently than 11 of 25); (2) the `game_count` parity (even vs odd) determines whether match-night ties are mathematically possible, which informs the `tiebreaker_trigger='even_total_games_only'` configuration. If `game_count` varied across the season, both normalization and the parity assumption would be ill-defined; the season-stability invariant prevents this.
...
- `supabase/migrations/20260418000002_lock_tier1_preferences.sql` — `lock_tier1_preferences()` Postgres trigger enforcing season-stability immutability on the Tier 1 columns (Team Geometry's `lineup_size` and `game_generation` sit in Tier 1; `max_roster_size` is NOT in Tier 1 and remains mutable mid-season)
