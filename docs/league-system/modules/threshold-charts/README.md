...
### Architectural intent: Charts are orthogonal to Mechanisms and Systems

Any Chart should be **composable** with any [Handicap System](../handicap-systems/README.md) whose output type the Chart accepts, and any [Handicap Mechanism](../handicap-mechanisms/README.md) that expects the Chart's output shape. When a Chart's input type doesn't match an upstream encoding's output type, the architecturally correct fix is a **[Converter](../../PRINCIPLES.md#converter--deep-dive)** — not bundling chart logic into the encoding.

**The current codebase couples specific Charts to specific encoding-runners** (e.g., the 3v3 Points Chart is hardcoded inside the `bca3v3` SystemModule rather than queried as an independent Chart Module). **This is implementation artifact, not architectural intent.** Step-2 refactors will lift Charts out as first-class Modules selected by the league configuration. See the [PRINCIPLES § Module — § 8 implication on orthogonality claims](../../PRINCIPLES.md#8-io-contracts-at-module-boundaries) for the honesty rule: orthogonality claims are only true once the necessary Converter exists.
