...
**Encoding-locked input is a contract, not a preference.** Every Chart — whether stored as a discrete table or expressed as a formula — has its input domain bound to ONE specific [Handicap System](../handicap-systems/README.md)'s output type. A Chart calibrated for FargoRate (100–850) cannot meaningfully consume a Points value (-2 to +2); the math is undefined outside the input domain and produces nonsense, not a bad answer. Pairing a Chart with any other Handicap System REQUIRES a **[Converter](../../PRINCIPLES.md#converter--deep-dive)** placed in front of the Chart to translate the upstream output into a value the Chart's input domain accepts.

**Output-shape coupling to the downstream Mechanism.** A Chart's output is shaped for ONE specific [Handicap Mechanism](../handicap-mechanisms/README.md) (asymmetric per-side game targets for `extra_games`; bonus-points value for `start_points`; per-pairing race lengths for `race_length_adjustment`; etc.). A Chart whose output shape doesn't match the active Mechanism's expected input is similarly un-composable without an output-side adapter.

**Composability promise (per [Principle 10 — Composability contract](../../PRINCIPLES.md#10-composability-contract--no-break-composition)).** Any LO-selected combination of encoding × Chart × Mechanism MUST chain to a runnable output. The bridging Converter is always already present in the architecture — its existence is a **precondition** Principle 10 enforces at the time a new Handicap System (or any new typed Module) is admitted: no Module is accepted without the adapters needed to bridge it to every existing type it can compose with. The Chart receives a value in its input domain; the match runs; a warning surfaces flagging untested combinations. The system never refuses, never errors — not because it fabricates fallbacks at runtime, but because the architecture refuses to admit Modules that would create un-bridgeable mismatches in the first place.
