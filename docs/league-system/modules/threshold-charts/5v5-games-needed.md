...
- **Even game_count:** target_stronger_base = half+1 and target_weaker_base = half. At diff=0 the chart returns a 1-gap pair (e.g., 10/9 for 18 games). A midpoint outcome (half wins for both teams) is a tie at the natural midpoint; what happens to that tie is the [Win Calculator](../win-calculator.md)'s decision, not the Chart's.
...
- **Parity-aware tie semantics.** Even game counts naturally produce a 1-wide tie band (a midpoint tie the [Win Calculator](../win-calculator.md) then resolves); odd game counts collapse to race mode (no ties possible).
...
- **Downstream tie-handling dependency for even game counts:** when game_count is even and the chart's 1-wide tie band is reachable, a midpoint outcome (`game_count/2, game_count/2`) produces a tie that the [Win Calculator](../win-calculator.md) resolves. Odd game counts have no reachable tie outcome.
