...
- **Parity-aware tie semantics.** Even game counts naturally produce a 1-wide tie band (allowing midpoint ties to be detected and routed to the Tiebreaker module); odd game counts collapse to race mode (no ties possible).
...
- **Asymmetric targets may leave unresolved middle configurations** — depending on `(diff, game_count)`, the two targets may sum to more than the total game count, leaving outcomes where neither side hits its target. Resolving that band is a downstream [Win Calculator](../win-calculator.md) + [Standings & Tiebreakers](../standings-tiebreakers.md) responsibility; an LO using this Chart needs to confirm both Modules handle that case (tie, overtime, tiebreaker games, secondary criterion) in a way the league accepts.
...
- **Downstream Standings & Tiebreakers dependency for even game counts:** when game_count is even and the chart's 1-wide tie band is reachable, a midpoint outcome (`game_count/2, game_count/2`) produces a tie that the [Standings & Tiebreakers](../standings-tiebreakers.md) Module resolves per its `tiebreaker_trigger` / `tiebreaker_format` configuration. Odd game counts have no reachable tie outcome.
