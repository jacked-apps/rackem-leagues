...
- **Migrations MUST be TIMESTAMPED, not just DATEstamped (full `YYYYMMDDHHMMSS`)**: Every file in `supabase/migrations/` is keyed by its leading timestamp — that IS its version. NEVER stamp a migration with a date plus `000000` (e.g. `20260604000000_...`). Two migrations that land on the same day with `000000` collide on the SAME version, and `supabase db push` aborts with `duplicate key ... schema_migrations_pkey, Key (version)=(...) already exists` — which **breaks the staging/prod deploy for everyone** until the duplicate is renamed (this happened: PR #193, and earlier #175). ALWAYS use a unique full timestamp down to the second — prefer `supabase migration new <name>` (it stamps real time), or if hand-naming, use the actual current time `HHMMSS`, never `000000`. Before adding a migration, check `supabase/migrations/` for any existing file sharing your date and pick a later second so versions are unique and strictly ordered.
